...
# Keep the pre-commit transition approximation aligned with the existing
# post-commit constraint/soft-reference stabilization window.
OUTER_CONSTRAINT_TRANSITION_SEC = 0.6
...
class MPCController(Node):
    ...
    def __init__(self, config_path: str, ref_vel_config_path: Optional[str]) -> None:
        ...
        # Performance metrics are initialized from _initialize(), so the ego
        # ID must exist before entering that initialization sequence.
    ...
    def _initialize(self) -> None:
        ...
        self._slow_lead_overtake_speed_margin = max(float(getattr(
            switch_cfg, "slow_lead_overtake_speed_margin", 0.5)), 0.0)
        self._slow_lead_overtake_committed_speed_margin = max(float(getattr(
            switch_cfg, "slow_lead_overtake_committed_speed_margin", 1.5)),
        ...
        self._slow_lead_overtake_commit_distance = min(max(float(getattr(
            switch_cfg, "slow_lead_overtake_commit_distance", 18.0)), 0.0),
        ...
        self._outer_candidate_reference_handoff = None
        ...
        self._l0_prohibited_soft_recovery_active = False
        ...
        self._constraint_diagnostics_enabled = bool(
            ENABLE_RUNTIME_DIAGNOSTICS
            and getattr(
        ...
        self._overtake_soft_transition_start_alpha = 0.0
        self._overtake_soft_transition_lateral_offset = 0.0
        self._overtake_soft_transition_lateral_targets = None
        ...
        self._prepass_timeout_candidate_lane_idx = None
        self._prepass_timeout_heading_error = None
        # One-cycle proof that the MPC solve requested by an ordinary
        # OvertakeHorizonMotionHold really used the Center full-width corridor.
        self._motionhold_validated_full_width = None
        # Ownership of the one-loop, candidate-conditioned full-width solve
        # used by every new outer-lane commit path.  This is guidance/proof
        # state only; it must never imply hard outer-lane ownership.
        self._outer_candidate_guidance = None
        # Last fresh Center/full-width solve available before the next solve.
        # Candidate guidance may consume this grid once, with strict loop and
        # prediction identity, to avoid a solve-time dependency cycle.
        self._latest_full_width_time_grid = None
        # One-solve time-aligned moving-V2X ownership captured from the exact
        # candidate prediction which authorized a new outer commit.
        self._outer_commit_time_aligned_v2x = None
        # Diagnostic-only snapshots used to compare the lane-center envelope
        # approximation with the first accurate MPC path after an outer commit.
        self._candidate_envelope_diagnostic_cache = {}
        self._outer_commit_prediction_diagnostic = None
        # Attempt-scoped history is deliberately separate from the episode-scoped
        # attempted lanes and timeout progress above.
        self._prepass_last_confirmed_commit = None
        self._prepass_short_failed_outer_lanes = set()
        ...
        if self.USE_OBSTACLE_AVOIDANCE:
            ...
            existing_map_data = self._map.data.copy()
            existing_map_obstacles = list(self._map.obstacles)
            self._map.reset_map()
            self._map.add_obstacles(self._static_obstacles)
            # Immutable occupancy used only while moving V2X is supplied per
            # spatial step.  It preserves walls and configured static objects.
            self._static_obstacle_map_data = self._map.data.copy()
            # Snapshot construction is observational. Preserve the exact map
            # state which the existing first control cycle expects.
            self._map.data = existing_map_data
            self._map.obstacles = existing_map_obstacles
            #動的障害物(V2Xなど他車両)
    ...
    def _initialize_performance_metrics(self) -> None:
        """Initialize event-driven metrics without changing controller state."""
        run_id = datetime.now().strftime("%Y%m%d_%H%M%S")
        # run_autoware.bash changes cwd to /output/<run>/d<id>, so the default
        # is persisted with the corresponding Autoware log on the host.
        base_dir = os.environ.get(
            "AICHALLENGE_METRICS_DIR",
            os.path.join(os.getcwd(), "mpc_metrics"),
        )
        self._metrics_dir = os.path.join(
            base_dir, f"{run_id}_{self._ego_vehicle_id}")
        self._summary_csv_path = os.path.join(
            self._metrics_dir, "run_summary.csv")
        self._overtake_csv_path = os.path.join(
            self._metrics_dir, "overtake_events.csv")
        self._metrics_run_started_sec = self._metrics_now_sec()
        self._metrics_active_overtake = None
        self._metrics_last_mpc_id = id(self._mpc)
        self._metrics_last_mpc_infeasibility_counter = int(
            self._mpc.infeasibility_counter)
        self._metrics_emergency_brake_active = False
        self._metrics_safety_recovery_started_sec = None
        self._metrics_summary_written = False
        self._metrics_total = self._new_metrics_bucket()
        self._metrics_lap = self._new_metrics_bucket()
        try:
            os.makedirs(self._metrics_dir, exist_ok=True)
            self._append_csv_row(
                self._summary_csv_path,
                [
                    "timestamp", "scope", "lap", "lap_time_sec",
                    "overtake_attempts", "overtake_successes",
                    "overtake_failures", "overtake_aborts",
                    "mpc_infeasible_count",
                    "safety_recovery_total_duration_sec",
                    "emergency_brake_count",
                ],
                None,
            )
            self._append_csv_row(
                self._overtake_csv_path,
                [
                    "timestamp", "event", "result", "target_vehicle_id",
                    "selected_lane", "opponent_distance_m",
                    "heading_error_deg", "lateral_speed_mps",
                    "ego_speed_mps", "opponent_speed_mps", "elapsed_sec",
                    "mpc_infeasible_count", "reason",
                ],
                None,
            )
            self.get_logger().info(
                f"[PerformanceMetrics] CSV output: {self._metrics_dir}")
        except OSError as exc:
            self.get_logger().warn(
                f"[PerformanceMetrics] failed to initialize CSV output: {exc}")

    @staticmethod
    def _new_metrics_bucket():
        return {
            "overtake_attempts": 0,
            "overtake_successes": 0,
            "overtake_failures": 0,
            "overtake_aborts": 0,
            "mpc_infeasible_count": 0,
            "safety_recovery_total_duration_sec": 0.0,
            "emergency_brake_count": 0,
        }

    def _metrics_now_sec(self) -> float:
        return float(self.get_clock().now().nanoseconds) / 1e9

    def _append_csv_row(self, path, header, row) -> None:
        """Append one event/summary row; header creation is also event-driven."""
        try:
            ...
            self.get_logger().warn(
                f"[PerformanceMetrics] CSV write failed ({path}): {exc}",
                throttle_duration_sec=5.0,
            )

    def _metrics_increment(self, name: str, amount=1) -> None:
        self._metrics_total[name] += amount
        self._metrics_lap[name] += amount

    def _metrics_observe_mpc_result(self) -> None:
        """Convert MPC's consecutive-failure counter into an accumulated count."""
        mpc_id = id(self._mpc)
        current = int(self._mpc.infeasibility_counter)
        if mpc_id != self._metrics_last_mpc_id:
            # A path switch changes the MPC instance. Its current consecutive
            # count belongs to that instance and has not been observed yet.
            delta = max(current, 0)
            self._metrics_last_mpc_id = mpc_id
        else:
            delta = max(
                current - self._metrics_last_mpc_infeasibility_counter, 0)
        if delta:
            self._metrics_increment("mpc_infeasible_count", delta)
            active = self._metrics_active_overtake
            if active is not None:
                active["mpc_infeasible_count"] += delta
        self._metrics_last_mpc_infeasibility_counter = current

    def _metrics_set_safety_recovery(self, active: bool, now_sec: float) -> None:
        if active and self._metrics_safety_recovery_started_sec is None:
            self._metrics_safety_recovery_started_sec = now_sec
        elif not active and self._metrics_safety_recovery_started_sec is not None:
            duration = max(
                now_sec - self._metrics_safety_recovery_started_sec, 0.0)
            self._metrics_increment(
                "safety_recovery_total_duration_sec", duration)
            self._metrics_safety_recovery_started_sec = None

    def _metrics_split_active_safety_recovery(self, now_sec: float) -> None:
        """Assign elapsed recovery time to the ending lap without ending it."""
        if self._metrics_safety_recovery_started_sec is None:
            return
        duration = max(
            now_sec - self._metrics_safety_recovery_started_sec, 0.0)
        self._metrics_increment("safety_recovery_total_duration_sec", duration)
        self._metrics_safety_recovery_started_sec = now_sec

    def _metrics_set_emergency_brake(self, active: bool) -> None:
        if active and not self._metrics_emergency_brake_active:
            self._metrics_increment("emergency_brake_count")
        self._metrics_emergency_brake_active = active

    def _metrics_start_overtake(
        self, now_sec, target_id, lane_idx, opponent_distance,
        pose, ego_speed, opponent_speed,
    ) -> None:
        if target_id is None or lane_idx not in (0, 2):
            return
        if self._metrics_active_overtake is not None:
            self._metrics_end_overtake(
                "aborted", now_sec, "superseded_by_new_committed_attempt",
                clear_short_commit_history=False,
            )
        candidate_heading = self._candidate_lane_heading(
            lane_idx, self._car.wp_id)
        heading_error = absolute_heading_difference(
            pose.theta, candidate_heading)
        measured_lateral_speed = abs(float(self._odom.twist.twist.linear.y))
        lateral_speed = max(
            measured_lateral_speed,
            abs(float(ego_speed) * math.sin(heading_error)),
        )
        active = {
            "start_sec": now_sec,
            "target_id": target_id,
        ...
        self._metrics_active_overtake = active
        self._metrics_increment("overtake_attempts")
        self._append_csv_row(
            self._overtake_csv_path, [],
            [
                f"{now_sec:.9f}", "start", "", target_id, f"L{lane_idx}",
                f"{float(opponent_distance):.3f}",
                f"{math.degrees(heading_error):.3f}",
                f"{lateral_speed:.3f}", f"{abs(float(ego_speed)):.3f}",
                f"{float(opponent_speed):.3f}", "", 0, "committed",
            ],
        )

    def _metrics_end_overtake(
        self, result: str, now_sec: float, reason: str,
        target_id=None, *, clear_short_commit_history: bool = True,
    ) -> None:
        active = self._metrics_active_overtake
        if active is None:
            return
        if target_id is not None and target_id != active["target_id"]:
            return
        counter_name = {
            "success": "overtake_successes",
            "failed": "overtake_failures",
            "aborted": "overtake_aborts",
        }[result]
        self._metrics_increment(counter_name)
        elapsed = max(now_sec - active["start_sec"], 0.0)
        self._append_csv_row(
            self._overtake_csv_path, [],
            [
                f"{now_sec:.9f}", "end", result, active["target_id"],
                f"L{active['lane_idx']}", "", "", "", "", "",
                f"{elapsed:.3f}", active["mpc_infeasible_count"], reason,
            ],
        )
        self._metrics_active_overtake = None
        if clear_short_commit_history:
            self._clear_prepass_short_commit_history()

    def _metrics_write_summary(self, scope, lap, lap_time, bucket) -> None:
        now_sec = self._metrics_now_sec()
        self._append_csv_row(
            self._summary_csv_path, [],
            [
                f"{now_sec:.9f}", scope, lap,
                "" if lap_time is None else f"{float(lap_time):.3f}",
                bucket["overtake_attempts"], bucket["overtake_successes"],
                bucket["overtake_failures"], bucket["overtake_aborts"],
                bucket["mpc_infeasible_count"],
                f"{bucket['safety_recovery_total_duration_sec']:.3f}",
                bucket["emergency_brake_count"],
            ],
        )
    ...
    def _pure_pursuit_feedback_is_safe(
        ...
        for index in range(steps + 1):
            rollout_x.append(float(predicted.x))
            rollout_y.append(float(predicted.y))
            rollout_times.append(float(index) * dt)
            ...
            if not lower <= e_y <= upper:
                return False, (
                    ...
                    rollout_x, rollout_y, rollout_times)
            if index < steps:
        ...
        return True, "ok", (rollout_x, rollout_y, rollout_times)

    def _pure_pursuit_rollout_is_v2x_safe(
        self, pose: Pose2D, rollout,
    ):
        """Validate one already-computed PP rollout against all active V2X."""
        result = {
            "vehicle_id": None,
            "distance": None,
            "longitudinal_clearance": None,
            "lateral_clearance": None,
        }
        if not hasattr(self, "_v2x_tracker"):
            return True, result
        rollout_x, rollout_y, rollout_times = rollout
        minimum_clearance = (
        ...
        for vehicle_id in self._v2x_tracker.active_vehicle_ids():
            ...
            vehicle_vx, vehicle_vy = self._v2x_tracker.velocity(vehicle_id)
            distance = math.hypot(
                float(vehicle_x) - float(pose.x),
                float(vehicle_y) - float(pose.y),
            )
            longitudinal_clearance = None
            lateral_clearance = None
            current_envelope_unsafe = False
            vehicle_frenet = self._center_frenet(vehicle_x, vehicle_y)
            if ego_frenet is not None and vehicle_frenet is not None:
                longitudinal = signed_closed_path_arc_distance(
                    ego_frenet[0], vehicle_frenet[0],
                    self._center_arc_total_length)
                if longitudinal is not None:
                    ...
                    lateral_clearance = lateral_vehicle_clearance(
                        abs(vehicle_frenet[1] - ego_frenet[1]),
                        float(self._cfg.bicycle_model.width),
                        self._v2x_parallel_vehicle_half_width,
                    )
                    # A distant front conflict remains allowed. Reject only a
                    # vehicle already inside the existing parallel envelope.
                    current_envelope_unsafe = bool(
                        longitudinal_clearance
                            <= self._parallel_safety_longitudinal_clearance
                        and lateral_clearance
                            <= self._parallel_warning_clearance
                    )
            moving_prediction_safe = prediction_clears_moving_vehicle(
                rollout_x,
                rollout_y,
                rollout_times,
                vehicle_x=vehicle_x,
            ...
            if current_envelope_unsafe or not moving_prediction_safe:
                result.update({
    ...
    def _wall_v2x_nearby_vehicle(self, pose: Pose2D):
        """Return the closest V2X vehicle in the existing parallel envelope."""
        if not hasattr(self, "_v2x_tracker"):
            return None
        ego_frenet = self._center_frenet(pose.x, pose.y)
        if ego_frenet is None:
            return None
        ego_s, ego_lateral = ego_frenet
        nearest = None
        for vehicle_id in self._v2x_tracker.active_vehicle_ids():
            samples = self._v2x_tracker._samples.get(vehicle_id)
            if not samples:
            ...
            _, vehicle_x, vehicle_y = samples[-1]
            vehicle_frenet = self._center_frenet(vehicle_x, vehicle_y)
            if vehicle_frenet is None:
                continue
            vehicle_s, vehicle_lateral = vehicle_frenet
            longitudinal = signed_closed_path_arc_distance(
                ego_s, vehicle_s, self._center_arc_total_length)
            if longitudinal is None or not (
                ...
                continue
            lateral_clearance = lateral_vehicle_clearance(
                abs(vehicle_lateral - ego_lateral),
                float(self._cfg.bicycle_model.width),
                self._v2x_parallel_vehicle_half_width,
            )
            longitudinal_clearance = longitudinal_vehicle_clearance(
            ...
            if (
                lateral_clearance > self._parallel_warning_clearance
                or longitudinal_clearance
                    > self._parallel_safety_longitudinal_clearance
            ):
                continue
            candidate = {
                ...
                "lateral_clearance": float(lateral_clearance),
            }
            if (
                nearest is None
                or lateral_clearance < nearest["lateral_clearance"]
            ):
                nearest = candidate
        return nearest

    def _wall_v2x_controlled_pass_is_safe(
        self, pose: Pose2D, target_id, *, mpc_fresh_accurate: bool,
    ):
        """Prove a wall-improving, V2X-clear pass with the fresh MPC rollout."""
        result = {
            "wall_improving": False,
            "v2x_safe": False,
            "pass_complete": False,
            "current_longitudinal": None,
            "terminal_longitudinal": None,
            "terminal_longitudinal_clearance": None,
            "current_wall_violation": math.inf,
            "terminal_wall_violation": math.inf,
        }
        if not mpc_fresh_accurate or target_id is None:
            return False, result
        prediction = self._fresh_accurate_current_mpc_prediction()
        if prediction is None:
            return False, result
        pred_x, pred_y, prediction_times = prediction

        half_width = 0.5 * float(self._cfg.bicycle_model.width)
        guard = float(getattr(
            self._cfg.mpc, "prediction_outer_boundary_guard", 0.10))

        def wall_violation(x, y):
            ...
            lower = float(waypoint.lb) + half_width + guard
            upper = float(waypoint.ub) - half_width - guard
            return max(lower - e_y, e_y - upper, 0.0)

        current_violation = wall_violation(pose.x, pose.y)
        prediction_violations = [
            wall_violation(x, y) for x, y in zip(pred_x, pred_y)
        ]
        terminal_violation = float(prediction_violations[-1])
        if current_violation <= 0.0:
            wall_improving = max(prediction_violations) <= 1e-6
        else:
            wall_improving = bool(
                max(prediction_violations) <= current_violation + 1e-6
                and terminal_violation < current_violation
            )
        result["wall_improving"] = wall_improving
        result["current_wall_violation"] = float(current_violation)
        result["terminal_wall_violation"] = terminal_violation
        ...
        for vehicle_id in self._v2x_tracker.active_vehicle_ids():
            ...
            _, vehicle_x, vehicle_y = samples[-1]
            vehicle_vx, vehicle_vy = self._v2x_tracker.velocity(vehicle_id)
            if not prediction_clears_moving_vehicle(
                pred_x,
                pred_y,
                prediction_times,
                vehicle_x=vehicle_x,
                ...
                v2x_safe = False
                break
        result["v2x_safe"] = v2x_safe

        target_samples = self._v2x_tracker._samples.get(target_id)
        if not target_samples:
            return False, result
        _, target_x, target_y = target_samples[-1]
        target_vx, target_vy = self._v2x_tracker.velocity(target_id)
        terminal_time = float(prediction_times[-1])
        target_terminal_x = float(target_x) + float(target_vx) * terminal_time
        target_terminal_y = float(target_y) + float(target_vy) * terminal_time
        current_longitudinal = self._center_longitudinal_between(
            pose.x, pose.y, target_x, target_y)
        terminal_longitudinal = self._center_longitudinal_between(
            float(pred_x[-1]), float(pred_y[-1]),
            target_terminal_x, target_terminal_y,
        )
        result["current_longitudinal"] = current_longitudinal
        result["terminal_longitudinal"] = terminal_longitudinal
        if terminal_longitudinal is not None:
            terminal_clearance = longitudinal_vehicle_clearance(
                terminal_longitudinal,
                self._parallel_ego_half_length,
                self._parallel_vehicle_half_length,
            )
            result["terminal_longitudinal_clearance"] = terminal_clearance
            result["pass_complete"] = bool(
                current_longitudinal is not None
                and terminal_longitudinal < current_longitudinal
                and terminal_longitudinal < 0.0
                and terminal_clearance > self._parallel_critical_clearance
            )

        safe = bool(
            result["wall_improving"]
            and result["v2x_safe"]
            and result["pass_complete"]
        )
        return safe, result
    ...
    def _stuck_recovery_acceleration(
        self, target_speed: float, actual_speed: float
    ) -> float:
        """Select longitudinal acceleration for the active recovery owner."""
        straight_reentry_drive_active = bool(
            self._straight_reentry_active
            and self._straight_reentry_direction > 0
            and self._current_gear_is_drive()
            and target_speed > 0.0
        )
        if straight_reentry_drive_active:
            # Straight reentry in DRIVE is an intentional low-speed forward
            # manoeuvre. Use the same feedback law and limits as ordinary
            # forward control instead of the non-driving recovery brake.
            return float(np.clip(
                self.KP * (target_speed - actual_speed),
                self._mpc_cfg.a_min,
                self._mpc_cfg.a_max,
            ))
        straight_reentry_reverse_active = bool(
            self._straight_reentry_active
            and self._straight_reentry_direction < 0
        ...
        reverse_driving = bool(
            ...
            or straight_reentry_reverse_active
        )
        if not reverse_driving:
            return -8.0
        if self._stuck_reverse_command_mode in (
            "negative_speed_positive_accel", "awsim_reverse_button"
        ):
            if self._stuck_reverse_acceleration_positive:
                return abs(self._stuck_reverse_acceleration)
            return -abs(self._stuck_reverse_acceleration)
        return self._stuck_reverse_acceleration
    ...
    def _ordinary_outer_lane_quality(self, pose, ego_speed: float):
        """Compare binary-safe outer lanes using existing V2X/wall margins."""
        prediction_limit = self._prepass_lane_fallback_prediction_sec
        prediction_times = sorted(set(
            [0.0, float(prediction_limit)]
            + [
                float(sample_time) for sample_time in self._v2x_t_samples
                if 0.0 <= float(sample_time) <= prediction_limit
            ]
        ))
        moving_vehicles = []
        for vehicle_id in self._v2x_tracker.active_vehicle_ids():
        ...
        half_width = 0.5 * float(self._cfg.bicycle_model.width)
        boundary_guard = max(float(getattr(
            self._cfg.mpc, "prediction_outer_boundary_guard", 0.10)), 0.0)
        minimum_clearance = half_width + self._v2x_vehicle_radius
        horizon_wp = car.get_closest_waypoint(pose.x, pose.y)
        quality_by_lane = {}
        limiting_vehicle_by_lane = {}
        for lane_idx in (0, 2):
            candidate_x = []
            candidate_y = []
            for prediction_time in prediction_times:
                estimated_x = (
                    pose.x
                    + float(ego_speed) * math.cos(pose.theta)
                    * prediction_time
                )
                estimated_y = (
                    pose.y
                    + float(ego_speed) * math.sin(pose.theta)
                    * prediction_time
                )
                wp_id = car.get_closest_waypoint(estimated_x, estimated_y)
                ...
                normal_angle = float(waypoint.psi) + math.pi / 2.0
                candidate_x.append(
                    float(waypoint.x) + lane_center * math.cos(normal_angle))
                candidate_y.append(
                    float(waypoint.y) + lane_center * math.sin(normal_angle))
            if candidate_x:
                vehicle_margin, limiting_vehicle_id = (
                    minimum_predicted_vehicle_margin(
                        candidate_x,
                        candidate_y,
                        prediction_times,
                ...
                vehicle_margin, limiting_vehicle_id = -math.inf, None
            wall_margin = math.inf
            for offset in range(self._mpcN_center.N + 1):
                lanes = reference_path.get_lane_bounds(horizon_wp + offset)
                if lane_idx >= len(lanes):
                    wall_margin = -math.inf
                    break
                upper, lower = lanes[lane_idx]
                ...
                wall_margin = min(wall_margin, lane_wall_margin)
            quality_by_lane[lane_idx] = (vehicle_margin, wall_margin)
            limiting_vehicle_by_lane[lane_idx] = limiting_vehicle_id
        return quality_by_lane, limiting_vehicle_by_lane, boundary_guard

    def _discard_outer_candidate_guidance(self, reason: str) -> None:
        record = getattr(self, "_outer_candidate_guidance", None)
        if record is None:
            return
        self._outer_candidate_guidance = None
        self.get_logger().debug(
            "[OuterCandidateGuidanceDiscarded] "
            f"vehicle_id={record['vehicle_id']}, lane=L{record['lane_idx']}, "
            f"source={record['source']}, reason={reason}"
        )

    @staticmethod
    def _valid_constraint_time_grid(values, expected_length):
        if values is None or len(values) != int(expected_length):
            return None
        result = [float(value) for value in values]
        if (
            any(not np.isfinite(value) or value < 0.0 for value in result)
            or any(
                later + 1e-9 < earlier
    ...
    def _validated_prior_full_width_time_grid(self):
        """Return only the immediately preceding solve's owned time grid."""
        record = getattr(self, "_latest_full_width_time_grid", None)
        if (
            record is None
            or record.get("control_loop") != self._loop - 1
            or record.get("mpc_id") != id(self._mpc)
            or record.get("reference_path_id")
                != id(self._mpc.model.reference_path)
            or record.get("controller_reference_path_id")
                != id(self._reference_path)
            or self._mpc.current_prediction is None
            or record.get("prediction_id") != id(self._mpc.current_prediction)
            or self._mpc.infeasibility_counter != 0
            or getattr(self._mpc, "used_prediction_fallback", False)
            or self._mpc.recovery_requested
            or self._mpc.time_budget_exceeded
            or not getattr(self._mpc, "last_solution_accurate", False)
        ):
            return None
        return self._valid_constraint_time_grid(
            record.get("constraint_times"), self._mpc.N)

    def _capture_latest_full_width_time_grid(
        self, *, full_width_ownership: bool,
    ) -> None:
        """Publish this solve's grid for the immediately following callback."""
        if not full_width_ownership:
            self._latest_full_width_time_grid = None
            return
        fresh = self._fresh_accurate_current_mpc_prediction()
        times = self._valid_constraint_time_grid(
            getattr(self._mpc, "current_constraint_prediction_times", None),
            self._mpc.N,
        )
        if fresh is None or times is None:
            self._latest_full_width_time_grid = None
            return
        self._latest_full_width_time_grid = {
            "control_loop": int(self._loop),
            "mpc_id": id(self._mpc),
            "reference_path_id": id(self._mpc.model.reference_path),
            "controller_reference_path_id": id(self._reference_path),
            "prediction_id": id(self._mpc.current_prediction),
            "constraint_times": tuple(times),
        }

    def _activate_outer_candidate_time_aligned_v2x(
        self, *, guidance_applied: bool, applied_lane_idx,
    ):
        """Install a prior solve's grid on one candidate-guided solve."""
        record = self._outer_candidate_guidance
        if (
            not guidance_applied
            or applied_lane_idx is not None
            or record is None
            or record.get("lane_idx") not in (0, 2)
            or self._mpc is not self._mpcN_center
            or self._reference_path is not self._reference_pathN_center
            or getattr(self, "_static_obstacle_map_data", None) is None
        ):
            return None
        times = self._validated_prior_full_width_time_grid()
        if times is None:
            return None
        prior = self._latest_full_width_time_grid
        self._mpc.dynamic_v2x_by_step = self._time_aligned_v2x_by_step(times)
        self._mpc.static_occupancy_data = self._static_obstacle_map_data
        return {
            "vehicle_id": record["vehicle_id"],
            "lane_idx": int(record["lane_idx"]),
            "source": str(record["source"]),
            "candidate_loop": int(self._loop),
            "grid_source_loop": int(prior["control_loop"]),
            "grid_source_mpc_id": int(prior["mpc_id"]),
            "grid_source_reference_path_id": int(prior["reference_path_id"]),
            "grid_source_prediction_id": int(prior["prediction_id"]),
            "constraint_times": tuple(times),
        }

    def _capture_outer_candidate_reference_handoff(
        self, *, vehicle_id, lane_idx: int, source: str,
    ) -> bool:
        """Preserve the proved candidate reference across its outer commit."""
        record = self._outer_candidate_guidance
        if (
            record is None
            or lane_idx not in (0, 2)
            or record["vehicle_id"] != vehicle_id
            or record["lane_idx"] != int(lane_idx)
            or not self._outer_candidate_guided_prediction_ready(
                vehicle_id, lane_idx)
            or self._mpcN_center.soft_target_lane_idx != int(lane_idx)
        ):
            self._outer_candidate_reference_handoff = None
            return False

        lateral_targets = self._mpcN_center.soft_lateral_targets
        constraint_times = self._validated_candidate_used_time_grid(
            vehicle_id, lane_idx, rebase=False)
        if constraint_times is None:
            self._outer_candidate_reference_handoff = None
            self._outer_commit_time_aligned_v2x = None
            return False
        self._outer_candidate_reference_handoff = {
            "vehicle_id": vehicle_id,
            "lane_idx": int(lane_idx),
            "source": source,
            "control_loop": int(self._loop),
            "start_e_y": float(self._mpcN_center.soft_target_start_e_y),
            "alpha": float(self._mpcN_center.soft_target_alpha),
            "lateral_offset": float(
                self._mpcN_center.soft_target_lateral_offset),
            "lateral_targets": (
                None if lateral_targets is None
                else np.asarray(lateral_targets, dtype=float).copy()
            ),
        }
        self._outer_commit_time_aligned_v2x = {
            "source": str(source),
            "vehicle_id": vehicle_id,
            "lane_idx": int(lane_idx),
            "guidance_loop": int(record["guidance_loop"]),
            "prediction_loop": int(record["prediction_loop"]),
            "mpc_id": id(self._mpc),
            "prediction_id": id(self._mpc.current_prediction),
            "commit_loop": int(self._loop),
            "constraint_times": tuple(constraint_times),
            "candidate_state_time_sec": record["candidate_state_time_sec"],
            "grid_source": "candidate_used_grid",
        }
        return True

    def _outer_candidate_reference_handoff_matches(
        self, *, source: str, vehicle_id, lane_idx: int, control_loop: int
    ) -> bool:
        """Return whether a captured handoff belongs to this exact commit."""
        handoff = self._outer_candidate_reference_handoff
        return bool(
            handoff is not None
            and handoff.get("source") == source
            and handoff.get("vehicle_id") == vehicle_id
            and handoff.get("lane_idx") == int(lane_idx)
            and handoff.get("control_loop") == int(control_loop)
        )

    def _arm_outer_candidate_guidance(
        self, *, vehicle_id, lane_idx: int, source: str, now_sec: float
    ) -> None:
        if lane_idx not in (0, 2) or vehicle_id is None:
            self._discard_outer_candidate_guidance("candidate_unavailable")
            return
        record = self._outer_candidate_guidance
        if (
            record is not None
            and record["vehicle_id"] == vehicle_id
            and record["lane_idx"] == int(lane_idx)
        ...
        furthest_target = max(
            target_centers, key=lambda value: abs(value - start_e_y))
        ramp_sec = lateral_reference_ramp_duration(
            start_e_y,
            furthest_target,
            self._l1_soft_rejoin_ramp_sec,
            self._race_handoff_max_reference_speed,
        )
        self._outer_candidate_guidance = {
            "vehicle_id": vehicle_id,
            "lane_idx": int(lane_idx),
            "source": source,
            "guidance_loop": int(self._loop),
            "guidance_started_at": float(now_sec),
            "start_e_y": start_e_y,
            "target_e_y": float(furthest_target),
            ...
            "grid_source_loop": None,
            "grid_source_mpc_id": None,
            "grid_source_reference_path_id": None,
            "grid_source_prediction_id": None,
            "reference_path_id": None,
            "ready_logged": False,
            "last_guidance_alpha": None,
        }
        self.get_logger().info(
            "[OuterCandidateGuidanceArmed] "
            f"vehicle_id={vehicle_id}, candidate_lane=L{lane_idx}, "
            f"source={source}, loop={self._loop}, start_e_y={start_e_y:.3f}, "
            f"target_e_y={float(furthest_target):.3f}"
        )

    def _outer_candidate_guided_prediction_ready(
        self, vehicle_id, lane_idx: int
    ) -> bool:
        ...
        return bool(
            ...
            and record["vehicle_id"] == vehicle_id
            and record["lane_idx"] == int(lane_idx)
            and record["prediction_loop"] == self._loop - 1
            and record["mpc_id"] == id(self._mpc)
            and self._mpc.current_prediction is not None
            and record["prediction_id"] == id(self._mpc.current_prediction)
            and record.get("candidate_solve_time_aligned", False)
            and self._validated_candidate_used_time_grid(
                vehicle_id, lane_idx) is not None
            and self._validated_candidate_constraint_time_grid(
                vehicle_id, lane_idx) is not None
            and self._fresh_accurate_current_mpc_prediction() is not None
            and self._rebased_candidate_world_prediction(
                vehicle_id, lane_idx) is not None
        )

    def _candidate_prediction_age_sec(self, record=None, now_sec=None):
        """Return ROS/sim age of the state which seeded a candidate solve."""
        record = self._outer_candidate_guidance if record is None else record
        if record is None:
            return None
        state_time = record.get("candidate_state_time_sec")
        if state_time is None or not np.isfinite(state_time):
            return None
        if now_sec is None:
            now_sec = float(self.get_clock().now().nanoseconds) / 1e9
        age = float(now_sec) - float(state_time)
        if not np.isfinite(age) or age < 0.0:
            return None
        return age

    @staticmethod
    def _rebase_candidate_prediction(pred_x, pred_y, times, age_sec):
        """Drop consumed world samples and rebase the rest to current time."""
        if (
            age_sec is None or not np.isfinite(age_sec) or age_sec < 0.0
            or len(pred_x) != len(pred_y) or len(pred_x) != len(times)
        ):
            return None
        remaining = [
            index for index, value in enumerate(times)
            if np.isfinite(value) and float(value) - float(age_sec) >= 0.0
        ]
        if not remaining:
            return None
        return (
            [float(pred_x[index]) for index in remaining],
            [float(pred_y[index]) for index in remaining],
            [float(times[index]) - float(age_sec) for index in remaining],
            len(times) - len(remaining),
        )

    def _rebased_candidate_world_prediction(self, vehicle_id, lane_idx: int):
        record = getattr(self, "_outer_candidate_guidance", None)
        if (
            record is None
            or record.get("vehicle_id") != vehicle_id
            or record.get("lane_idx") != int(lane_idx)
        ):
            return None
        fresh = self._fresh_accurate_current_mpc_prediction()
        if fresh is None:
            return None
        pred_x, pred_y, times = fresh
        return self._rebase_candidate_prediction(
            pred_x, pred_y, times,
            self._candidate_prediction_age_sec(record),
        )

    @staticmethod
    def _rebase_candidate_constraint_grid(times, age_sec):
        """Rebase without changing constraint-index/waypoint-index ownership."""
        if age_sec is None or not np.isfinite(age_sec) or age_sec < 0.0:
            return None
        values = [float(value) for value in times]
        if not values or max(values) - float(age_sec) < 0.0:
            return None
        return [max(value - float(age_sec), 0.0) for value in values]

    def _validated_candidate_constraint_time_grid(
        self, vehicle_id, lane_idx: int,
    ):
        """Return the exact N-point grid owned by this candidate proof."""
        record = getattr(self, "_outer_candidate_guidance", None)
        if (
            record is None
            ...
            or record.get("vehicle_id") != vehicle_id
            or record.get("lane_idx") != int(lane_idx)
            or record.get("prediction_loop") != self._loop - 1
            or record.get("mpc_id") != id(self._mpc)
            or self._mpc.current_prediction is None
            or record.get("prediction_id") != id(self._mpc.current_prediction)
            or self._mpc.infeasibility_counter != 0
            ...
            return None
        times = record.get("constraint_time_grid_result")
        values = self._valid_constraint_time_grid(times, self._mpc.N)
        if values is None:
            return None
        world_times = self._current_mpc_prediction_times(
            len(self._mpc.current_prediction[0]))
        if (
            ...
            or world_times is None
            or len(values) < 2
            or len(world_times) != len(values[1:-1])
            or any(
                abs(float(actual) - float(expected)) > 1e-6
                for actual, expected in zip(world_times, values[1:-1])
            )
        ):
            return None
        return values

    def _validated_candidate_used_time_grid(
        self, vehicle_id, lane_idx: int, *, rebase=True,
    ):
        """Return the grid actually used to build this candidate's bounds."""
        record = getattr(self, "_outer_candidate_guidance", None)
        if (
            record is None
            or not record.get("candidate_solve_time_aligned", False)
            or record.get("vehicle_id") != vehicle_id
            or record.get("lane_idx") != int(lane_idx)
            or record.get("prediction_loop") != self._loop - 1
            or record.get("mpc_id") != id(self._mpc)
            or record.get("reference_path_id")
                != id(self._mpc.model.reference_path)
            or record.get("grid_source_loop")
                != record.get("prediction_loop") - 1
            or record.get("grid_source_mpc_id") != id(self._mpc)
            or record.get("grid_source_reference_path_id")
                != id(self._mpc.model.reference_path)
            or record.get("grid_source_prediction_id") is None
            or self._mpc.current_prediction is None
            or record.get("prediction_id") != id(self._mpc.current_prediction)
        ):
            return None
        values = self._valid_constraint_time_grid(
            record.get("constraint_time_grid_used"), self._mpc.N)
        if values is None or not rebase:
            return values
        return self._rebase_candidate_constraint_grid(
            values, self._candidate_prediction_age_sec(record))

    def _time_aligned_v2x_by_step(self, constraint_times):
        """Predict every active V2X once at each constraint-owned time."""
        result = [[] for _ in constraint_times]
        for vehicle_id in self._v2x_tracker.active_vehicle_ids():
            positions = self._v2x_tracker.predict_positions(
                vehicle_id, constraint_times)
            if len(positions) != len(constraint_times):
                continue
            for step, ((vehicle_x, vehicle_y), prediction_time) in enumerate(
                zip(positions, constraint_times)
            ):
                obstacle = Obstacle(
                    cx=float(vehicle_x),
                    cy=float(vehicle_y),
                    radius=self._v2x_vehicle_radius,
                )
                obstacle.vehicle_id = vehicle_id
                obstacle.prediction_time = float(prediction_time)
                result[step].append(obstacle)
        return [self._filter_obstacles_to_corridor(step) for step in result]

    def _activate_outer_commit_time_aligned_v2x(self) -> bool:
        """Install step-specific V2X on the exact authorized commit solve."""
        record = self._outer_commit_time_aligned_v2x
        active = bool(
            record is not None
            and record["commit_loop"] == self._loop
            and record["lane_idx"] == self._target_lane_idx
            and record["mpc_id"] == id(self._mpc)
            and self._mpc.current_prediction is not None
            and record["prediction_id"] == id(self._mpc.current_prediction)
            ...
            and getattr(self, "_static_obstacle_map_data", None) is not None
        )
        if not active:
            self._mpc.dynamic_v2x_by_step = None
            self._mpc.static_occupancy_data = None
            return False
        times = self._rebase_candidate_constraint_grid(
            record["constraint_times"],
            self._candidate_prediction_age_sec(record),
        )
        if times is None:
            self._mpc.dynamic_v2x_by_step = None
        ...
        self._mpc.static_occupancy_data = self._static_obstacle_map_data
        self.get_logger().info(
            "[OuterCommitTimeAlignedV2X] "
            f"source={record['source']}, "
            f"target_vehicle_id={record['vehicle_id']}, "
            f"lane=L{record['lane_idx']}, "
            f"prediction_loop={record['prediction_loop']}, "
            "time_grid_valid=True, "
            f"time_grid_first={times[0]:.3f}, "
            f"time_grid_last={times[-1]:.3f}, "
            f"active_v2x_count={len(self._v2x_tracker.active_vehicle_ids())}, "
            f"grid_source={record.get('grid_source', 'unknown')}, "
            "mode=time_aligned"
        )
        return True

    def _candidate_lane_center_prediction(
        self, pose, ego_speed: float, lane_idx: int, vehicle_id=None,
        *, include_consumed=False, timing=None,
        geometry_frenet_cache=None,
    ):
        """Return only the fresh MPC path solved with this lane's guidance."""
        prediction_prepare_start = (
            time.perf_counter() if timing is not None else None)
        if lane_idx not in (0, 2):
            if timing is not None:
                timing["prediction_prepare_ms"] += (
                    time.perf_counter() - prediction_prepare_start) * 1000.0
            return None
        if not self._outer_candidate_guided_prediction_ready(
            vehicle_id, lane_idx
        ):
            if timing is not None:
                timing["prediction_prepare_ms"] += (
        ...
        fresh_prediction = self._rebased_candidate_world_prediction(
            vehicle_id, lane_idx)
        if fresh_prediction is None:
            if timing is not None:
                timing["prediction_prepare_ms"] += (
                    time.perf_counter() - prediction_prepare_start) * 1000.0
            return None
        pred_x, pred_y, prediction_times, consumed_samples = fresh_prediction
        candidate_x = [float(value) for value in pred_x]
        candidate_y = [float(value) for value in pred_y]
        prediction_times = [float(value) for value in prediction_times]
        if (
            not candidate_x
            or len(candidate_x) != len(candidate_y)
            or len(candidate_x) != len(prediction_times)
            or any(not np.isfinite(value) for value in candidate_x)
            or any(not np.isfinite(value) for value in candidate_y)
        ):
            if timing is not None:
                timing["prediction_prepare_ms"] += (
                    time.perf_counter() - prediction_prepare_start) * 1000.0
        ...
        geometry_prepare_start = (
            time.perf_counter() if timing is not None else None)
        samples = []
        for step, (prediction_time, sample_x, sample_y) in enumerate(zip(
            prediction_times, candidate_x, candidate_y
        )):
            frenet_cache_key = (float(sample_x), float(sample_y))
            if (
                geometry_frenet_cache is not None
                and frenet_cache_key in geometry_frenet_cache
            ):
                frenet = geometry_frenet_cache[frenet_cache_key]
                if timing is not None:
                    timing["geometry_frenet_cache_hits"] = (
                        timing.get("geometry_frenet_cache_hits", 0) + 1)
            else:
                frenet = self._center_frenet(sample_x, sample_y)
                if geometry_frenet_cache is not None:
                    geometry_frenet_cache[frenet_cache_key] = frenet
                if timing is not None:
                    timing["geometry_frenet_cache_misses"] = (
                        timing.get("geometry_frenet_cache_misses", 0) + 1)
                    timing["geometry_center_frenet_call_count"] = (
                        timing.get(
                            "geometry_center_frenet_call_count", 0) + 1)
            samples.append({
                "step": step,
                "time": float(prediction_time),
                "x": float(sample_x),
                "y": float(sample_y),
                "s": None if frenet is None else float(frenet[0]),
                "e_y": None if frenet is None else float(frenet[1]),
            })
        if timing is not None:
            timing["geometry_prepare_ms"] += (
                time.perf_counter() - geometry_prepare_start) * 1000.0
        if include_consumed:
            return (
                candidate_x, candidate_y, prediction_times, samples,
                consumed_samples,
            )
        return candidate_x, candidate_y, prediction_times, samples

    def _candidate_guided_prediction_terminal_fit(
        self, pose, ego_speed: float, lane_idx: int, vehicle_id=None,
        *, timing=None, geometry_frenet_cache=None,
    ):
        """Verify that the solved candidate trajectory reaches its outer lane."""
        candidate_prediction = self._candidate_lane_center_prediction(
            pose, ego_speed, lane_idx, vehicle_id, timing=timing,
            geometry_frenet_cache=geometry_frenet_cache,
        )
        if candidate_prediction is None:
            return None
        terminal_fit_start = (
            time.perf_counter() if timing is not None else None)
        candidate_x, candidate_y, _, diagnostic_samples = candidate_prediction
        if not candidate_x or len(candidate_x) != len(candidate_y):
            if timing is not None:
                timing["terminal_fit_ms"] += (
                    time.perf_counter() - terminal_fit_start) * 1000.0
            return None

        terminal_x = float(candidate_x[-1])
        terminal_y = float(candidate_y[-1])
        terminal_lane_error = self._lane_lateral_error(
            terminal_x, terminal_y, int(lane_idx))
        terminal_frenet_cache_key = (terminal_x, terminal_y)
        if (
            geometry_frenet_cache is not None
            and terminal_frenet_cache_key in geometry_frenet_cache
        ):
            terminal_frenet = geometry_frenet_cache[
                terminal_frenet_cache_key]
            if timing is not None:
                timing["geometry_frenet_cache_hits"] = (
                    timing.get("geometry_frenet_cache_hits", 0) + 1)
        else:
            terminal_frenet = self._center_frenet(terminal_x, terminal_y)
            if geometry_frenet_cache is not None:
                geometry_frenet_cache[
                    terminal_frenet_cache_key] = terminal_frenet
            if timing is not None:
                timing["geometry_frenet_cache_misses"] = (
                    timing.get("geometry_frenet_cache_misses", 0) + 1)
                timing["geometry_center_frenet_call_count"] = (
                    timing.get("geometry_center_frenet_call_count", 0) + 1)
        terminal_e_y = (
            None if terminal_frenet is None else float(terminal_frenet[1]))

        candidate_lane_center_e_y = None
        terminal_wp_id = self._carN_center.get_closest_waypoint(
            terminal_x, terminal_y)
        terminal_lanes = self._reference_pathN_center.get_lane_bounds(
            terminal_wp_id)
        if 0 <= int(lane_idx) < len(terminal_lanes):
            upper, lower = terminal_lanes[int(lane_idx)]
            candidate_lane_center_e_y = 0.5 * (float(upper) + float(lower))

        allowed_error = float(
            self._slow_lead_speed_match_release_lateral_error)
        fit = bool(
            np.isfinite(terminal_lane_error)
            and terminal_lane_error <= allowed_error
        )
        result = {
            "terminal_x": terminal_x,
            "terminal_y": terminal_y,
            "terminal_e_y": terminal_e_y,
            "candidate_lane_center_e_y": candidate_lane_center_e_y,
            "terminal_lane_error": float(terminal_lane_error),
            "allowed_error": allowed_error,
            "fit": fit,
            "terminal_sample": diagnostic_samples[-1],
        }
        if timing is not None:
            timing["terminal_fit_ms"] += (
                time.perf_counter() - terminal_fit_start) * 1000.0
    ...
    def _log_outer_candidate_trajectory_fit(
        self, *, source, vehicle_id, lane_idx: int, fit_result
    ) -> None:
        if not ENABLE_RUNTIME_DIAGNOSTICS:
            return
        if fit_result is None:
            return
        fit = bool(fit_result["fit"])
        self.get_logger().info(
            "[OuterCandidateTrajectoryFit] "
            f"source={source}, vehicle_id={vehicle_id}, "
            f"candidate_lane=L{lane_idx}, "
            f"terminal_e_y={fit_result['terminal_e_y']}, "
            "candidate_lane_center_e_y="
            f"{fit_result['candidate_lane_center_e_y']}, "
            "terminal_lane_error="
            f"{fit_result['terminal_lane_error']:.3f}m, "
            f"allowed_error={fit_result['allowed_error']:.3f}m, "
            f"fit={fit}, action="
            f"{'allow_commit' if fit else 'hold_full_width_guidance'}",
            throttle_duration_sec=1.0,
        )

    def _candidate_lane_predicted_envelope_conflict(
        self, pose, ego_speed: float, lane_idx: int, vehicle_id=None,
        *, timing=None, collision_timing=None, geometry_frenet_cache=None,
    ):
        """Check a continuous outer transition against every active V2X.

        This is a pre-commit rejection gate, not a shadow MPC solve. Keep the
        current pose as the first point so an envelope which already overlaps
        cannot be hidden by the prospective lateral transition.
        """
        candidate_prediction = self._candidate_lane_center_prediction(
            pose, ego_speed, lane_idx, vehicle_id,
            include_consumed=True, timing=timing,
            geometry_frenet_cache=geometry_frenet_cache,
        )
        if candidate_prediction is None:
            return None
        candidate_x, candidate_y, prediction_times, diagnostic_samples, consumed_samples = (
            candidate_prediction)
        dynamic_v2x_prepare_start = (
            time.perf_counter() if timing is not None else None)
        moving_vehicles = []
        for vehicle_id in self._v2x_tracker.active_vehicle_ids():
            samples = self._v2x_tracker._samples.get(vehicle_id)
            ...
            vehicle_vx, vehicle_vy = self._v2x_tracker.velocity(vehicle_id)
            moving_vehicles.append((
                vehicle_id, float(vehicle_x), float(vehicle_y),
                float(vehicle_vx), float(vehicle_vy),
            ))
        if timing is not None:
            timing["dynamic_v2x_prepare_ms"] += (
                time.perf_counter() - dynamic_v2x_prepare_start) * 1000.0
        # This cache is observational only. Commit sites consume the exact ego
        # and traffic samples which the safety gate evaluated in this loop.
        if ENABLE_RUNTIME_DIAGNOSTICS:
            self._candidate_envelope_diagnostic_cache[int(lane_idx)] = {
                "control_loop": int(self._loop),
                "prediction_source": "candidate_guided_full_width_mpc",
                "prediction_age": self._candidate_prediction_age_sec(),
                "candidate_state_time_sec": self._outer_candidate_guidance.get(
                    "candidate_state_time_sec"),
                "consumed_prediction_samples": int(consumed_samples),
                "samples": diagnostic_samples,
                "moving_vehicles": list(moving_vehicles),
            }
        envelope_collision_start = (
            time.perf_counter() if timing is not None else None)
        conflict = minimum_predicted_envelope_conflict(
            candidate_x, candidate_y, prediction_times, moving_vehicles,
            project_frenet=self._center_frenet,
            arc_total_length=self._center_arc_total_length,
            ego_width=float(self._cfg.bicycle_model.width),
            other_half_width=self._v2x_parallel_vehicle_half_width,
            ego_half_length=self._parallel_ego_half_length,
            other_half_length=self._parallel_vehicle_half_length,
            timing=collision_timing,
            external_candidate_frenet_cache=geometry_frenet_cache,
        )
        if timing is not None:
            timing["envelope_collision_ms"] += (
                time.perf_counter() - envelope_collision_start) * 1000.0
        if conflict is None:
            return None
        ego_current_frenet = self._center_frenet(pose.x, pose.y)
        blocking_samples = self._v2x_tracker._samples.get(
            conflict["vehicle_id"])
        vehicle_current_frenet = None
        if blocking_samples:
            _, vehicle_x, vehicle_y = blocking_samples[-1]
            vehicle_current_frenet = self._center_frenet(vehicle_x, vehicle_y)
        conflict["current_lateral_clearance"] = None
        if ego_current_frenet is not None and vehicle_current_frenet is not None:
            conflict["current_lateral_clearance"] = lateral_vehicle_clearance(
                vehicle_current_frenet[1] - ego_current_frenet[1],
                float(self._cfg.bicycle_model.width),
                self._v2x_parallel_vehicle_half_width,
            )
        return conflict
    ...
    @staticmethod
    def _prediction_diagnostic_interpolate(samples, sample_time, key):
        """Linearly interpolate a diagnostic field on its recorded times."""
        valid = [
            sample for sample in samples
            if sample.get(key) is not None and sample.get("time") is not None
        ]
        if not valid:
            return None
        if sample_time <= valid[0]["time"]:
            return float(valid[0][key])
        if sample_time >= valid[-1]["time"]:
            return float(valid[-1][key])
        for left, right in zip(valid, valid[1:]):
            if left["time"] <= sample_time <= right["time"]:
                duration = float(right["time"] - left["time"])
                if duration <= 0.0:
                    return float(left[key])
                alpha = (float(sample_time) - left["time"]) / duration
                return float(left[key]) + alpha * (
                    float(right[key]) - float(left[key]))
        return None

    def _current_mpc_prediction_times(self, prediction_length):
        """Return times paired with the current spatial MPC prediction."""
        times = getattr(self._mpc, "current_prediction_times", None)
        if times is None or len(times) != int(prediction_length):
            return None
        result = [float(value) for value in times]
        if (
            any(not np.isfinite(value) or value < 0.0 for value in result)
            or any(
                later + 1e-9 < earlier
    ...
    def _fresh_accurate_current_mpc_prediction(self):
        """Return one fresh actual MPC trajectory with its spatial times."""
        if (
            self._mpc.current_prediction is None
            or self._mpc.infeasibility_counter != 0
            or getattr(self._mpc, "used_prediction_fallback", False)
            or self._mpc.recovery_requested
            or self._mpc.time_budget_exceeded
        ...
        if not pred_x or len(pred_x) != len(pred_y):
            return None
        prediction_times = self._current_mpc_prediction_times(len(pred_x))
        if prediction_times is None:
            return None
        return pred_x, pred_y, prediction_times

    @staticmethod
    def _format_prediction_diagnostic_samples(samples, include_delta=False):
        fields = []
        for sample in samples:
            e_y = sample.get("e_y")
            spatial_step = sample.get("spatial_step")
            step_label = str(sample["step"])
            if spatial_step is not None:
                step_label += f"(spatial={spatial_step})"
            field = (
                f"{step_label}@{sample['time']:.3f}:"
                f"x={sample['x']:.3f},y={sample['y']:.3f},"
                f"e_y={e_y:.3f}" if e_y is not None else
                f"{step_label}@{sample['time']:.3f}:"
                f"x={sample['x']:.3f},y={sample['y']:.3f},e_y=none"
            )
            if include_delta:
                delta = sample.get("delta_e_y")
                field += (
                    f",delta_e_y={delta:.3f}"
                    if delta is not None else ",delta_e_y=none")
            progress = sample.get("transition_progress")
            if progress is not None:
                field += f",transition_progress={float(progress):.3f}"
            fields.append(field)
        return "[" + "; ".join(fields) + "]"

    def _start_outer_commit_prediction_diagnostic(
        self, *, source: str, vehicle_id, lane_idx: int, pose,
        speed: float, now_sec: float,
    ) -> None:
        """Attach the gate's exact lane-center samples to a new commit."""
        if not ENABLE_RUNTIME_DIAGNOSTICS:
            return
        cached = self._candidate_envelope_diagnostic_cache.get(int(lane_idx))
        if cached is None or cached["control_loop"] != self._loop:
            return
        candidate_samples = [dict(sample) for sample in cached["samples"]]
        ego_frenet = self._center_frenet(pose.x, pose.y)
        closest_wp = self._carN_center.get_closest_waypoint(pose.x, pose.y)
        lanes = self._reference_pathN_center.get_lane_bounds(closest_wp)
        target_lane_e_y = None
        if int(lane_idx) < len(lanes):
            upper, lower = lanes[int(lane_idx)]
            target_lane_e_y = 0.5 * (float(upper) + float(lower))
        candidate_heading = self._candidate_lane_heading(lane_idx, closest_wp)
        heading_error = absolute_heading_difference(
            pose.theta, candidate_heading)
        signed_lateral_speed = float(self._odom.twist.twist.linear.y)
        yaw_rate = float(self._odom.twist.twist.angular.z)
        current_steering = float(getattr(
            self._mpc, "previous_steering", self._last_u[1]))
        prediction_age = cached.get("prediction_age")
        prediction_age_text = (
            "not_tracked" if prediction_age is None
            else f"{float(prediction_age):.3f}s"
        )
        self._outer_commit_prediction_diagnostic = {
            "source": str(source),
            "vehicle_id": vehicle_id,
            "lane_idx": int(lane_idx),
            "commit_loop": int(self._loop),
            "commit_time": float(now_sec),
            "candidate_gate_loop": int(cached["control_loop"]),
            "candidate_samples": candidate_samples,
            "candidate_moving_vehicles": list(
                cached.get("moving_vehicles", [])),
            "actual_samples": None,
        }
        self.get_logger().info(
            "[OuterCommitPredictionCandidate] "
            f"source={source}, vehicle_id={vehicle_id}, lane=L{lane_idx}, "
            f"commit_loop={self._loop}, gate_loop={cached['control_loop']}, "
            f"prediction_source={cached.get('prediction_source')}, "
            f"prediction_age={prediction_age_text}, "
            "candidate_state_time="
            f"{cached.get('candidate_state_time_sec')}, "
            f"current_time={float(now_sec):.9f}, "
            "consumed_prediction_samples="
            f"{cached.get('consumed_prediction_samples')}, "
            "remaining_prediction_samples="
            f"{len(candidate_samples)}, "
            f"ego_e_y={None if ego_frenet is None else ego_frenet[1]}, "
            f"target_lane_center_e_y={target_lane_e_y}, "
            f"heading_error={math.degrees(heading_error):.3f}deg, "
            f"signed_lateral_speed={signed_lateral_speed:.3f}m/s, "
            f"yaw_rate={yaw_rate:.3f}rad/s, "
            f"steering={current_steering:.4f}rad, speed={speed:.3f}m/s, "
            "soft_transition_start_e_y="
            f"{None if ego_frenet is None else ego_frenet[1]}, "
            "candidate_samples="
            f"{self._format_prediction_diagnostic_samples(candidate_samples)}"
        )

    def _prospective_outer_corridor_feasibility(
        self, *, pose, lane_idx: int, vehicle_id=None,
    ):
        """Build a diagnostic-only outer corridor without solving the MPC.

        The copied ReferencePath executes the same physical/obstacle free-space,
        lane connection, retry relaxation, and outer-boundary guard code as an
        actual constrained solve. A validated candidate uses the same per-step
        V2X time grid as its commit solve; all other calls preserve the existing
        flattened map. Copied path state keeps this check observational.
        """
        if lane_idx not in (0, 2):
            return None
        reference_path = self._reference_pathN_center
        car = self._carN_center
        mpc = self._mpcN_center
        constraint_times = self._validated_candidate_used_time_grid(
            vehicle_id, lane_idx)
        dynamic_v2x_by_step = (
            None if constraint_times is None
            else self._time_aligned_v2x_by_step(constraint_times)
        )
        static_snapshot = getattr(self, "_static_obstacle_map_data", None)
        if static_snapshot is None:
            constraint_times = None
            dynamic_v2x_by_step = None
        static_occupancy_data = (
            None if dynamic_v2x_by_step is None
            else static_snapshot
        )
        center_wp = int(car.get_closest_waypoint(pose.x, pose.y))
        required_width = lane_minimum_free_segment_width(
            lane_idx,
            float(self._cfg.bicycle_model.width),
            reference_path.inner_lane_width,
        )

        current_lanes = reference_path.get_lane_bounds(center_wp)
        current_waypoint = reference_path.get_waypoint(center_wp)
        current_e_y = (
            math.cos(float(current_waypoint.psi))
            * (float(pose.y) - float(current_waypoint.y))
            - math.sin(float(current_waypoint.psi))
            * (float(pose.x) - float(current_waypoint.x))
        )
        minimum_relaxation = 0.0
        if lane_idx < len(current_lanes):
            lane_ub, lane_lb = current_lanes[lane_idx]
            minimum_relaxation = max(
                float(lane_lb) - current_e_y,
                current_e_y - float(lane_ub),
                0.0,
            ) + 0.10
        configured_retries = tuple(
            max(float(value), 0.0)
            for value in mpc.lane_constraint_retry_relaxation_m
            if float(value) > 0.0
        )
        eligible_retries = tuple(
            value for value in configured_retries
            if value + 1e-9 >= minimum_relaxation
        )
        if not eligible_retries and configured_retries:
            eligible_retries = (max(configured_retries),)
        relaxation_profiles = (0.0,) + eligible_retries

        candidates = []

        corridor_inner_start = time.perf_counter()
        corridor_copy_sec = 0.0
        corridor_reference_path_copy_sec = 0.0
        corridor_waypoint_copy_sec = 0.0
        corridor_border_cells_copy_sec = 0.0
        corridor_unsafe_static_copy_sec = 0.0
        corridor_update_constraints_sec = 0.0
        corridor_profile_count = 0
        shared_transition_collision_cache = {}
        shared_transition_collision_cache_stats = {
            "hits": 0,
            "misses": 0,
        }
        shared_precomputed_segment_bound_cache = {}
        shared_precomputed_segment_bound_cache_stats = {
            "hits": 0,
            "misses": 0,
        }

        # Every retry profile sees identical physical/static/dynamic free space.
        # Compute that raw, pre-lane-filtering layer once; each diagnostic copy
        # still applies its own lane relaxation, filtering, combination search,
        # smoothing, and bound selection below.
        precomputed_free_segments_hor = []
        for n in range(mpc.N):
            raw_wp = reference_path.get_waypoint(center_wp + 1 + n)
            step_dynamic_obstacles = (
                None if dynamic_v2x_by_step is None
                else dynamic_v2x_by_step[n]
            )
            precomputed_free_segments_hor.append(
                reference_path._compute_free_segments(
                    raw_wp,
                    max(float(mpc.model.width), 0.0),
                    wp_idx=(center_wp + 1 + n),
                    occupancy_data=static_occupancy_data,
                    dynamic_obstacles=step_dynamic_obstacles,
                )
            )

        for lane_relaxation in relaxation_profiles:
            corridor_profile_count += 1
            corridor_copy_start = time.perf_counter()

            reference_path_copy_start = time.perf_counter()
            diagnostic_path = copy.copy(reference_path)
            corridor_reference_path_copy_sec += (
                time.perf_counter() - reference_path_copy_start
            )
            waypoint_copy_start = time.perf_counter()
            diagnostic_path.waypoints = list(reference_path.waypoints)
            copied_waypoint_indices = set()
            for horizon_index in range(mpc.N):
                waypoint_index = center_wp + 1 + horizon_index
                if waypoint_index >= reference_path.n_waypoints:
                    if reference_path.circular:
                        waypoint_index %= reference_path.n_waypoints
                    else:
                        waypoint_index = reference_path.n_waypoints - 1
                if waypoint_index in copied_waypoint_indices:
                    continue
                diagnostic_path.waypoints[waypoint_index] = copy.copy(
                    reference_path.waypoints[waypoint_index])
                copied_waypoint_indices.add(waypoint_index)
            corridor_waypoint_copy_sec += (
                time.perf_counter() - waypoint_copy_start
            )
            border_cells_copy_start = time.perf_counter()
            diagnostic_path.border_cells = copy.deepcopy(
                reference_path.border_cells)
            corridor_border_cells_copy_sec += (
                time.perf_counter() - border_cells_copy_start
            )
            unsafe_static_copy_start = time.perf_counter()
            diagnostic_path.unsafe_static_fallback_wp_ids = list(
                reference_path.unsafe_static_fallback_wp_ids)
            corridor_unsafe_static_copy_sec += (
                time.perf_counter() - unsafe_static_copy_start
            )

            corridor_copy_sec += (
                time.perf_counter() - corridor_copy_start
            )

            diagnostic_path.target_lane_idx = int(lane_idx)

            corridor_update_start = time.perf_counter()
            upper, lower, _ = diagnostic_path.update_path_constraints(
                center_wp + 1,
                [pose.x, pose.y, pose.theta],
                mpc.N,
                mpc.model.length,
                mpc.model.width,
                mpc.model.safety_margin,
                lane_relaxation=lane_relaxation,
                toward_center_only=(
                    mpc.lane_constraint_retry_relax_toward_center_only),
                taper_retry_over_horizon=(
                    mpc.lane_constraint_retry_taper_over_horizon),
                retry_terminal_ratio=(
                    mpc.lane_constraint_retry_terminal_ratio),
                connect_lane_from_current_pose=True,
                lane_connection_points=mpc.lane_constraint_connection_points,
                dynamic_v2x_by_step=dynamic_v2x_by_step,
                static_occupancy_data=static_occupancy_data,
                precomputed_free_segments_hor=precomputed_free_segments_hor,
                precomputed_combination_collision_cache=(
                    shared_transition_collision_cache),
            ...
            corridor_update_constraints_sec += (
                time.perf_counter() - corridor_update_start
            )
            ...
            lower, upper = apply_outer_boundary_guard(
                ...
                upper,
                lane_idx,
                mpc.prediction_outer_boundary_guard,
            )
            lower, upper = zero_inverted_bounds(lower, upper)
            widths = np.asarray(upper, dtype=float) - np.asarray(
                lower, dtype=float)
            wp_ids = np.asarray([
                (center_wp + 1 + index) % reference_path.n_waypoints
                for index in range(len(widths))
            ], dtype=int)
            min_index = int(np.nanargmin(widths))
            candidates.append({
                "lane_relaxation": float(lane_relaxation),
                "widths": widths,
                "wp_ids": wp_ids,
                "min_index": min_index,
                "min_width": float(widths[min_index]),
                "below_required_count": int(np.count_nonzero(
                    ~np.isfinite(widths) | (widths < required_width))),
            })

        # This is deliberately optimistic: report would_block only when the
        # best corridor available through the existing retry profiles remains
        # narrower than the existing lane-specific required width.
        result = max(
            candidates,
            key=lambda item: (
                item["min_width"], -item["below_required_count"]),
        )
        result["center_wp"] = center_wp
        result["required_width"] = float(required_width)

        base_widths = []
        horizon_centers = []
        for wp_id in result["wp_ids"]:
            lanes = reference_path.get_lane_bounds(int(wp_id))
            if lane_idx >= len(lanes):
                base_widths.append(math.nan)
                continue
            lane_ub, lane_lb = lanes[lane_idx]
            base_widths.append(float(lane_ub) - float(lane_lb))
            waypoint = reference_path.get_waypoint(int(wp_id))
            lane_center = 0.5 * (float(lane_ub) + float(lane_lb))
            normal_angle = float(waypoint.psi) + math.pi / 2.0
            horizon_centers.append((
                float(waypoint.x) + lane_center * math.cos(normal_angle),
                float(waypoint.y) + lane_center * math.sin(normal_angle),
            ))
        result["base_width_min"] = float(np.nanmin(base_widths))
        result["would_block"] = bool(
            not math.isfinite(result["min_width"])
            or result["min_width"] < required_width)

        nearest_clearance = math.inf
        prospective_obstacles = (
            None if dynamic_v2x_by_step is None
            else dynamic_v2x_by_step
        )
        for index, (center_x, center_y) in enumerate(horizon_centers):
            obstacles = (
                getattr(self._map, "obstacles", [])
                if prospective_obstacles is None
                else list(self._static_obstacles)
                    + list(prospective_obstacles[index])
            )
        ...
        result["nearest_obstacle_clearance"] = nearest_clearance

        limiting_vehicle_id = None
        limiting_vehicle_clearance = math.inf
        if dynamic_v2x_by_step is None:
            predictions = self._v2x_tracker.predict_all(self._v2x_t_samples)
            prediction_items = [
                (predicted_vehicle_id, vehicle_x, vehicle_y, center_x, center_y)
                for predicted_vehicle_id, positions in predictions.items()
                for vehicle_x, vehicle_y in positions
                for center_x, center_y in horizon_centers
            ]
        else:
            prediction_items = [
                (
                    ...
                    obstacle.cx,
                    obstacle.cy,
                    horizon_centers[index][0],
                    horizon_centers[index][1],
                )
                for index, obstacles in enumerate(dynamic_v2x_by_step)
                for obstacle in obstacles
            ]
        for (
            predicted_vehicle_id, vehicle_x, vehicle_y, center_x, center_y
            ...
            if clearance < limiting_vehicle_clearance:
                limiting_vehicle_clearance = clearance
                limiting_vehicle_id = predicted_vehicle_id
        result["limiting_vehicle_id"] = limiting_vehicle_id
        result["time_aligned"] = dynamic_v2x_by_step is not None
        result["constraint_times"] = constraint_times

        corridor_inner_total_sec = (
            time.perf_counter() - corridor_inner_start
        )
        corridor_other_sec = max(
            0.0,
            corridor_inner_total_sec
            - corridor_copy_sec
            - corridor_update_constraints_sec,
        ...
        if (
            ENABLE_RUNTIME_DIAGNOSTICS
            and corridor_inner_total_sec > 0.020
        ):
            self.get_logger().info(
                "[ProspectiveCorridorTiming] "
                f"loop={self._loop}, "
                f"lane=L{lane_idx}, "
                f"profiles={corridor_profile_count}, "
                f"relaxations={relaxation_profiles}, "
                f"copy={corridor_copy_sec * 1000.0:.1f}ms, "
                f"copy_ms={corridor_copy_sec * 1000.0:.3f}, "
                "reference_path_copy_ms="
                f"{corridor_reference_path_copy_sec * 1000.0:.3f}, "
                "waypoint_copy_ms="
                f"{corridor_waypoint_copy_sec * 1000.0:.3f}, "
                "border_cells_copy_ms="
                f"{corridor_border_cells_copy_sec * 1000.0:.3f}, "
                "unsafe_static_copy_ms="
                f"{corridor_unsafe_static_copy_sec * 1000.0:.3f}, "
                f"update_constraints="
                f"{corridor_update_constraints_sec * 1000.0:.1f}ms, "
                f"other={corridor_other_sec * 1000.0:.1f}ms, "
                f"total={corridor_inner_total_sec * 1000.0:.1f}ms, "
                "shared_transition_cache_hits="
                f"{shared_transition_collision_cache_stats['hits']}, "
                "shared_transition_cache_misses="
                f"{shared_transition_collision_cache_stats['misses']}, "
                "precomputed_bound_cache_hits="
                f"{shared_precomputed_segment_bound_cache_stats['hits']}, "
                "precomputed_bound_cache_misses="
                f"{shared_precomputed_segment_bound_cache_stats['misses']}"
            )

        return result

    def _log_outer_commit_corridor_prospective(
        self, *, source: str, vehicle_id, lane_idx: int, pose,
    ):
        """Emit one observational corridor record for an actual new commit."""
        try:
            result = self._prospective_outer_corridor_feasibility(
                pose=pose, lane_idx=lane_idx, vehicle_id=vehicle_id)
        except Exception as exc:  # Diagnostic failure must never change control.
            self.get_logger().warn(
                "[OuterCommitCorridorProspective] gate_active=True, "
                f"source={source}, vehicle_id={vehicle_id}, lane=L{lane_idx}, "
        ...
        if ENABLE_RUNTIME_DIAGNOSTICS:
            ...
            min_wp = int(result["wp_ids"][min_index])
            nearest = result["nearest_obstacle_clearance"]
            nearest_text = (
                f"{nearest:.3f}m" if math.isfinite(nearest) else "none")
            self.get_logger().info(
                "[OuterCommitCorridorProspective] gate_active=True, "
                f"source={source}, vehicle_id={vehicle_id}, lane=L{lane_idx}, "
                f"center_wp={result['center_wp']}, "
                f"base_width_min={result['base_width_min']:.3f}m, "
                "prospective_effective_width_min="
                f"{result['min_width']:.3f}m, "
                f"min_index={min_index}, min_wp={min_wp}, "
                f"required_width={result['required_width']:.3f}m, "
                f"below_required_width={result['below_required_count']}/"
                f"{len(result['widths'])}, "
                f"nearest_obstacle_clearance={nearest_text}, "
                f"limiting_vehicle={result['limiting_vehicle_id']}, "
                f"lane_relaxation={result['lane_relaxation']:.3f}m, "
                f"would_block={result['would_block']}"
            )
        return result

    def _outer_commit_corridor_would_block(
        self, *, source: str, vehicle_id, lane_idx: int, pose,
    ) -> bool:
        """Apply the existing prospective diagnostic as a new-commit gate."""
        result = self._log_outer_commit_corridor_prospective(
            source=source,
            vehicle_id=vehicle_id,
            lane_idx=lane_idx,
            pose=pose,
        )
        if result is None or not result["would_block"]:
            return False
        min_index = result["min_index"]
        self.get_logger().warn(
            "[OuterCommitCorridorBlocked] prospective outer hard corridor "
            "does not fit the existing required vehicle width: "
            f"source={source}, vehicle_id={vehicle_id}, lane=L{lane_idx}, "
            f"effective_width_min={result['min_width']:.3f}m, "
            f"required_width={result['required_width']:.3f}m, "
            f"below_required_width={result['below_required_count']}/"
            f"{len(result['widths'])}, "
            f"limiting_index={min_index}, "
            f"limiting_wp={int(result['wp_ids'][min_index])}, "
            f"limiting_vehicle={result['limiting_vehicle_id']}, "
            "action=exclude_new_outer_commit"
        )
        return True

    def _capture_outer_commit_actual_prediction_diagnostic(
        self, *, applied_lane_idx, transition_duration_sec: float,
        now_sec: float,
    ) -> None:
        """Record the first fresh accurate outer-constrained MPC solution."""
        if not ENABLE_RUNTIME_DIAGNOSTICS:
            return
        record = self._outer_commit_prediction_diagnostic
        if record is None:
            return
        age = float(now_sec) - record["commit_time"]
        if age > 0.6:
            self._outer_commit_prediction_diagnostic = None
            return
        if record["actual_samples"] is not None:
            return
        commit_owned_solve = bool(
            self._target_lane_idx == record["lane_idx"]
            and (
                applied_lane_idx == record["lane_idx"]
                or (
                    self._applied_corridor_mode == "lane_transition"
                    and self._overtake_soft_transition_lane_idx
                        == record["lane_idx"]
                )
            )
        )
        if (
            not commit_owned_solve
            or self._reference_path is not self._reference_pathN_center
            or self._mpc.current_prediction is None
            or self._mpc.infeasibility_counter != 0
            or getattr(self._mpc, "used_prediction_fallback", False)
        ...
        actual_samples = []
        for step, (prediction_time, sample_x, sample_y) in enumerate(zip(
            prediction_times, pred_x, pred_y
        )):
            frenet = self._center_frenet(sample_x, sample_y)
            actual_e_y = None if frenet is None else float(frenet[1])
            candidate_e_y = self._prediction_diagnostic_interpolate(
                record["candidate_samples"], prediction_time, "e_y")
            actual_samples.append({
                ...
                "e_y": actual_e_y,
                "delta_e_y": (
                    None if actual_e_y is None or candidate_e_y is None
                    else actual_e_y - candidate_e_y),
            })
        record["actual_samples"] = actual_samples
        self.get_logger().info(
            "[OuterCommitPredictionActual] "
            f"source={record['source']}, vehicle_id={record['vehicle_id']}, "
            f"lane=L{record['lane_idx']}, commit_loop={record['commit_loop']}, "
            f"actual_loop={self._loop}, age={age:.3f}s, "
            "soft_transition_start_e_y="
            f"{self._overtake_soft_transition_start_e_y}, "
            f"soft_transition_duration={transition_duration_sec:.3f}s, "
            "actual_samples="
            f"{self._format_prediction_diagnostic_samples(actual_samples, True)}"
        )

    def _log_outer_commit_prediction_conflict_diagnostic(
        self, conflict, *, now_sec: float,
    ) -> None:
        """Log blocker envelopes for an early post-commit conflict."""
        if not ENABLE_RUNTIME_DIAGNOSTICS:
            return
        record = self._outer_commit_prediction_diagnostic
        if record is None or conflict is None:
            return
        age = float(now_sec) - record["commit_time"]
        if age < 0.0 or age > 0.6 or record["actual_samples"] is None:
            return
        vehicle_id = conflict["vehicle_id"]
        candidate_vehicle_state = next((
            vehicle for vehicle in record["candidate_moving_vehicles"]
            if vehicle[0] == vehicle_id
        ), None)
        candidate_vehicle_samples = []
        if candidate_vehicle_state is not None:
            _, base_x, base_y, base_vx, base_vy = candidate_vehicle_state
            for ego_sample in record["candidate_samples"]:
                prediction_time = ego_sample["time"]
                predicted_x = base_x + base_vx * prediction_time
                predicted_y = base_y + base_vy * prediction_time
                vehicle_frenet = self._center_frenet(predicted_x, predicted_y)
                longitudinal = None
                lateral_clearance = None
                longitudinal_clearance = None
                if (
                    vehicle_frenet is not None
                    and ego_sample["s"] is not None
                    and ego_sample["e_y"] is not None
                ):
                    longitudinal = signed_closed_path_arc_distance(
                        ego_sample["s"], vehicle_frenet[0],
                        self._center_arc_total_length)
                    lateral_clearance = lateral_vehicle_clearance(
                        vehicle_frenet[1] - ego_sample["e_y"],
                        float(self._cfg.bicycle_model.width),
                        self._v2x_parallel_vehicle_half_width)
                    longitudinal_clearance = longitudinal_vehicle_clearance(
                        longitudinal, self._parallel_ego_half_length,
                        self._parallel_vehicle_half_length)
                candidate_vehicle_samples.append(
                    f"{ego_sample['step']}@{prediction_time:.3f}:"
                    f"x={predicted_x:.3f},y={predicted_y:.3f},"
                    f"e_y={None if vehicle_frenet is None else round(vehicle_frenet[1], 3)},"
                    f"s={None if vehicle_frenet is None else round(vehicle_frenet[0], 3)},"
                    f"longitudinal={None if longitudinal is None else round(longitudinal, 3)},"
                    f"lat_clearance={None if lateral_clearance is None else round(lateral_clearance, 3)},"
                    f"lon_clearance={None if longitudinal_clearance is None else round(longitudinal_clearance, 3)}"
                )
        tracked = self._v2x_tracker._samples.get(vehicle_id)
        if not tracked:
            return
        _, vehicle_x, vehicle_y = tracked[-1]
        vehicle_vx, vehicle_vy = self._v2x_tracker.velocity(vehicle_id)
        vehicle_samples = []
        for ego_sample in record["actual_samples"]:
            prediction_time = ego_sample["time"]
            predicted_x = float(vehicle_x) + float(vehicle_vx) * prediction_time
            predicted_y = float(vehicle_y) + float(vehicle_vy) * prediction_time
            vehicle_frenet = self._center_frenet(predicted_x, predicted_y)
            longitudinal = None
            lateral_clearance = None
            longitudinal_clearance = None
            ...
            vehicle_samples.append(
                f"{ego_sample['step']}(spatial="
                f"{ego_sample.get('spatial_step', ego_sample['step'])})"
        ...
        self.get_logger().info(
            "[OuterCommitPredictionConflict] "
            f"source={record['source']}, vehicle_id={record['vehicle_id']}, "
            f"lane=L{record['lane_idx']}, blocking_vehicle={vehicle_id}, "
            f"age={age:.3f}s, first_conflict_step="
            f"{conflict.get('prediction_step')}, first_conflict_time="
            f"{conflict.get('prediction_time')}, "
            "candidate_vehicle_samples="
            f"[{'; '.join(candidate_vehicle_samples)}], "
            f"actual_vehicle_samples=[{'; '.join(vehicle_samples)}]"
        )
        self._outer_commit_prediction_diagnostic = None
    ...
    def _clear_prepass_soft_objective(self) -> None:
        """Remove only the applied outer objective, preserving its episode."""
    ...
    def _clear_prepass_soft_guidance(self) -> None:
        """Reset all Prepass soft state at the end of an episode."""
        self._clear_prepass_soft_objective()
        ...
        self._prepass_soft_neutral_hold_active = False

    @staticmethod
    def _prepass_soft_neutral_stability_ready(
        *, neutral_active, candidate_lane, latched_lane,
        candidate_binary_safe, mpc_stable, heading_stable, dynamics_stable,
    ) -> bool:
        """Require every existing stability gate before leaving neutral."""
        return bool(
            neutral_active
            and candidate_lane in (0, 2)
            and latched_lane == candidate_lane
            and candidate_binary_safe
            and mpc_stable
            and heading_stable
            and dynamics_stable
        )
    ...
    @staticmethod
    def _prepass_soft_neutral_should_exit(
        *, neutral_active, stability_ready, stable_elapsed, required_sec,
    ) -> bool:
        """Release neutral only after the existing stable duration."""
        return bool(
            neutral_active
            and stability_ready
            and float(stable_elapsed) >= float(required_sec)
        )

    def _enter_prepass_soft_neutral_hold(
        self, *, current_candidate, next_candidate, heading_error,
        lateral_speed, yaw_rate, mpc_stable,
    ) -> None:
        """Drop an unsafe outer objective without resetting its episode."""
        if self._prepass_soft_neutral_hold_active:
            return
        self._prepass_soft_neutral_hold_active = True
        self._prepass_fallback_recovery_stable_since = None
        self._clear_prepass_soft_objective()
        self.get_logger().warn(
            "[PrepassSoftNeutralHoldEnter] current outer ownership became "
            "unsafe; clearing its objective and retaining "
            "full-width recovery: "
            f"vehicle_id={self._overtake_target_vehicle_id}, "
            f"unsafe_candidate=L{current_candidate}, "
            "next_candidate="
            f"{'L' + str(next_candidate) if next_candidate in (0, 2) else 'none'}, "
            f"heading_error={math.degrees(heading_error):.1f}deg, "
            f"lateral_speed={lateral_speed:.2f}m/s, "
            f"yaw_rate={yaw_rate:.2f}rad/s, mpc_stable={mpc_stable}"
        )

    def _clear_prohibited_l0_soft_state(self) -> None:
        """Remove only Prepass soft state that still points toward L0."""
        if self._prepass_soft_candidate_lane_idx == 0:
            self._prepass_soft_candidate_lane_idx = None
            self._prepass_soft_candidate_last_seen_at = None
            self._prepass_soft_guidance_started_at = None
            self._prepass_soft_guidance_start_e_y = None
            self._prepass_soft_guidance_ramp_sec = None
        if self._prepass_soft_pending_lane_idx == 0:
            self._prepass_soft_pending_lane_idx = None
            self._prepass_soft_pending_since = None
        if self._prepass_timeout_candidate_lane_idx == 0:
            self._clear_prepass_timeout_progress()

    def _l0_entry_prohibited_at_center_wp(self, center_wp: int) -> bool:
        """Return whether one Center waypoint belongs to an L0 ban zone."""
        n_waypoints = self._reference_pathN_center.n_waypoints
        if n_waypoints <= 0:
            return False
        normalized_wp = int(center_wp) % n_waypoints
        return any(
            (
                start_wp <= normalized_wp <= end_wp
                if start_wp <= end_wp
                else normalized_wp >= start_wp or normalized_wp <= end_wp
            )
            for start_wp, end_wp in self._l0_entry_prohibited_zones
        )

    def _future_l0_entry_prohibition(
        self, center_wp: int,
    ) -> Tuple[bool, int, Optional[Tuple[int, int]]]:
        """Find an L0 ban in the existing MPC plus heading-lookahead range."""
        n_waypoints = self._reference_pathN_center.n_waypoints
        if n_waypoints <= 0:
            return False, int(center_wp), None
        lookahead_wps = (
            max(int(self._mpcN_center.N), 0)
            + max(int(self._prepass_heading_lookahead_wps), 0)
        )
        normalized_wp = int(center_wp) % n_waypoints
        lookahead_end_wp = (normalized_wp + lookahead_wps) % n_waypoints
        for offset in range(lookahead_wps + 1):
            future_wp = (normalized_wp + offset) % n_waypoints
            for start_wp, end_wp in self._l0_entry_prohibited_zones:
                in_zone = (
                    start_wp <= future_wp <= end_wp
                    if start_wp <= end_wp
                    else future_wp >= start_wp or future_wp <= end_wp
                )
                if in_zone:
                    return True, lookahead_end_wp, (start_wp, end_wp)
        return False, lookahead_end_wp, None

    def _clear_prepass_timeout_progress(self) -> None:
        """Drop candidate progress owned by the current Prepass episode."""
        self._prepass_timeout_candidate_lane_idx = None
        self._prepass_timeout_heading_error = None

    def _discard_motionhold_validated_full_width(self, reason: str) -> None:
        """Discard the one-cycle full-width proof without affecting control."""
        record = self._motionhold_validated_full_width
        if record is None:
            return
        self._motionhold_validated_full_width = None
        self.get_logger().info(
            "[MotionHoldFullWidthDiscarded] "
            f"vehicle_id={record['vehicle_id']}, "
            f"lane=L{record['lane_idx']}, "
            f"source_loop={record['control_loop']}, reason={reason}",
            throttle_duration_sec=1.0,
        )

    def _clear_prepass_short_commit_history(self) -> None:
        """Drop short commit failures owned by the complete overtake attempt."""
        self._prepass_last_confirmed_commit = None
        self._prepass_short_failed_outer_lanes.clear()

    def _record_prepass_short_commit_failure(
        self, lane_idx, now_sec: float, failure_reason: str
    ) -> bool:
        """Record a provisional commit that failed within the existing timeout."""
        confirmed = self._prepass_last_confirmed_commit
        self._prepass_last_confirmed_commit = None
        if (
            confirmed is None
            or lane_idx not in (0, 2)
            or int(confirmed[0]) != int(lane_idx)
        ):
            return False
        confirmed_at = float(confirmed[1])
        elapsed = max(float(now_sec) - confirmed_at, 0.0)
        if elapsed >= self._prepass_lane_fallback_recovery_timeout_sec:
            return False
        self._prepass_short_failed_outer_lanes.add(int(lane_idx))
        failed_lanes = sorted(self._prepass_short_failed_outer_lanes)
        self.get_logger().warn(
            "[PrepassShortCommitFailure] provisional fallback commit failed "
            "before the existing recovery timeout: "
            f"vehicle_id={self._overtake_target_vehicle_id}, "
            f"lane=L{lane_idx}, confirmed_at={confirmed_at:.3f}, "
            f"elapsed_since_commit={elapsed:.3f}s/"
            f"{self._prepass_lane_fallback_recovery_timeout_sec:.3f}s, "
            f"failure_reason={failure_reason}, "
            f"short_failed_outer_lanes={failed_lanes}"
        )
        return all(
            outer_lane in self._prepass_short_failed_outer_lanes
            for outer_lane in (0, 2)
        )
    ...
    def _reset_overtake_state_for_target_change(
        ...
        self._mpc.current_prediction_times = None
        self._mpc.current_constraint_prediction_times = None
        self.get_logger().warn(
    ...
    def _committed_lane_predicted_envelope_conflict(self, pose):
        """Return the first all-V2X envelope conflict on a fresh MPC path."""
        if (
            not hasattr(self, "_v2x_tracker")
            or self._mpc.current_prediction is None
            or self._mpc.infeasibility_counter != 0
        ...
        for vehicle_id in self._v2x_tracker.active_vehicle_ids():
            ...
            moving_vehicles.append((
                ...
                float(vehicle_vx), float(vehicle_vy)))
        conflict = minimum_predicted_envelope_conflict(
            pred_x, pred_y, prediction_times, moving_vehicles,
            project_frenet=self._center_frenet,
            arc_total_length=self._center_arc_total_length,
            ego_width=float(self._cfg.bicycle_model.width),
            other_half_width=self._v2x_parallel_vehicle_half_width,
        ...
        if conflict is None:
            return None
        ego_current_frenet = self._center_frenet(pose.x, pose.y)
        samples = self._v2x_tracker._samples.get(conflict["vehicle_id"])
        vehicle_current_frenet = None
        if samples:
            _, vehicle_x, vehicle_y = samples[-1]
            vehicle_current_frenet = self._center_frenet(vehicle_x, vehicle_y)
    ...
    def _select_prepass_retry_lane(
        ...
        l0_prohibited = any(
            (start_wp <= center_wp <= end_wp if start_wp <= end_wp
             else center_wp >= start_wp or center_wp <= end_wp)
            for start_wp, end_wp in self._l0_entry_prohibited_zones
        )
        lane_idx = next((
            candidate for candidate in order
            if candidate in (0, 2)
            and not (candidate == 0 and l0_prohibited)
    ...
    def _select_prepass_fallback_lane(
        ...
        samples = self._relative_lane_vehicle_samples(pose, ego_speed)
        excluded_outer_lanes = (
            self._prepass_attempted_outer_lanes
            | self._prepass_short_failed_outer_lanes
        )
        ...
        selected_lane_idx = next((
            ...
            if not (lane_idx == 0 and l0_prohibited)
        ...
        if self._prepass_short_failed_outer_lanes:
            self.get_logger().info(
                "[PrepassShortFailedLaneExcluded] short-failed outer lanes "
                "are excluded from this attempt's fallback selection: "
                f"vehicle_id={self._overtake_target_vehicle_id}, "
                f"failed_lane="
                f"{'L' + str(failed_lane_idx) if failed_lane_idx in (0, 2) else 'none'}, "
                "short_failed_outer_lanes="
                f"{sorted(self._prepass_short_failed_outer_lanes)}, "
                "attempted_outer_lanes="
                f"{sorted(self._prepass_attempted_outer_lanes)}, "
                f"remaining_candidates={list(candidate_order)}, "
                "action=exclude_short_failed_from_fallback_candidate",
                throttle_duration_sec=1.0,
            )
    ...
    def _start_race_rejoin_handoff(self, position_gap, now_sec, center_wp):
        ...
        self._mpcN_race.current_prediction_times = None
        self._mpcN_race.current_constraint_prediction_times = None
    ...
    def _record_mpc_solve_timing(
        self, mpc, source: str, get_control_wall_ms: float,
    ) -> None:
        """Record diagnostic-only OSQP and enclosing MPC wall time."""
        if not ENABLE_RUNTIME_DIAGNOSTICS:
            return
        solve_times_ms = list(getattr(
            mpc, "last_optimizer_solve_wall_ms", ()))
        solve_wall_total_ms = float(sum(solve_times_ms))
        solve_wall_max_ms = float(max(solve_times_ms, default=0.0))
        solve_call_count = len(solve_times_ms)
        valid = bool(
            mpc.infeasibility_counter == 0
            and mpc.current_prediction is not None
            and not getattr(mpc, "used_prediction_fallback", False)
            and not mpc.recovery_requested
            and not mpc.time_budget_exceeded
            and bool(getattr(mpc, "last_solution_accurate", False))
        )
        record = {
            "source": source,
            "solve_call_count": solve_call_count,
            "solve_wall_total_ms": solve_wall_total_ms,
            "solve_wall_max_ms": solve_wall_max_ms,
            "get_control_wall_ms": float(get_control_wall_ms),
            "valid": valid,
        }
        self._control_mpc_timing_records.append(record)
        self.get_logger().info(
            "[MPCSolveTiming] "
            f"loop={self._loop}, "
            f"source={source}, "
            f"solve_wall_ms={solve_wall_total_ms:.3f}, "
            f"solve_call_count={solve_call_count}, "
            f"solve_wall_max_ms={solve_wall_max_ms:.3f}, "
            f"get_control_wall_ms={get_control_wall_ms:.3f}, "
            f"valid={valid}"
        )
        get_control_timing = getattr(
            mpc, "last_get_control_timing_ms", None)
        if (
            isinstance(get_control_timing, dict)
            and (
                float(get_control_wall_ms) > 20.0
                or self._loop % 40 == 0
            )
        ):
            self.get_logger().info(
                ...
                f"loop={self._loop}, "
                f"source={source}, "
                f"total_ms={get_control_timing['total_ms']:.3f}, "
                f"solve_ms={get_control_timing['solve_ms']:.3f}, "
                f"non_solve_ms={get_control_timing['non_solve_ms']:.3f}, "
                f"state_prepare_ms={get_control_timing['state_prepare_ms']:.3f}, "
                f"problem_setup_ms={get_control_timing['problem_setup_ms']:.3f}, "
                "linearization_reference_ms="
                f"{get_control_timing['linearization_reference_ms']:.3f}, "
                f"path_constraints_ms={get_control_timing['path_constraints_ms']:.3f}, "
                f"sparse_matrix_ms={get_control_timing['sparse_matrix_ms']:.3f}, "
                f"constraint_bounds_ms={get_control_timing['constraint_bounds_ms']:.3f}, "
                f"cost_vector_ms={get_control_timing['cost_vector_ms']:.3f}, "
                f"optimizer_update_ms={get_control_timing['optimizer_update_ms']:.3f}, "
                f"result_postprocess_ms={get_control_timing['result_postprocess_ms']:.3f}, "
                f"fallback_ms={get_control_timing['fallback_ms']:.3f}, "
                f"tail_ms={get_control_timing['tail_ms']:.3f}, "
                f"other_ms={get_control_timing['other_ms']:.3f}, "
                "problem_build_call_count="
                f"{get_control_timing['problem_build_call_count']}, "
                f"valid={valid}"
            )
            self.get_logger().info(
                "[MPCPathConstraintsTiming] "
                f"loop={self._loop}, "
                f"source={source}, "
                "path_constraints_total_ms="
                f"{get_control_timing['path_constraints_ms']:.3f}, "
                f"metadata_ms={get_control_timing['path_constraint_metadata_ms']:.3f}, "
                "reference_path_bounds_ms="
                f"{get_control_timing['path_constraint_reference_bounds_ms']:.3f}, "
                "boundary_guard_ms="
                f"{get_control_timing['path_constraint_boundary_guard_ms']:.3f}, "
                "state_bounds_ms="
                f"{get_control_timing['path_constraint_state_bounds_ms']:.3f}, "
                "lateral_reference_ms="
                f"{get_control_timing['path_constraint_lateral_reference_ms']:.3f}, "
                "reference_free_segments_ms="
                f"{get_control_timing['reference_free_segments_ms']:.3f}, "
                "reference_combination_ms="
                f"{get_control_timing['reference_combination_ms']:.3f}, "
                "reference_smoothing_ms="
                f"{get_control_timing['reference_smoothing_ms']:.3f}, "
                "reference_other_ms="
                f"{get_control_timing['reference_other_ms']:.3f}, "
                "reference_internal_total_ms="
                f"{get_control_timing['reference_internal_total_ms']:.3f}, "
                "reference_wrapper_overhead_ms="
                f"{get_control_timing['reference_wrapper_overhead_ms']:.3f}, "
                "reference_timing_call_count="
                f"{get_control_timing['reference_timing_call_count']}, "
                "reference_free_line_aa_ms="
                f"{get_control_timing['reference_free_line_aa_ms']:.3f}, "
                "reference_free_static_map_check_ms="
                f"{get_control_timing['reference_free_static_map_check_ms']:.3f}, "
                "reference_free_dynamic_obstacle_check_ms="
                f"{get_control_timing['reference_free_dynamic_obstacle_check_ms']:.3f}, "
                "reference_free_dynamic_geometry_prepare_ms="
                f"{get_control_timing['reference_free_dynamic_geometry_prepare_ms']:.3f}, "
                "reference_free_conversion_ms="
                f"{get_control_timing['reference_free_conversion_ms']:.3f}, "
                "reference_free_other_ms="
                f"{get_control_timing['reference_free_other_ms']:.3f}, "
                "reference_free_other_setup_ms="
                f"{get_control_timing['reference_free_other_setup_ms']:.3f}, "
                "reference_free_other_endpoint_map_prepare_ms="
                f"{get_control_timing['reference_free_other_endpoint_map_prepare_ms']:.3f}, "
                "reference_free_other_cell_loop_bookkeeping_ms="
                f"{get_control_timing['reference_free_other_cell_loop_bookkeeping_ms']:.3f}, "
                "reference_free_other_post_loop_finalization_ms="
                f"{get_control_timing['reference_free_other_post_loop_finalization_ms']:.3f}, "
                "reference_free_other_detail_total_ms="
                f"{get_control_timing['reference_free_other_detail_total_ms']:.3f}, "
                "reference_free_other_remaining_ms="
                f"{get_control_timing['reference_free_other_remaining_ms']:.3f}, "
                "reference_free_internal_total_ms="
                f"{get_control_timing['reference_free_internal_total_ms']:.3f}, "
                "reference_free_section_overhead_ms="
                f"{get_control_timing['reference_free_section_overhead_ms']:.3f}, "
                "reference_free_timing_call_count="
                f"{get_control_timing['reference_free_timing_call_count']}, "
                "detail_total_ms="
                f"{get_control_timing['path_constraint_detail_total_ms']:.3f}, "
                f"other_ms={get_control_timing['path_constraint_other_ms']:.3f}, "
                "problem_build_call_count="
                f"{get_control_timing['problem_build_call_count']}, "
                f"valid={valid}"
            )
    ...
    def _run_race_rejoin_probe(self, predicted_pose, recovery_active, now_sec,
        ...
        self._mpcN_race.previous_steering = self._mpcN_center.previous_steering
        probe_control_start = time.perf_counter()
        self._mpcN_race.get_control()
        self._record_mpc_solve_timing(
            self._mpcN_race,
            "probe",
            (time.perf_counter() - probe_control_start) * 1000.0,
        )
        success = (
    ...
    def _update_overtake_transition_soft_reference(
        ...
        if self._overtake_soft_transition_lane_idx != lane_idx:
            ...
            handoff = self._outer_candidate_reference_handoff
            handoff_used = bool(
                handoff is not None and handoff["lane_idx"] == int(lane_idx))
            if handoff_used:
                self._overtake_soft_transition_start_e_y = handoff["start_e_y"]
                self._overtake_soft_transition_start_alpha = handoff["alpha"]
                self._overtake_soft_transition_lateral_offset = handoff[
                    "lateral_offset"]
                self._overtake_soft_transition_lateral_targets = handoff[
                    "lateral_targets"]
            else:
            ...
            if handoff_used:
                target_e_y = self._mpcN_center._compute_lane_center(
                    self._carN_center.wp_id, lane_idx)
                if self._overtake_soft_transition_lateral_targets is not None:
                    target_e_y = float(
                        self._overtake_soft_transition_lateral_targets[0])
                target_e_y += self._overtake_soft_transition_lateral_offset
                effective_e_y = (
                    self._overtake_soft_transition_start_e_y
                    + (target_e_y - self._overtake_soft_transition_start_e_y)
                    * self._overtake_soft_transition_start_alpha
                )
                self.get_logger().info(
                    "[OuterCandidateReferenceHandoff] "
                    f"source={handoff['source']}, "
                    f"vehicle_id={handoff['vehicle_id']}, lane=L{lane_idx}, "
                    "precommit_guidance_alpha="
                    f"{self._overtake_soft_transition_start_alpha:.3f}, "
                    f"precommit_effective_reference={effective_e_y:.3f}m, "
                    f"postcommit_transition_reference={effective_e_y:.3f}m, "
                    "handoff_used=True"
                )
            self._outer_candidate_reference_handoff = None
        ...
        alpha = (
            self._overtake_soft_transition_start_alpha
            + (1.0 - self._overtake_soft_transition_start_alpha)
            * transition_progress
        )
        ...
        self._mpcN_center.set_soft_lateral_reference(
            ...
            alpha=alpha,
            lateral_offset=self._overtake_soft_transition_lateral_offset,
            lateral_targets=self._overtake_soft_transition_lateral_targets,
        )

    def _log_prepass_soft_candidate_decision(
        self, diagnostics, *, action: str, pending_elapsed_sec: float,
    ) -> None:
        """Log Prepass switch evidence without affecting lane selection."""
        if not diagnostics:
            return

        lane_parts = []
        for lane_idx in (0, 2):
            details = diagnostics["lanes"][lane_idx]
            conflicts = details["conflicts"]
            lane_parts.append(
                f"L{lane_idx}[physical_passage="
                f"{details['physical_passage']}, "
                f"front={conflicts.get('front', [])}, "
                f"side={conflicts.get('side', [])}, "
                f"rear={conflicts.get('rear', [])}, "
                f"binary_safe={details['binary_safe']}, "
                f"v2x_margin={details['v2x_margin']:.2f}m, "
                f"limiting_vehicle={details['limiting_vehicle_id']}, "
                f"wall_margin={details['wall_margin']:.2f}m]"
            )
        self.get_logger().info(
            "[PrepassSoftCandidateDecision] "
            f"vehicle_id={diagnostics['vehicle_id']}, "
            f"current_candidate={diagnostics['current_candidate']}, "
            f"raw_candidate={diagnostics['raw_candidate']}, "
            f"pending_candidate={diagnostics['pending_candidate']}, "
            f"pending_elapsed_sec={pending_elapsed_sec:.3f}, "
            + ", ".join(lane_parts)
            + f", center_wp={diagnostics['center_wp']}, "
            f"current_e_y={diagnostics['current_e_y']:.3f}m, "
            f"signed_lateral_speed="
            f"{diagnostics['signed_lateral_speed']:+.3f}m/s "
            "(positive=L2 direction, negative=L0 direction), "
            f"heading_error_l0={diagnostics['heading_error_l0_deg']:.2f}deg, "
            f"heading_error_l2={diagnostics['heading_error_l2_deg']:.2f}deg, "
            f"action={action}, reason={diagnostics['reason']}"
        )

    def _latch_prepass_soft_candidate(
        self, lane_idx, now_sec: float, diagnostics=None,
    ):
        ...
        if self._prepass_soft_pending_lane_idx != raw_lane:
            ...
            if diagnostics is not None:
                diagnostics["pending_candidate"] = f"L{raw_lane}"
                self._log_prepass_soft_candidate_decision(
                    diagnostics,
                    action="start_pending_switch",
        ...
        if pending_for < self._prepass_soft_switch_confirm_sec:
            if diagnostics is not None:
                ...
                self._log_prepass_soft_candidate_decision(
                    diagnostics,
                    action="continue_pending_switch",
                    pending_elapsed_sec=pending_for,
                )
            return latched_lane
    ...
    def _update_prepass_soft_reference(
        ...
        # Neutral recovery deliberately keeps the candidate/pending decision
        # state alive, but it must never apply either outer-lane objective.
        # Keep this guard at the writer as well as at its caller so entering
        # neutral earlier in the same _control() cycle takes effect immediately.
        if self._prepass_soft_neutral_hold_active:
            self._clear_prepass_soft_objective()
            return
    ...
    def _update_outer_candidate_guidance_reference(
        self, *, enabled: bool, now_sec: float
    ) -> bool:
        """Apply candidate guidance without creating hard lane ownership."""
        record = self._outer_candidate_guidance
        if not enabled or record is None or record["lane_idx"] not in (0, 2):
            return False
        elapsed = max(float(now_sec) - record["guidance_started_at"], 0.0)
        alpha = (
            1.0 if record["ramp_sec"] <= 0.0
            else min(elapsed / record["ramp_sec"], 1.0)
        )
        self._mpcN_center.set_soft_lateral_reference(
            lane_idx=record["lane_idx"],
            start_e_y=record["start_e_y"],
            alpha=alpha,
        )
        record["last_guidance_alpha"] = float(alpha)
        return True

    def _capture_outer_candidate_guided_prediction(
        self, *, guidance_applied: bool, applied_lane_idx,
        time_aligned_transaction=None, candidate_state_time_sec=None,
    ) -> None:
        record = self._outer_candidate_guidance
        if not guidance_applied or record is None or applied_lane_idx is not None:
            return
        guidance_has_effect = bool(
            (
                record["source"] in ("normal", "preempt")
                and record["last_guidance_alpha"] is not None
                and record["last_guidance_alpha"] > 0.0
            )
            or (
                record["source"] == "fallback"
                and self._loop > record["guidance_loop"]
            )
        )
        if not guidance_has_effect:
            return
        fresh = self._fresh_accurate_current_mpc_prediction()
        if fresh is None:
            record["prediction_loop"] = None
            record["prediction_id"] = None
            record["constraint_time_grid_used"] = None
            record["constraint_time_grid_result"] = None
            record["candidate_solve_time_aligned"] = False
            return
        constraint_times = getattr(
            self._mpc, "current_constraint_prediction_times", None)
        if constraint_times is None or len(constraint_times) != self._mpc.N:
            record["prediction_loop"] = None
            record["prediction_id"] = None
            record["constraint_time_grid_used"] = None
            record["constraint_time_grid_result"] = None
            record["candidate_solve_time_aligned"] = False
            return
        record["prediction_loop"] = int(self._loop)
        record["candidate_state_time_sec"] = (
            None if candidate_state_time_sec is None
            else float(candidate_state_time_sec)
        )
        record["mpc_id"] = id(self._mpc)
        record["reference_path_id"] = id(self._mpc.model.reference_path)
        record["prediction_id"] = id(self._mpc.current_prediction)
        record["constraint_time_grid_result"] = tuple(
            float(value) for value in constraint_times)
        transaction_matches = bool(
            time_aligned_transaction is not None
            and time_aligned_transaction.get("vehicle_id")
                == record["vehicle_id"]
            and time_aligned_transaction.get("lane_idx")
                == int(record["lane_idx"])
            and time_aligned_transaction.get("source") == record["source"]
            and time_aligned_transaction.get("candidate_loop") == self._loop
            and time_aligned_transaction.get("grid_source_mpc_id")
                == id(self._mpc)
            and time_aligned_transaction.get("grid_source_reference_path_id")
                == id(self._mpc.model.reference_path)
        )
        record["candidate_solve_time_aligned"] = transaction_matches
        record["constraint_time_grid_used"] = (
            tuple(time_aligned_transaction["constraint_times"])
            if transaction_matches else None
        )
        record["grid_source_loop"] = (
            time_aligned_transaction["grid_source_loop"]
            if transaction_matches else None
        )
        record["grid_source_mpc_id"] = (
            time_aligned_transaction["grid_source_mpc_id"]
            if transaction_matches else None
        )
        record["grid_source_reference_path_id"] = (
            time_aligned_transaction["grid_source_reference_path_id"]
            if transaction_matches else None
        )
        record["grid_source_prediction_id"] = (
            time_aligned_transaction["grid_source_prediction_id"]
            if transaction_matches else None
        )
        pred_x, pred_y, prediction_times = fresh
        samples = []
        for step, (sample_x, sample_y, sample_time) in enumerate(zip(
        ...
        if transaction_matches and not record["ready_logged"]:
            record["ready_logged"] = True
            self.get_logger().info(
                "[OuterCandidateGuidedPredictionReady] "
                f"vehicle_id={record['vehicle_id']}, "
                f"candidate_lane=L{record['lane_idx']}, "
                f"guidance_loop={record['guidance_loop']}, "
                f"prediction_loop={self._loop}, fresh=True, "
                f"samples={self._format_prediction_diagnostic_samples(samples)}"
            )
        used = record.get("constraint_time_grid_used")
        result = record.get("constraint_time_grid_result")
        self.get_logger().info(
            "[OuterCandidateTimeAlignedV2X] "
            f"vehicle_id={record['vehicle_id']}, lane=L{record['lane_idx']}, "
            f"source={record['source']}, candidate_loop={self._loop}, "
            f"mode={'time_aligned' if transaction_matches else 'warmup_flatten'}, "
            f"grid_source_loop={record.get('grid_source_loop')}, "
            f"grid_used_first={None if not used else used[0]}, "
            f"grid_used_last={None if not used else used[-1]}, "
            f"grid_result_first={None if not result else result[0]}, "
            f"grid_result_last={None if not result else result[-1]}, "
            f"commit_eligible={transaction_matches}"
        )
    ...
    def _release_lost_follow_target(self, reason="hold expired") -> None:
        ...
        failure_terms = ("timeout", "infeasible", "mpc", "no passage")
        metric_result = (
            "failed"
            if any(term in reason.lower() for term in failure_terms)
            else "aborted"
        )
        self._metrics_end_overtake(
            metric_result, self._metrics_now_sec(), reason,
            target_id=lost_id)
        self._clear_prepass_short_commit_history()
        self._prepass_fallback_follow_active = False
        self._prepass_follow_last_retry_at = None
    ...
    def _complete_overtake_target_behind(
        ...
        self._metrics_end_overtake(
            "success", self._metrics_now_sec(),
            f"OvertakeTargetPhysicalComplete:{source}", target_id=target_id)
        self._clear_prepass_short_commit_history()
        self._overtake_target_committed = False
    ...
    def _apply_straight_reentry(self, now, pose, u) -> bool:
        ...
        localization_failed = not self._localization_consistent
    ...
    def _map_marker_callback(self, msg: MarkerArray) -> None:
        ...
        if len(left_pts) > 0 and len(right_pts) > 0:
            ...
            lane_marker_cache = getattr(self, "_lane_marker_cache", None)
            if lane_marker_cache is not None:
                lane_marker_cache.clear()
            self._lane_marker_last_published_cache_key = None
    ...
    def _awsim_status_callback(self, msg):
        ...
        if laps > self._current_laps:
            ...
            metric_now = self._metrics_now_sec()
            self._metrics_split_active_safety_recovery(metric_now)
            self._metrics_write_summary(
                "lap", self._current_laps, self._last_lap_time,
                self._metrics_lap)
            self._metrics_lap = self._new_metrics_bucket()
    ...
    def _publish_lane_markers(
        self, ref_path: ReferencePath, n_lanes: int = 3, *, use_cache=True,
    ) -> None:
        """
        ...
        import math
        target_lane_idx = getattr(ref_path, "target_lane_idx", None)
        cache_key = (
            id(ref_path), int(n_lanes), target_lane_idx, float(self._map_z))
        cache_hit = False
        cache_miss = bool(use_cache)
        if use_cache:
            lane_marker_cache = getattr(self, "_lane_marker_cache", None)
            if lane_marker_cache is None:
                lane_marker_cache = {}
                self._lane_marker_cache = lane_marker_cache
            cached_markers = lane_marker_cache.get(cache_key)
            if cached_markers is not None:
                cache_hit = True
                cache_miss = False
                cache_lookup_end = time.perf_counter()
                publish_start = time.perf_counter()
                # /mpc/lane_bounds is RELIABLE/TRANSIENT_LOCAL with depth 1,
                # and these markers keep the default zero (forever) lifetime.
                # Republish only when the latest retained MarkerArray differs.
                publish_skipped = (
                    getattr(
                        self,
                        "_lane_marker_last_published_cache_key",
                        None,
                    ) == cache_key
                )
                if not publish_skipped:
                    self._lane_marker_pub.publish(cached_markers)
                    self._lane_marker_last_published_cache_key = cache_key
                publish_end = time.perf_counter()
                timing_end = time.perf_counter()
                loop = getattr(self, "_loop", None)
                if loop is not None:
                    control_rate = int(self._mpc_cfg.control_rate)
                    if loop % control_rate == 0:
                        self.get_logger().info(
                            "[LaneMarkerTiming] "
                            f"loop={loop}, "
                            ...
                            "cache_lookup_ms="
                            f"{(cache_lookup_end - cache_lookup_start) * 1000.0:.3f}, "
                            "geometry_build_ms=0.000, "
                            f"publish_ms={(publish_end - publish_start) * 1000.0:.3f}, "
                            f"publish_skipped={publish_skipped}, "
                            f"total_ms={(timing_end - timing_start) * 1000.0:.3f}, "
                            f"marker_count={len(cached_markers.markers)}"
                        )
                return
        cache_lookup_end = time.perf_counter()
        geometry_build_start = time.perf_counter()
        ...
        for lane_idx in range(n_lanes):
            ...
            is_target_lane = target_lane_idx == lane_idx
            if is_target_lane:
                lane_colors = [
        ...
        geometry_build_end = time.perf_counter()
        if use_cache:
            lane_marker_cache[cache_key] = markers
        publish_start = time.perf_counter()
        ...
        if loop is not None:
            ...
            if loop % control_rate == 0:
                self.get_logger().info(
                    ...
                    f"{(cache_lookup_end - cache_lookup_start) * 1000.0:.3f}, "
                    "geometry_build_ms="
                    f"{(geometry_build_end - geometry_build_start) * 1000.0:.3f}, "
                    f"publish_ms={(publish_end - publish_start) * 1000.0:.3f}, "
                    "publish_skipped=False, "
                    f"total_ms={(timing_end - timing_start) * 1000.0:.3f}, "
                    f"marker_count={len(markers.markers)}"
                )
    ...
    def _control(self):
        wall_cycle_start = time.perf_counter()
        self._control_mpc_timing_records = []
        control_phase_wall_ms = {}
        ...
        # 制御周期を維持
        wall_sleep_start = time.perf_counter()
        self._control_rate.sleep()
        wall_after_sleep = time.perf_counter()
        ...
        ordinary_l0_entry_prohibited_active = (
            self._l0_entry_prohibited_at_center_wp(center_wp_temp)
        )
        l0_prohibited_old_soft_candidate = self._prepass_soft_candidate_lane_idx
        if ordinary_l0_entry_prohibited_active:
            # Keep physical passage for diagnostics, but remove L0 control intent.
            self._clear_prohibited_l0_soft_state()
        else:
            self._l0_prohibited_soft_recovery_active = False
        ...
        motionhold_record = self._motionhold_validated_full_width
        motionhold_record_invalid_reason = None
        if motionhold_record is not None:
            if motionhold_record["control_loop"] != self._loop - 1:
                motionhold_record_invalid_reason = "older_than_one_cycle"
            elif self._reference_path is not self._reference_pathN_center:
                motionhold_record_invalid_reason = "center_path_released"
            elif motionhold_record["mpc_id"] != id(self._mpc):
                motionhold_record_invalid_reason = "mpc_instance_changed"
            elif (
                self._mpc.current_prediction is None
                or motionhold_record["prediction_id"]
                    != id(self._mpc.current_prediction)
            ):
                motionhold_record_invalid_reason = "prediction_changed"
            elif self._mpc.infeasibility_counter != 0:
                motionhold_record_invalid_reason = "mpc_failure"
            elif getattr(self._mpc, "used_prediction_fallback", False):
                motionhold_record_invalid_reason = "prediction_fallback"
            elif self._mpc.recovery_requested:
                motionhold_record_invalid_reason = "mpc_recovery_requested"
            elif self._mpc.time_budget_exceeded:
                motionhold_record_invalid_reason = "mpc_time_budget_exceeded"
            elif self._mpc_safety_recovery_active:
                motionhold_record_invalid_reason = "safety_recovery"
            elif recovery_active:
                motionhold_record_invalid_reason = "reverse_recovery"
            elif self._post_reverse_full_width_recovery_active:
                motionhold_record_invalid_reason = "post_reverse_recovery"
            elif (
                ...
                or self._race_rejoin_handoff_active
                or self._l0_prohibited_soft_recovery_active
            ):
                motionhold_record_invalid_reason = "other_lateral_ownership"
        if motionhold_record_invalid_reason is not None:
            self._discard_motionhold_validated_full_width(
                motionhold_record_invalid_reason)
            motionhold_record = None
        ...
        physical_ego_lane_idx = self._lane_index_for_position(pose.x, pose.y)
        l0_prohibited_l1_hard_deferred = False
        l0_prohibited_l1_failed_readiness_conditions = ()
        l0_prohibited_l1_conflicts = {
            "front": [], "side": [], "rear": []}
        if (
            ordinary_l0_entry_prohibited_active
            and physical_ego_lane_idx in (1, 2)
            and not self._l1_probe_active
            and not self._l0_prohibited_soft_recovery_active
        ):
            # Do not create an L1-constrained problem until the existing V2X,
            # motion-stability and fresh-prediction entry gates are ready.
            # Actual constraint/MPC feasibility remains owned by the
            # controlled L1 probe below.
            l0_prohibited_l1_conflicts = classify_lane_conflicts(
                1,
                self._relative_lane_vehicle_samples(pose, float(v)),
                front_distance=self._prepass_lane_fallback_front_distance,
                side_distance=self._prepass_lane_fallback_side_distance,
                rear_distance=self._prepass_lane_fallback_rear_distance,
            )
            l0_prohibited_l1_failed_readiness_conditions = (
                l0_prohibited_l1_hard_defer_reasons(
                    l0_prohibited=ordinary_l0_entry_prohibited_active,
                    physical_lane_idx=physical_ego_lane_idx,
                    l1_conflicts=l0_prohibited_l1_conflicts,
                    heading_error=center_heading_error,
                    max_heading_error=(
                        self._center_lane_rejoin_max_heading),
                    lateral_speed=center_lateral_speed,
                    max_lateral_speed=(
                        self._center_lane_rejoin_max_lateral_speed),
                    yaw_rate=center_yaw_rate,
                    max_yaw_rate=self._center_lane_rejoin_max_yaw_rate,
                    l1_lateral_error=l1_lateral_error,
                    max_l1_lateral_error=self._l1_rejoin_max_lateral_error,
                    prediction_available=fresh_full_width_prediction,
                    prediction_fit=l1_prediction_fit_ratio,
                    minimum_prediction_fit=(
                        self._l1_prediction_min_fit_ratio),
                    # A stored L1 request is safe existing hard ownership only
                    # while the vehicle is physically in L1.  From physical
                    # L2 it is still a new L1-hard entry and must be gated.
                    l1_hard_already_owned=(
                        physical_ego_lane_idx == 1
                        and self._target_lane_idx == 1
                    ),
                )
            )
            l0_prohibited_l1_hard_deferred = bool(
                l0_prohibited_l1_failed_readiness_conditions)
        if (
            ordinary_l0_entry_prohibited_active
            and (
                physical_ego_lane_idx == 0
                or (
                    physical_ego_lane_idx == 1
                    and l0_prohibited_l1_hard_deferred
                )
            )
            and not self._l1_probe_active
            and not self._l0_prohibited_soft_recovery_active
        ):
            # Avoid jumping directly to L1 hard bounds from physical L0, or
            # from a physical L1 pose that is not entry-ready. Existing
            # full-width stability and controlled L1-probe gates apply.
            self._l1_safety_recovery_active = True
            self._l0_prohibited_soft_recovery_active = True
            self._l1_safety_recovery_stable_since = None
            self._l1_safety_recovery_context = "rejoin"
            self._l1_safety_reprobe_pending = True
            self._center_lane_rejoin_active = False
            self._center_lane_rejoin_constraint_released = False
            self._center_lane_rejoin_stable_since = None
            if l0_prohibited_l1_hard_deferred:
                blockers = sorted(set(
                    l0_prohibited_l1_conflicts["front"]
                    + l0_prohibited_l1_conflicts["side"]
                    + l0_prohibited_l1_conflicts["rear"]
                ))
                constraint_target = getattr(
                    self._mpc, "_constraint_target_lane", None)
                constraint_upper = getattr(
                    self._mpc, "_prediction_upper_bounds", None)
                constraint_lower = getattr(
                    self._mpc, "_prediction_lower_bounds", None)
                effective_width = math.nan
                if (
                    constraint_target == 1
                    and constraint_upper is not None
                    and constraint_lower is not None
                    and len(constraint_upper) == len(constraint_lower)
                    and len(constraint_upper) > 0
                ):
                    effective_width = float(np.nanmin(
                        np.asarray(constraint_upper, dtype=float)
                        - np.asarray(constraint_lower, dtype=float)))
                self.get_logger().warn(
                    "[L0ProhibitedL1HardDeferred] "
                    f"center_wp={center_wp_temp}, "
                    f"physical_ego_lane={physical_ego_lane_idx}, "
                    f"l1_v2x_clear="
                    f"{lane_conflicts_are_clear(l0_prohibited_l1_conflicts)}, "
                    f"heading="
                    f"{math.degrees(center_heading_error):.1f}deg/"
                    f"{math.degrees(self._center_lane_rejoin_max_heading):.1f}deg, "
                    ...
                    f"prediction_available={fresh_full_width_prediction}, "
                    f"prediction_fit={l1_prediction_fit_ratio:.2f}/"
                    f"{self._l1_prediction_min_fit_ratio:.2f}, "
                    f"failed_readiness_conditions="
                    f"{list(l0_prohibited_l1_failed_readiness_conditions)}, "
                    f"effective_width="
                    f"{effective_width if math.isfinite(effective_width) else 'unavailable_before_l1_probe'}, "
                    f"blocking_vehicle={blockers}, "
                    "selected_recovery_mode=full-width+l1-soft-then-probe",
                    throttle_duration_sec=1.0,
                )
        ...
        if trajectory_switched:
            ...
            for switched_mpc in (self._mpcN_race, self._mpcN_center):
                ...
                switched_mpc.current_prediction_times = None
                switched_mpc.current_constraint_prediction_times = None
        ...
        overtake_selection_start = time.perf_counter()
        ...
        # These classifications are required by ordinary candidate selection
        # below.  With no valid lead velocity they safely default to False.
        lead_is_stationary = (
        ...
        left_is_free = False
        right_is_free = False
        ordinary_horizon_candidate = False
        ordinary_horizon_motion_hold = False
        motionhold_candidate_vehicle_id = None
        motionhold_candidate_lane_idx = None
        motionhold_validated_commit = False
        motionhold_validated_source_loop = None
        # Leaving the geographic L0 prohibition must not immediately create a
        # new L0 commit while the L1 recovery started there still owns state.
        # This hold suppresses only ordinary L0 candidates; L2 remains usable.
        l0_recovery_exit_hold = bool(
        ...
        if (
            ...
            if is_overtake_zone:
                ...
                phase_ms = (time.perf_counter() - phase_start) * 1000.0
                control_phase_wall_ms["vehicle_passage"] = phase_ms
                if ENABLE_RUNTIME_DIAGNOSTICS and phase_ms > 20.0:
                    self.get_logger().info(
                        "[ControlPhaseTiming] "
                        f"loop={self._loop}, "
                        f"phase=vehicle_passage, wall={phase_ms:.1f}ms"
                    )
                # L0EntryProhibited applies early only to ordinary new
                # overtake candidates. Keep the physical passage result intact
                # for stopped/deadlock/Prepass/SafetyRecovery processing.
                ordinary_horizon_passage = dict(horizon_passage)
                ordinary_new_candidate = (
                    self._overtake_lane_idx not in (0, 2)
                )
                (
                    future_l0_prohibited,
                    future_l0_lookahead_end_wp,
                    future_l0_prohibited_zone,
                ) = self._future_l0_entry_prohibition(center_wp_temp)
                future_l0_prohibited = bool(
                    ordinary_new_candidate
                    and not lead_is_special_slow
                    and future_l0_prohibited
                )
                if (
                    ordinary_l0_entry_prohibited_active
                    or l0_recovery_exit_hold
                    or future_l0_prohibited
                ):
                    ordinary_horizon_passage[0] = False
                phase_start = time.perf_counter()
                ...
                phase_ms = (time.perf_counter() - phase_start) * 1000.0
                control_phase_wall_ms["horizon_conflicts"] = phase_ms
                if ENABLE_RUNTIME_DIAGNOSTICS and phase_ms > 20.0:
                    self.get_logger().info(
                        "[ControlPhaseTiming] "
                        f"loop={self._loop}, "
                        f"phase=horizon_conflicts, wall={phase_ms:.1f}ms"
                    )
                future_l0_candidate_excluded = bool(
                    future_l0_prohibited
                    and horizon_passage.get(0, False)
                    and lane_conflicts_are_clear(horizon_conflicts[0])
                )
                if future_l0_candidate_excluded:
                    future_l2_available = bool(
                        ordinary_horizon_passage.get(2, False)
                        and lane_conflicts_are_clear(horizon_conflicts[2])
                    )
                    self.get_logger().info(
                        "[L0FutureEntryProhibited] excluding a new L0 "
                        "candidate before binary lane selection: "
                        f"center_wp={center_wp_temp}, "
                        f"lookahead_end_wp={future_l0_lookahead_end_wp}, "
                        "prohibited_zone="
                        f"{future_l0_prohibited_zone}, candidate=L0, "
                        f"l2_available={future_l2_available}, "
                        "action=exclude_new_l0_candidate",
                        throttle_duration_sec=1.0,
                    )
                if l0_recovery_exit_hold:
                    horizon_l2_available = bool(
                        horizon_passage.get(2, False)
                        and lane_conflicts_are_clear(horizon_conflicts[2])
                    )
                    self.get_logger().info(
                        "[L0RecoveryExitHold] L1 recovery is unfinished; "
                        "suppressing only the ordinary HorizonProbe L0 "
                        "candidate: "
                        f"center_wp={center_wp_temp}, "
                        "l1_safety_recovery_active="
                        f"{self._l1_safety_recovery_active}, "
                        "l1_safety_reprobe_pending="
                        f"{self._l1_safety_reprobe_pending}, "
                        f"l1_probe_active={self._l1_probe_active}, "
                        f"l1_probe_context={self._l1_probe_context}, "
                        "horizon_l0_excluded=True, "
                        f"l2_available={horizon_l2_available}, "
                        "action=hold_new_horizon_l0_commit",
                        throttle_duration_sec=1.0,
                    )
                ordinary_safe_outer_lanes = [
                    lane_idx for lane_idx in (0, 2)
                    if ordinary_horizon_passage.get(lane_idx, False)
                    and lane_conflicts_are_clear(
                        horizon_conflicts.get(lane_idx, {}))
                ]
                ordinary_commit_outer_lanes = ordinary_safe_outer_lanes
                ordinary_commit_passage = ordinary_horizon_passage
                if (
                    ordinary_new_candidate
                    and self._prepass_short_failed_outer_lanes
                ):
                    ordinary_commit_outer_lanes = (
                        exclude_short_failed_normal_commit_candidates(
                            ordinary_safe_outer_lanes,
                            self._prepass_short_failed_outer_lanes,
                        )
                    )
                    excluded_short_failed_lanes = sorted(
                        set(ordinary_safe_outer_lanes)
                        - set(ordinary_commit_outer_lanes)
                    )
                    if excluded_short_failed_lanes:
                        ordinary_commit_passage = dict(
                            ordinary_horizon_passage)
                        for lane_idx in excluded_short_failed_lanes:
                            ordinary_commit_passage[lane_idx] = False
                        self.get_logger().info(
                            "[OvertakeHorizonShortFailedLaneExcluded] "
                            "binary-safe outer lanes that failed shortly "
                            "earlier in this target's attempt are excluded "
                            "from ordinary new commits: "
                            f"vehicle_id={opponent_vehicle_id}, "
                            "binary_safe_outer_lanes="
                            f"{ordinary_safe_outer_lanes}, "
                            "short_failed_outer_lanes="
                            f"{sorted(self._prepass_short_failed_outer_lanes)}, "
                            "remaining_commit_candidates="
                            f"{ordinary_commit_outer_lanes}, "
                            "action=exclude_short_failed_from_normal_commit",
                            throttle_duration_sec=1.0,
                        )
                ordinary_commit_passage = dict(ordinary_commit_passage)
                for lane_idx in (0, 2):
                    if lane_idx not in ordinary_commit_outer_lanes:
                        ordinary_commit_passage[lane_idx] = False
                if (
                    len(ordinary_commit_outer_lanes) == 2
                    and ordinary_new_candidate
                ):
                    phase_start = time.perf_counter()
                    (
                        ordinary_lane_quality,
                        ordinary_lane_limiting_vehicle,
                        ordinary_lane_tie_margin,
                    ) = self._ordinary_outer_lane_quality(pose, v)
                    phase_ms = (time.perf_counter() - phase_start) * 1000.0
                    control_phase_wall_ms["lane_quality"] = phase_ms
                    if ENABLE_RUNTIME_DIAGNOSTICS and phase_ms > 20.0:
                        self.get_logger().info(
                            "[ControlPhaseTiming] "
                            f"loop={self._loop}, "
                            f"phase=lane_quality, wall={phase_ms:.1f}ms"
                        )
                    selected_outer_lane, selection_reason = (
                        select_ranked_safe_outer_lane(
                            new_target_lane_idx,
                            ordinary_commit_passage,
                            horizon_conflicts,
                            ordinary_lane_quality,
                            tie_margin=ordinary_lane_tie_margin,
                        )
                    )
                    l0_v2x_margin, l0_wall_margin = (
                        ordinary_lane_quality[0])
                    l2_v2x_margin, l2_wall_margin = (
                        ordinary_lane_quality[2])
                    preferred_lane = (
                        new_target_lane_idx
                        if new_target_lane_idx in (0, 2) else 0
                    )
                    self.get_logger().info(
                        "[OvertakeLaneRanking] both outer lanes passed the "
                        "binary safety gate; selecting by predicted margin: "
                        f"vehicle_id={opponent_vehicle_id}, "
                        f"preferred_lane=L{preferred_lane}, "
                        f"l0_v2x_margin={l0_v2x_margin:.2f}m, "
                        f"l2_v2x_margin={l2_v2x_margin:.2f}m, "
                        f"l0_wall_margin={l0_wall_margin:.2f}m, "
                        f"l2_wall_margin={l2_wall_margin:.2f}m, "
                        "l0_limiting_vehicle="
                        f"{ordinary_lane_limiting_vehicle[0]}, "
                        "l2_limiting_vehicle="
                        f"{ordinary_lane_limiting_vehicle[2]}, "
                        f"selected_lane=L{selected_outer_lane}, "
                ...
                ordinary_horizon_candidate = selected_outer_lane in (0, 2)
                ...
                if selected_outer_lane not in (0, 2):
                    if (
                        self._outer_candidate_guidance is not None
                        and self._outer_candidate_guidance["source"] == "normal"
                    ):
                        self._discard_outer_candidate_guidance(
                            "ordinary_candidate_unavailable_or_unsafe")
                    self.get_logger().info(
            ...
            self._discard_outer_candidate_guidance("target_released")
        ...
        active_target_has_manoeuvre_state = bool(
            self._overtake_target_committed
        ...
        if active_target_physically_complete:
            ...
            self._complete_overtake_target_behind(
                ...
                source=(
                    "committed"
                    if self._overtake_target_committed
            ...
            self._overtake_target_committed = True
        overtake_target_locked = bool(
            active_overtake_target_id is not None
            and self._overtake_target_committed)
        if (
        ...
        if (
            ...
            if urgent_locked_target_switch:
                self.get_logger().warn(
                    ...
                    f"lane=L{self._overtake_lane_idx}, "
        ...
        preempt_evaluation_start = time.perf_counter()
        preempt_target_prep_ms = 0.0
        preempt_passage_evaluation_ms = 0.0
        preempt_lane_candidate_ms = 0.0
        preempt_prospective_ms = 0.0
        preempt_prospective_l0_ms = 0.0
        preempt_prospective_l2_ms = 0.0
        preempt_prediction_envelope_ms = 0.0
        preempt_prediction_envelope_detail = (
            {
                "guidance_check_ms": 0.0,
                "prediction_prepare_ms": 0.0,
                "geometry_prepare_ms": 0.0,
                "dynamic_v2x_prepare_ms": 0.0,
                "envelope_collision_ms": 0.0,
                "terminal_fit_ms": 0.0,
                "geometry_frenet_cache_hits": 0,
                "geometry_frenet_cache_misses": 0,
                "geometry_center_frenet_call_count": 0,
            }
            if ENABLE_RUNTIME_DIAGNOSTICS else None
        )
        preempt_envelope_collision_detail = (
            {} if ENABLE_RUNTIME_DIAGNOSTICS else None)
        preempt_stability_ms = 0.0
        preempt_commit_decision_ms = 0.0

        def timed_preempt_corridor_would_block(lane_idx):
            nonlocal preempt_prospective_ms
            nonlocal preempt_prospective_l0_ms
            nonlocal preempt_prospective_l2_ms
            if not ENABLE_RUNTIME_DIAGNOSTICS:
                return self._outer_commit_corridor_would_block(
                    source="preempt",
                    vehicle_id=opponent_vehicle_id,
                    lane_idx=lane_idx,
                    pose=pose,
                )
            prospective_start = time.perf_counter()
            result = self._outer_commit_corridor_would_block(
                source="preempt",
                vehicle_id=opponent_vehicle_id,
                lane_idx=lane_idx,
                pose=pose,
            ...
            elapsed_ms = (
                time.perf_counter() - prospective_start) * 1000.0
            preempt_prospective_ms += elapsed_ms
            if lane_idx == 0:
                preempt_prospective_l0_ms += elapsed_ms
            elif lane_idx == 2:
                preempt_prospective_l2_ms += elapsed_ms
            return result
        ...
        l1_rejoin_preempt_commit = False
        preempt_reference_handoff_captured = False
        ...
        preempt_target_prep_ms = (
            time.perf_counter() - preempt_evaluation_start) * 1000.0
        if preempt_eligible:
            preempt_passage_start = time.perf_counter()
            preempt_passage, _ = self._vehicle_passage(
                opponent_vehicle_id, pose)
            # This is also an ordinary new overtake request. Do not mutate the
            # physical passage used by recovery paths.
            ordinary_preempt_passage = dict(preempt_passage)
            if (
                ordinary_l0_entry_prohibited_active
                or l0_recovery_exit_hold
            ):
                ordinary_preempt_passage[0] = False
            preempt_passage_evaluation_ms = (
                time.perf_counter() - preempt_passage_start) * 1000.0
            preempt_lane_candidate_start = time.perf_counter()
            ...
            if l0_recovery_exit_hold:
                preempt_l2_available = bool(
                    preempt_passage.get(2, False)
                    and lane_conflicts_are_clear(preempt_conflicts[2])
                )
                self.get_logger().info(
                    "[L0RecoveryExitHold] L1 recovery is unfinished; "
                    "suppressing only the L1RejoinOvertakePreempt L0 "
                    "candidate: "
                    f"center_wp={center_wp_temp}, "
                    "l1_safety_recovery_active="
                    f"{self._l1_safety_recovery_active}, "
                    ...
                    f"l1_probe_active={self._l1_probe_active}, "
                    f"l1_probe_context={self._l1_probe_context}, "
                    "preempt_l0_excluded=True, "
                    f"l2_available={preempt_l2_available}, "
                    "action=hold_new_preempt_l0_commit",
                    throttle_duration_sec=1.0,
                )
            preempt_lane_idx = select_safe_outer_lane(
                new_target_lane_idx,
                ordinary_preempt_passage,
                preempt_conflicts,
            )
            preempt_lane_candidate_ms = (
                time.perf_counter() - preempt_lane_candidate_start) * 1000.0
            if (
                preempt_lane_idx in (0, 2)
                and timed_preempt_corridor_would_block(preempt_lane_idx)
            ):
                ordinary_preempt_passage[preempt_lane_idx] = False
                preempt_lane_idx = select_safe_outer_lane(
                    preempt_lane_idx,
                    ordinary_preempt_passage,
                    preempt_conflicts,
                ...
                if preempt_lane_idx in (0, 2):
                    if timed_preempt_corridor_would_block(preempt_lane_idx):
                        preempt_lane_idx = None
            if preempt_lane_idx in (0, 2):
                preempt_prediction_envelope_start = time.perf_counter()
                preempt_guidance_check_start = time.perf_counter()
                self._arm_outer_candidate_guidance(
                    vehicle_id=opponent_vehicle_id,
                    lane_idx=preempt_lane_idx,
                    source="preempt",
                    now_sec=current_time_sec,
                )
                preempt_guided_ready = (
                    self._outer_candidate_guided_prediction_ready(
                        opponent_vehicle_id, preempt_lane_idx)
                )
                if ENABLE_RUNTIME_DIAGNOSTICS:
                    preempt_prediction_envelope_detail[
                        "guidance_check_ms"
                    ] = (
                        time.perf_counter() - preempt_guidance_check_start
                    ) * 1000.0
                preempt_envelope_conflict = None
                preempt_terminal_fit = None
                if preempt_guided_ready:
                    preempt_geometry_frenet_cache = {}
                    preempt_envelope_conflict = (
                        self._candidate_lane_predicted_envelope_conflict(
                            pose, v, preempt_lane_idx, opponent_vehicle_id,
                            timing=(
                                preempt_prediction_envelope_detail
                                if ENABLE_RUNTIME_DIAGNOSTICS else None
                            ),
                    ...
                    preempt_terminal_fit = (
                        self._candidate_guided_prediction_terminal_fit(
                            pose, v, preempt_lane_idx, opponent_vehicle_id,
                            timing=(
                                preempt_prediction_envelope_detail
                                if ENABLE_RUNTIME_DIAGNOSTICS else None
                    ...
                    self._log_outer_candidate_trajectory_fit(
                        source="preempt",
                        vehicle_id=opponent_vehicle_id,
                        lane_idx=preempt_lane_idx,
                        fit_result=preempt_terminal_fit,
                    )
                if (
                    not preempt_guided_ready
                    or preempt_envelope_conflict is not None
                    or preempt_terminal_fit is None
                    or not preempt_terminal_fit["fit"]
                ):
                    if preempt_envelope_conflict is not None:
                        self.get_logger().info(
                            "[L1RejoinOvertakePreemptEnvelopeRejected] "
                            "candidate-guided trajectory is unsafe; keeping "
                            f"L1 rejoin: vehicle_id={opponent_vehicle_id}, "
                            f"candidate_lane=L{preempt_lane_idx}, "
                            "blocking_vehicle="
                            f"{preempt_envelope_conflict['vehicle_id']}, "
                            "predicted_lateral_clearance="
                            f"{preempt_envelope_conflict['predicted_lateral_clearance']}, "
                            "predicted_longitudinal_clearance="
                            f"{preempt_envelope_conflict['predicted_longitudinal_clearance']}, "
                            "prediction_time="
                            f"{preempt_envelope_conflict['prediction_time']}",
                            throttle_duration_sec=1.0,
                        )
                    preempt_lane_idx = None
                if ENABLE_RUNTIME_DIAGNOSTICS:
                    preempt_prediction_envelope_ms = (
                        time.perf_counter()
                        - preempt_prediction_envelope_start) * 1000.0
            if preempt_lane_idx in (0, 2):
                preempt_stability_start = time.perf_counter()
                candidate_heading = self._candidate_lane_heading(
                    preempt_lane_idx, wp)
                ...
                preempt_entry_stable = (
                    candidate_heading_error <= self._prepass_max_heading
                    and candidate_lateral_speed
                        <= self._center_lane_rejoin_max_lateral_speed
                    and center_yaw_rate
                        <= self._center_lane_rejoin_max_yaw_rate
                    and fresh_full_width_prediction
                )
                preempt_stability_ms = (
                    time.perf_counter() - preempt_stability_start) * 1000.0
                preempt_commit_decision_start = time.perf_counter()
                if preempt_entry_stable:
                    preempt_reference_handoff_captured = (
                        self._capture_outer_candidate_reference_handoff(
                            vehicle_id=opponent_vehicle_id,
                            lane_idx=preempt_lane_idx,
                            source="preempt",
                        )
                    ...
                    self._discard_outer_candidate_guidance(
                        "preempt_outer_commit")
                    self._cancel_l1_rejoin_for_overtake()
                    exclusive_l1_rejoin = False
                    new_target_lane_idx = preempt_lane_idx
                    l1_rejoin_preempt_commit = True
                    self._mpc.osqp_initialized = False
                    self.get_logger().warn(
                        "[L1RejoinOvertakePreempt] nearby forward vehicle has "
                        "a safe outer passing lane and vehicle state is stable; "
                        "cancelling L1 rejoin and "
                        f"selecting L{preempt_lane_idx}: vehicle_id="
                        f"{opponent_vehicle_id}, "
                    ...
                    self.get_logger().info(
                        "[L1RejoinOvertakePreemptHold] safe outer lane found "
                        "but vehicle state is not ready for a hard commit: "
                        f"vehicle_id={opponent_vehicle_id}, "
                        f"candidate=L{preempt_lane_idx}, "
                        f"heading_error="
                ...
                preempt_commit_decision_ms = (
                    time.perf_counter()
                    - preempt_commit_decision_start) * 1000.0
        candidate_commit_selection_start = time.perf_counter()
        motionhold_record = self._motionhold_validated_full_width
        if motionhold_record is not None:
            if not ordinary_horizon_candidate:
                self._discard_motionhold_validated_full_width(
                    "candidate_unavailable_or_unsafe")
            elif (
                motionhold_record["vehicle_id"] != opponent_vehicle_id
                or motionhold_record["lane_idx"] != new_target_lane_idx
            ):
                self._discard_motionhold_validated_full_width(
                    "candidate_vehicle_or_lane_changed")
        ...
        if recovery_active:
            ...
            new_latch_distance = (
                self._slow_lead_overtake_commit_distance
                if lead_is_special_slow
            ...
            if (
                ordinary_horizon_candidate
                and not existing_outer_latch
                and not l1_rejoin_preempt_commit
                and new_latch_within_distance
                and new_target_lane_idx in (0, 2)
            ):
                # Binary/ranking has selected the candidate. Verify the
                # prospective hard corridor before arming its soft guidance.
                phase_start = time.perf_counter()
                while (
                    new_target_lane_idx in (0, 2)
                    and self._outer_commit_corridor_would_block(
                        source="normal",
                        vehicle_id=opponent_vehicle_id,
                        lane_idx=new_target_lane_idx,
                        pose=pose,
                    )
                ):
                    ordinary_commit_passage[new_target_lane_idx] = False
                    ordinary_commit_outer_lanes = [
                        lane_idx for lane_idx in ordinary_commit_outer_lanes
                        if ordinary_commit_passage.get(lane_idx, False)
                    ]
                    new_target_lane_idx = select_safe_outer_lane(
                        new_target_lane_idx,
                        ordinary_commit_passage,
                    ...
                    ordinary_horizon_candidate = (
                        new_target_lane_idx in (0, 2))
                phase_ms = (time.perf_counter() - phase_start) * 1000.0
                control_phase_wall_ms["commit_corridor"] = phase_ms
                if ENABLE_RUNTIME_DIAGNOSTICS and phase_ms > 20.0:
                    self.get_logger().info(
                        "[ControlPhaseTiming] "
                        f"loop={self._loop}, "
                        f"phase=commit_corridor, wall={phase_ms:.1f}ms"
                    )
                if not ordinary_horizon_candidate:
                    new_target_lane_idx = 1
                else:
                    guided_lane_idx = new_target_lane_idx
                    self._arm_outer_candidate_guidance(
                        vehicle_id=opponent_vehicle_id,
                        lane_idx=guided_lane_idx,
                        source="normal",
                        now_sec=current_time_sec,
                    )
                    phase_start = time.perf_counter()
                    guided_prediction_ready = (
                        self._outer_candidate_guided_prediction_ready(
                            opponent_vehicle_id, guided_lane_idx)
                    )
                    guided_envelope_conflict = None
                    guided_terminal_fit = None
                    if guided_prediction_ready:
                        guided_envelope_conflict = (
                            self._candidate_lane_predicted_envelope_conflict(
                                pose,
                                v,
                                guided_lane_idx,
                        ...
                        self._log_outer_candidate_trajectory_fit(
                            ...
                            vehicle_id=opponent_vehicle_id,
                            lane_idx=guided_lane_idx,
                            fit_result=guided_terminal_fit,
                        )
                    phase_ms = (time.perf_counter() - phase_start) * 1000.0
                    control_phase_wall_ms[
                        "guided_candidate_checks"] = phase_ms
                    if ENABLE_RUNTIME_DIAGNOSTICS and phase_ms > 20.0:
                        self.get_logger().info(
                            "[ControlPhaseTiming] "
                            f"loop={self._loop}, "
                            f"phase=guided_candidate_checks, wall={phase_ms:.1f}ms"
                        )
                    if (
                        not guided_prediction_ready
                        or guided_envelope_conflict is not None
                        or guided_terminal_fit is None
                        or not guided_terminal_fit["fit"]
                    ):
                        if guided_envelope_conflict is not None:
                            self.get_logger().info(
                                "[OvertakeCandidateEnvelopeConflict] "
                                "candidate-guided full-width MPC trajectory "
                                "is unsafe; "
                                f"vehicle_id={opponent_vehicle_id}, "
                                f"candidate_lane=L{guided_lane_idx}, "
                                "blocking_vehicle="
                                f"{guided_envelope_conflict['vehicle_id']}, "
                                "predicted_lateral_clearance="
                                f"{guided_envelope_conflict['predicted_lateral_clearance']}, "
                                "predicted_longitudinal_clearance="
                                f"{guided_envelope_conflict['predicted_longitudinal_clearance']}, "
                                "prediction_time="
                                f"{guided_envelope_conflict['prediction_time']}, "
                                "action=hold_full_width_guidance",
                                throttle_duration_sec=1.0,
                            )
                        ordinary_horizon_candidate = False
                        new_target_lane_idx = None
                        ...
                        self.get_logger().info(
                            "[OuterCandidateGuidedEnvelopeClear] "
                            f"source=normal, vehicle_id={opponent_vehicle_id}, "
                            f"candidate_lane=L{guided_lane_idx}, "
                            "action=allow_existing_commit_gates",
                            throttle_duration_sec=1.0,
                        )
            if (
                self._motionhold_validated_full_width is not None
                and (
                    not new_latch_within_distance
                ...
                self._discard_motionhold_validated_full_width(
                    "candidate_not_eligible_for_ordinary_latch")
            ...
            if (
                ...
                ordinary_horizon_candidate
                and not existing_outer_latch
                and new_target_lane_idx == 0
                and new_latch_within_distance
                and not lead_is_special_slow
                and self._future_l0_entry_prohibition(center_wp_temp)[0]
            ):
                (
                    _,
                    final_l0_lookahead_end_wp,
                    final_l0_prohibited_zone,
                ) = self._future_l0_entry_prohibition(center_wp_temp)
                motionhold_record = self._motionhold_validated_full_width
                if (
                    motionhold_record is not None
                    and motionhold_record["vehicle_id"] == opponent_vehicle_id
                    and motionhold_record["lane_idx"] == 0
                ):
                    self._discard_motionhold_validated_full_width(
                        "future_l0_entry_prohibited")
                latch_candidate_vehicle_id = None
                latch_candidate_lane_idx = None
                new_target_lane_idx = None
                final_l2_available = bool(
                    ordinary_horizon_passage.get(2, False)
                    and lane_conflicts_are_clear(horizon_conflicts[2])
                )
                self.get_logger().info(
                    "[L0FutureEntryProhibited] blocking the final new L0 "
                    "hard commit: "
                    f"center_wp={center_wp_temp}, "
                    f"lookahead_end_wp={final_l0_lookahead_end_wp}, "
                    f"prohibited_zone={final_l0_prohibited_zone}, "
                    f"candidate=L0, l2_available={final_l2_available}, "
                    "action=block_final_l0_commit",
                    throttle_duration_sec=1.0,
                )
            elif (
                ordinary_horizon_candidate
                ...
                motionhold_record = self._motionhold_validated_full_width
                validated_record_matches_current_candidate = bool(
                    motionhold_record is not None
                    and motionhold_record["vehicle_id"]
                        == opponent_vehicle_id
                    and motionhold_record["lane_idx"]
                        == new_target_lane_idx
                    and motionhold_record["control_loop"] == self._loop - 1
                    and motionhold_record["mpc_id"] == id(self._mpc)
                    and self._mpc.current_prediction is not None
                    and motionhold_record["prediction_id"]
                        == id(self._mpc.current_prediction)
                    and self._mpc.infeasibility_counter == 0
                    and not getattr(
                ...
                candidate_fresh_full_width_prediction = bool(
                    fresh_full_width_prediction
                    or validated_record_matches_current_candidate
                )
                ordinary_horizon_motion_stable = (
                    candidate_heading_error <= self._prepass_max_heading
                    and candidate_lateral_speed
                        <= self._center_lane_rejoin_max_lateral_speed
                    and center_yaw_rate
                        <= self._center_lane_rejoin_max_yaw_rate
                    and candidate_fresh_full_width_prediction
                )
                if not ordinary_horizon_motion_stable:
                    ordinary_horizon_motion_hold = True
                    held_candidate_lane_idx = new_target_lane_idx
                    motionhold_candidate_vehicle_id = opponent_vehicle_id
                    motionhold_candidate_lane_idx = held_candidate_lane_idx
                    # Keep this cycle at full width. Do not create target/lane
                    # state, so the same physical candidate is evaluated again
                    # from fresh passage, conflict, and motion data next cycle.
                    latch_candidate_vehicle_id = None
                    latch_candidate_lane_idx = None
                    new_target_lane_idx = None
                    self.get_logger().info(
                        "[OvertakeHorizonMotionHold] safe outer corridor found "
                        "but vehicle state is not ready for a new hard latch: "
                        f"vehicle_id={opponent_vehicle_id}, "
                        f"candidate=L{held_candidate_lane_idx}, "
                        f"heading_error="
                        f"{math.degrees(candidate_heading_error):.1f}deg/"
                        f"{math.degrees(self._prepass_max_heading):.1f}deg, "
                        f"lateral_speed={candidate_lateral_speed:.2f}/"
                        ...
                        f"fresh_full_width_prediction="
                        f"{fresh_full_width_prediction}, "
                        f"distance={float(opponent_arc_distance):.2f}m/"
                        f"{new_latch_distance:.2f}m, "
                        "action=hold_new_latch",
                        throttle_duration_sec=1.0,
                    )
                elif validated_record_matches_current_candidate:
                    motionhold_validated_commit = True
                    motionhold_validated_source_loop = motionhold_record[
                        "control_loop"]
            (
                new_target_lane_idx,
                self._overtake_target_vehicle_id,
            ...
            if overtake_latch_started:
                preserve_preempt_handoff = bool(
                    l1_rejoin_preempt_commit
                    and preempt_reference_handoff_captured
                    and self._outer_candidate_reference_handoff_matches(
                        source="preempt",
                        vehicle_id=self._overtake_target_vehicle_id,
                        lane_idx=self._overtake_lane_idx,
                        control_loop=self._loop,
                    )
                )
                ...
                self._discard_outer_candidate_guidance(
                    "normal_outer_commit")
                if motionhold_validated_commit:
                    self.get_logger().info(
                        "[MotionHoldValidatedCommit] "
                        f"vehicle_id={self._overtake_target_vehicle_id}, "
                        f"lane=L{self._overtake_lane_idx}, "
                        f"source_loop={motionhold_validated_source_loop}, "
                        f"current_loop={self._loop}",
                        throttle_duration_sec=0.5,
                    )
                self._motionhold_validated_full_width = None
        recovery_lane_safety_start = time.perf_counter()
        if (
        ...
        if overtake_latch_started:
            ...
            self.get_logger().info(
                ...
                f"vehicle_id={self._overtake_target_vehicle_id}, "
                f"lane={lane_label}, new_commit_lane={lane_label}"
            )
            outer_commit_source = (
                "preempt" if l1_rejoin_preempt_commit else "normal")
            self._start_outer_commit_prediction_diagnostic(
                source=outer_commit_source,
                vehicle_id=self._overtake_target_vehicle_id,
                lane_idx=self._overtake_lane_idx,
                pose=pose,
                speed=v,
        ...
        retry_target_id = self._overtake_target_vehicle_id
            ...
            if (
                ...
                if not is_follow_retry_within_distance(
                    ...
                    self.get_logger().info(
                        ...
                        f"{self._slow_lead_overtake_commit_distance:.2f}m, "
        ...
        if (
            ...
            latched_lane_idx = int(self._overtake_lane_idx)
            ...
            predicted_envelope_conflict = (
                self._committed_lane_predicted_envelope_conflict(pose)
            )
            latched_lane_unsafe = bool(
                not lane_conflicts_are_clear(
                    live_conflicts[latched_lane_idx])
                or predicted_envelope_conflict is not None
            )
            ...
            if latched_lane_unsafe:
                if predicted_envelope_conflict is not None:
                    self._log_outer_commit_prediction_conflict_diagnostic(
                        predicted_envelope_conflict,
                        now_sec=current_time_sec,
                    )
                repeated_short_commit_failure = (
                    self._record_prepass_short_commit_failure(
                        latched_lane_idx,
                        current_time_sec,
                        "OvertakeLatchedLaneUnsafe",
                    )
                )
                # Keep the failed side in Prepass/attempt history, but stop
                # treating the released side as an active outer-lane commit.
                # A later formal L0/L2 commit assigns this field again.
                self._overtake_lane_idx = None
                ...
                if predicted_envelope_conflict is not None:
                    blocking_vehicle_ids.add(
                        predicted_envelope_conflict["vehicle_id"])
                blocking_speeds = []
                ...
                if blocking_speeds:
                    ...
                    self._prepass_dynamic_conflict_speed_limit = None
                if repeated_short_commit_failure:
                    self._switch_prepass_to_follow(
                        "both outer lanes had a provisional commit fail within "
                        "the existing recovery timeout"
                    )
                    self._mpc.osqp_initialized = False
                    self.get_logger().warn(
                        "[PrepassRepeatedCommitFailureFollow] both outer lanes "
                        "failed shortly after provisional commit; switching to "
                        "follow without requesting reverse: "
                    ...
                    self._prepass_fallback_commit_lane_idx = None
                    self._prepass_fallback_commit_success_since = None
                    # A committed outer lane becoming unsafe is itself the
                    # start of a neutral recovery episode.  Do not wait for a
                    # later soft candidate to become unsafe before disabling
                    # lateral objective ownership: the opposite candidate may
                    # be tracked, but it must earn the existing MPC/heading/
                    # lateral-speed/yaw stability interval before guidance.
                    self._enter_prepass_soft_neutral_hold(
                        current_candidate=latched_lane_idx,
                        next_candidate=(
                            opposite_lane_idx if opposite_lane_safe else None
                        ),
                        heading_error=center_heading_error,
                        lateral_speed=center_lateral_speed,
                        yaw_rate=center_yaw_rate,
                    ...
                    self._mpc.osqp_initialized = False
                    self.get_logger().warn(
                        "[OvertakeLatchedLaneUnsafe] predicted traffic entered "
                        "the committed passing corridor; releasing it immediately "
                        "to full width before probing the opposite side: "
                        f"vehicle_id={latched_target_id}, "
                        f"failed_lane=L{latched_lane_idx}, "
                        "candidate_lane="
                        f"{'L' + str(opposite_lane_idx) if opposite_lane_safe else 'none'}, "
                        f"longitudinal={latched_target_longitudinal:.2f}m, "
                        f"physical_passage={live_passage}, "
                        f"conflicts={live_conflicts}, "
                        "reason="
                        f"{'predicted_envelope_conflict' if predicted_envelope_conflict is not None else 'corridor_conflict'}, "
                        "blocking_vehicle="
                        f"{predicted_envelope_conflict['vehicle_id'] if predicted_envelope_conflict is not None else sorted(blocking_vehicle_ids)}, "
                        "current_lateral_clearance="
                        f"{predicted_envelope_conflict['current_lateral_clearance'] if predicted_envelope_conflict is not None else None}, "
                        "predicted_min_lateral_clearance="
                        f"{predicted_envelope_conflict['predicted_lateral_clearance'] if predicted_envelope_conflict is not None else None}, "
                        "predicted_longitudinal_clearance="
                        f"{predicted_envelope_conflict['predicted_longitudinal_clearance'] if predicted_envelope_conflict is not None else None}, "
                        "prediction_time="
                        f"{predicted_envelope_conflict['prediction_time'] if predicted_envelope_conflict is not None else None}, "
                        "prediction_step="
                        f"{predicted_envelope_conflict['prediction_step'] if predicted_envelope_conflict is not None else None}"
                    )
                self.get_logger().warn(
                    "[OvertakeCommittedLaneCleared] released unsafe outer "
                    "lane from active commit ownership while preserving its "
                    "failure history: "
                    f"failed_lane=L{latched_lane_idx}, "
                    f"cleared_committed_lane=L{latched_lane_idx}, "
                    "active_commit_after_clear=none, "
                    "recovery_mode="
                    f"{'follow' if repeated_short_commit_failure else 'full_width'}, "
                    "new_commit_lane=none"
                )
        ...
        if (
            ...
            prepass_soft_diagnostics = None
            current_soft_candidate = self._prepass_soft_candidate_lane_idx
            center_wp = self._carN_center.get_closest_waypoint(
                pose.x, pose.y)
            l0_prohibited = self._l0_entry_prohibited_at_center_wp(center_wp)
            excluded_lanes = (
                self._prepass_attempted_outer_lanes
                | self._prepass_short_failed_outer_lanes
            )
            binary_safe = {
                lane_idx: bool(
                    recovery_physical_passage.get(lane_idx, False)
                    and lane_conflicts_are_clear(passage_conflicts[lane_idx])
                    and not (lane_idx == 0 and l0_prohibited)
                    and lane_idx not in excluded_lanes
                )
                for lane_idx in (0, 2)
            }
            if (
                current_soft_candidate in (0, 2)
                and recovery_candidate_lane in (0, 2)
                and recovery_candidate_lane != current_soft_candidate
            ):
                lane_quality, limiting_vehicles, tie_margin = (
                    self._ordinary_outer_lane_quality(pose, v)
                )
                diagnostic_passage = {
                    lane_idx: binary_safe[lane_idx] for lane_idx in (0, 2)
                }
                quality_lane, quality_reason = select_ranked_safe_outer_lane(
                    current_soft_candidate,
                    diagnostic_passage,
                    passage_conflicts,
                    lane_quality,
                    tie_margin=tie_margin,
                )
                if not binary_safe[current_soft_candidate]:
                    reason = "current_became_unsafe"
                elif (
                    quality_lane == recovery_candidate_lane
                    and quality_reason in ("v2x_margin", "wall_margin")
                ):
                    reason = "opposite_quality_better"
                else:
                    reason = "raw_candidate_order"

                center_waypoint = self._reference_pathN_center.get_waypoint(
                    center_wp)
                heading_delta = pose.theta - center_waypoint.psi
                odom_lateral_speed = float(
                    self._odom.twist.twist.linear.y)
                signed_lateral_speed = (
                    float(v) * math.sin(heading_delta)
                    + odom_lateral_speed * math.cos(heading_delta)
                )
                prepass_soft_diagnostics = {
                    "vehicle_id": self._overtake_target_vehicle_id,
                    "current_candidate": f"L{current_soft_candidate}",
                    "raw_candidate": f"L{recovery_candidate_lane}",
                    "pending_candidate": (
                        f"L{self._prepass_soft_pending_lane_idx}"
                        if self._prepass_soft_pending_lane_idx in (0, 2)
                        else "none"
                    ),
                    "center_wp": center_wp,
                    "current_e_y": float(
                        self._carN_center.spatial_state.e_y),
                    "signed_lateral_speed": signed_lateral_speed,
                    "heading_error_l0_deg": math.degrees(
                        absolute_heading_difference(
                            pose.theta,
                            self._candidate_lane_heading(0, center_wp),
                        )),
                    "heading_error_l2_deg": math.degrees(
                        absolute_heading_difference(
                            pose.theta,
                            self._candidate_lane_heading(2, center_wp),
                        )),
                    "reason": reason,
                    "lanes": {
                        lane_idx: {
                            "physical_passage": bool(
                                recovery_physical_passage.get(
                                    lane_idx, False)),
                            "conflicts": passage_conflicts[lane_idx],
                            "binary_safe": binary_safe[lane_idx],
                            "v2x_margin": lane_quality[lane_idx][0],
                            "wall_margin": lane_quality[lane_idx][1],
                            "limiting_vehicle_id": limiting_vehicles[lane_idx],
                        }
                        for lane_idx in (0, 2)
                    },
                }
            self._latch_prepass_soft_candidate(
                recovery_candidate_lane,
                current_time_sec,
                diagnostics=prepass_soft_diagnostics,
            )
            if (
                not self._prepass_soft_neutral_hold_active
                ...
                self._outer_candidate_guidance is not None
                and self._outer_candidate_guidance["source"] == "fallback"
            ):
                self._discard_outer_candidate_guidance(
                    (
                        "fallback_neutral_stability_hold"
                        if self._prepass_soft_neutral_hold_active
                        else "fallback_candidate_unavailable_or_unsafe"
                    )
                )
            ...
            current_soft_candidate_unsafe = bool(
                current_soft_candidate in (0, 2)
                and not binary_safe[current_soft_candidate]
            )
            if (
                current_soft_candidate_unsafe
                and not self._prepass_soft_neutral_hold_active
            ):
                self._enter_prepass_soft_neutral_hold(
                    current_candidate=current_soft_candidate,
                    next_candidate=recovery_candidate_lane,
                    heading_error=candidate_heading_error,
                    lateral_speed=candidate_lateral_speed,
                    yaw_rate=center_yaw_rate,
            ...
            if self._prepass_soft_neutral_hold_active:
                self._clear_prepass_soft_objective()
                self.get_logger().info(
                    "[PrepassSoftNeutralHold] outer objective remains "
                    "disabled while full-width recovery re-evaluates a safe "
                    "and stable candidate: "
                    f"vehicle_id={self._overtake_target_vehicle_id}, "
                    "candidate="
                    f"{'L' + str(recovery_candidate_lane) if recovery_candidate_lane in (0, 2) else 'none'}, "
                    "candidate_binary_safe="
                    f"{binary_safe.get(recovery_candidate_lane, False)}, "
                    f"heading_error={math.degrees(candidate_heading_error):.1f}deg, "
            ...
            if (
                ...
                previous_timeout_heading_error = (
                    self._prepass_timeout_heading_error)
                previous_timeout_heading_text = (
                    f"{math.degrees(previous_timeout_heading_error):.1f}deg"
                    if previous_timeout_heading_error is not None
                    else "none"
                )
                same_timeout_candidate = bool(
                    retry_lane_idx in (0, 2)
                    and retry_lane_idx
                        == self._prepass_timeout_candidate_lane_idx
                    and previous_timeout_heading_error is not None
                )
                timeout_candidate_progressing = bool(
                    same_timeout_candidate
                    and candidate_heading_error
                        < previous_timeout_heading_error
                    and prepass_mpc_stable
                    and prepass_dynamics_stable
                )
                timeout_progress_action = "not_applicable"
                timeout_attempted_outer_lanes = sorted(
                    self._prepass_attempted_outer_lanes)
                ...
                if (
                    ...
                    retry_envelope_conflict = None
                    retry_terminal_fit = None
                    if retry_guided_ready:
                        retry_envelope_conflict = (
                            self._candidate_lane_predicted_envelope_conflict(
                                pose,
                                v,
                                retry_lane_idx,
                        ...
                        self._log_outer_candidate_trajectory_fit(
                            source="retry",
                            vehicle_id=self._overtake_target_vehicle_id,
                            lane_idx=retry_lane_idx,
                            fit_result=retry_terminal_fit,
                        )
                    retry_corridor_blocked = (
                        self._outer_commit_corridor_would_block(
                            source="retry",
                            vehicle_id=self._overtake_target_vehicle_id,
                            lane_idx=retry_lane_idx,
                            pose=pose,
                    ...
                    if (
                        not retry_guided_ready
                        or retry_envelope_conflict is not None
                        or retry_terminal_fit is None
                        or not retry_terminal_fit["fit"]
                    ):
                        self._prepass_fallback_recovery_stable_since = None
                        self._prepass_fallback_recovery_started_at = (
                            current_time_sec)
                        action = (
                            f"L{retry_lane_idx} candidate guidance has no "
                            "fresh clear MPC envelope proof; keeping "
                            "full-width recovery"
                        )
                    elif retry_corridor_blocked:
                        ...
                        self._prepass_fallback_recovery_started_at = (
                            current_time_sec)
                        action = (
                            f"prospective L{retry_lane_idx} corridor is below "
                            "the existing required width; keeping full-width "
                            "recovery and evaluating the opposite candidate"
                        )
                    else:
                        self._capture_outer_candidate_reference_handoff(
                            vehicle_id=self._overtake_target_vehicle_id,
                    ...
                    # The first unstable timeout records a baseline. A later
                    # timeout for the same candidate may continue only while
                    # heading error is strictly improving and MPC/dynamics are
                    # currently stable. Otherwise try the opposite outer lane
                    # within this episode; after both sides fail, fall back to
                    # follow. Reverse remains owned exclusively by the existing
                    # close-distance and rear-clear branch above.
                    first_or_changed_candidate = not same_timeout_candidate
                    if first_or_changed_candidate or timeout_candidate_progressing:
                        self._prepass_timeout_candidate_lane_idx = retry_lane_idx
                        self._prepass_timeout_heading_error = (
                            candidate_heading_error)
                        self._prepass_fallback_recovery_stable_since = None
                        self._prepass_fallback_recovery_started_at = (
                            current_time_sec)
                        timeout_progress_action = (
                            "recovery_continues_initial_baseline"
                            if first_or_changed_candidate
                            else "recovery_continues_heading_improving"
                        )
                        action = (
                            f"passage L{retry_lane_idx} exists but vehicle "
                            "state is unstable; keeping full-width recovery "
                            "and re-evaluating after the timeout interval"
                        )
                    else:
                        self._prepass_attempted_outer_lanes.add(retry_lane_idx)
                        timeout_attempted_outer_lanes = sorted(
                            self._prepass_attempted_outer_lanes)
                        self._clear_prepass_timeout_progress()
                        both_outer_lanes_attempted = all(
                            lane_idx in self._prepass_attempted_outer_lanes
                            for lane_idx in (0, 2)
                        )
                        if both_outer_lanes_attempted:
                            timeout_progress_action = (
                                "both_outer_lanes_nonprogressing_follow")
                            self._switch_prepass_to_follow(
                                "both outer-lane timeout candidates failed to "
                                "make heading/MPC/dynamics progress"
                            )
                            action = (
                                "both outer lanes were non-progressing; "
                                "switching to follow for a later retry"
                            )
                        else:
                            self._prepass_fallback_recovery_stable_since = None
                            self._prepass_fallback_recovery_started_at = (
                                current_time_sec)
                            timeout_progress_action = (
                                "candidate_nonprogressing_evaluate_opposite")
                            action = (
                                f"passage L{retry_lane_idx} exists but made no "
                                "progress; keeping full-width recovery and "
                                "evaluating the opposite outer lane"
                            )
                ...
                self.get_logger().warn(
                    ...
                    f"{math.degrees(candidate_heading_error):.1f}deg/"
                    f"{math.degrees(self._prepass_max_heading):.1f}deg, "
                    f"previous_timeout_heading_error="
                    f"{previous_timeout_heading_text}, "
                    f"same_candidate={same_timeout_candidate}, "
                    f"progressing={timeout_candidate_progressing}, "
                    f"attempted_outer_lanes="
                    f"{timeout_attempted_outer_lanes}, "
                    f"progress_action={timeout_progress_action}, "
                    f"mpc_stable={prepass_mpc_stable}, "
            ...
            neutral_stability_ready = (
                self._prepass_soft_neutral_stability_ready(
                    neutral_active=self._prepass_soft_neutral_hold_active,
                    candidate_lane=recovery_candidate_lane,
                    latched_lane=self._prepass_soft_candidate_lane_idx,
                    candidate_binary_safe=binary_safe.get(
                        recovery_candidate_lane, False),
                    mpc_stable=prepass_mpc_stable,
                    heading_stable=prepass_heading_stable,
                    dynamics_stable=prepass_dynamics_stable,
                )
            ...
            neutral_hold_exited_this_cycle = False
            if self._prepass_soft_neutral_should_exit(
                neutral_active=self._prepass_soft_neutral_hold_active,
                stability_ready=neutral_stability_ready,
                stable_elapsed=recovery_stable_elapsed,
                required_sec=self._prepass_lane_fallback_recovery_stable_sec,
            ):
                neutral_hold_exited_this_cycle = True
                self._prepass_soft_neutral_hold_active = False
                # Stability accumulated without an outer objective authorizes
                # SoftGuidance only. A hard commit must earn a fresh stable
                # interval after that objective is applied.
                self._prepass_fallback_recovery_stable_since = None
                self._clear_prepass_soft_objective()
                self.get_logger().info(
                    "[PrepassSoftNeutralHoldExit] safe candidate remained "
                    "stable in neutral full-width recovery; allowing its "
                    "SoftGuidance but not a hard commit this cycle: "
                    f"vehicle_id={self._overtake_target_vehicle_id}, "
                    f"selected_candidate=L{recovery_candidate_lane}, "
                    f"stable_elapsed={recovery_stable_elapsed:.2f}s, "
                    f"heading_error={math.degrees(candidate_heading_error):.1f}deg, "
                    f"lateral_speed={candidate_lateral_speed:.2f}m/s, "
                    f"yaw_rate={center_yaw_rate:.2f}rad/s"
                )
            ...
            if (
                self._prepass_fallback_recovery_active
                and not neutral_hold_exited_this_cycle
                ...
                fallback_guided_ready = bool(
                    selected_fallback_lane in (0, 2)
                    and self._outer_candidate_guided_prediction_ready(
                        self._overtake_target_vehicle_id,
                        selected_fallback_lane,
                    )
                )
                fallback_envelope_conflict = None
                fallback_terminal_fit = None
                if fallback_guided_ready:
                    fallback_envelope_conflict = (
                        self._candidate_lane_predicted_envelope_conflict(
                            pose,
                            v,
                            selected_fallback_lane,
                    ...
                    self._log_outer_candidate_trajectory_fit(
                        source="fallback",
                        vehicle_id=self._overtake_target_vehicle_id,
                        lane_idx=selected_fallback_lane,
                        fit_result=fallback_terminal_fit,
                    )
                fallback_corridor_blocked = bool(
                    selected_fallback_lane in (0, 2)
                    and self._outer_commit_corridor_would_block(
                        source="fallback",
                        vehicle_id=self._overtake_target_vehicle_id,
                ...
                if (
                    selected_fallback_lane in (0, 2)
                    and (
                        not fallback_guided_ready
                        or fallback_envelope_conflict is not None
                        or fallback_terminal_fit is None
                        or not fallback_terminal_fit["fit"]
                    )
                ):
                    self._prepass_fallback_recovery_stable_since = None
                    self.get_logger().info(
                        "[PrepassCandidateGuidedEnvelopeHold] keeping full-"
                        "width SoftGuidance until the same candidate has a "
                        "fresh clear MPC trajectory: "
                        f"vehicle_id={self._overtake_target_vehicle_id}, "
                        f"candidate_lane=L{selected_fallback_lane}, "
                        f"fresh={fallback_guided_ready}, blocker="
                        f"{None if fallback_envelope_conflict is None else fallback_envelope_conflict['vehicle_id']}",
                        throttle_duration_sec=1.0,
                    )
                elif fallback_corridor_blocked:
                    ...
                    self._prepass_fallback_recovery_started_at = (
                        current_time_sec)
                    self.get_logger().warn(
                        "[PrepassLaneFallback] prospective corridor blocked "
                        "the selected new outer commit; keeping full-width "
                        "recovery for opposite-candidate evaluation: "
                        f"vehicle_id={self._overtake_target_vehicle_id}, "
                        f"candidate_lane=L{selected_fallback_lane}, "
                        f"attempted_outer_lanes="
                        f"{sorted(self._prepass_attempted_outer_lanes)}"
                    )
                elif selected_fallback_lane is not None:
                    self._prepass_fallback_recovery_active = False
                    self._prepass_fallback_recovery_stable_since = None
                    self._prepass_fallback_recovery_started_at = None
        ...
        if (
            self._prepass_soft_neutral_hold_active
            and not self._prepass_fallback_recovery_active
        ):
            # Follow, reverse, probe, commit, and target-release transitions
            # end ownership of the neutral Prepass episode.
            self._clear_prepass_soft_guidance()

        corridor_ownership_start = time.perf_counter()
        ...
        l0_entry_prohibited_active = (
            self._l0_entry_prohibited_at_center_wp(center_wp_temp)
        )
        if l0_entry_prohibited_active:
            l2_requested_lane_before_defer = new_target_lane_idx
            l2_new_l1_hard_deferred = bool(
                should_defer_l0_prohibited_physical_l2_l1_hard(
                    l0_prohibited=l0_entry_prohibited_active,
                    physical_lane_idx=physical_ego_lane_idx,
                    requested_lane_idx=l2_requested_lane_before_defer,
            ...
            if l2_new_l1_hard_deferred:
                # A valid L2 request remains legal in the L0-prohibited zone.
                # Any other request would newly apply L1-hard from physical
                # L2, regardless of which upstream owner produced it.
                self._l1_safety_recovery_active = True
                self._l0_prohibited_soft_recovery_active = True
                self._l1_safety_recovery_stable_since = None
                self._l1_safety_recovery_context = "rejoin"
                ...
                self.get_logger().warn(
                    ...
                    f"center_wp={center_wp_temp}, "
                    f"physical_ego_lane={physical_ego_lane_idx}, "
                    f"requested_lane={l2_requested_lane_before_defer}, "
                    f"target_lane={self._target_lane_idx}, "
                    f"l1_v2x_clear="
                    f"{lane_conflicts_are_clear(l0_prohibited_l1_conflicts)}, "
                    f"heading={math.degrees(center_heading_error):.1f}deg/"
                    f"{math.degrees(self._center_lane_rejoin_max_heading):.1f}deg, "
                    f"lateral_speed={center_lateral_speed:.2f}/"
                    f"{self._center_lane_rejoin_max_lateral_speed:.2f}m/s, "
                    f"yaw_rate={center_yaw_rate:.2f}/"
            ...
            if (new_target_lane_idx in (None, 0)
                    and not self._l0_prohibited_soft_recovery_active):
                new_target_lane_idx = 1
            elif self._l0_prohibited_soft_recovery_active:
                new_target_lane_idx = None
        ...
        if new_target_lane_idx != prev_lane_idx:
            ...
            if overtake_latch_started and new_target_lane_idx in (0, 2):
                can_change_lane = True
            if ordinary_horizon_motion_hold:
                # Releasing to full width is the motion gate's safety action;
                # the ordinary lane-change cooldown must not retain stale L1.
                can_change_lane = True
        ...
        reference_handoff_start = time.perf_counter()
        # Apply target lane
        # --- 制約切り替え安定化ウィンドウ ---
        # target_lane_idx が変わった瞬間に制約を即時切り替えると OSQP が infeasible になりやすい。
        # 通常変更だけは一定時間 full-width とし、安全回復とL0禁止は下の
        # corridor resolver が一意な優先順位で即時適用する。
        ...
        if self._target_lane_idx != self._prev_applied_lane_idx:
            ...
            self._constraint_transition_until = (
                current_time_sec + OUTER_CONSTRAINT_TRANSITION_SEC)
            self._prev_applied_lane_idx = self._target_lane_idx
        ...
        applied_lane_idx, self._applied_corridor_mode = resolve_applied_corridor(
            ...
            # While physically recovering from L0, full width must outrank the
            # ordinary geographic L1 hard replacement. The controlled probe
            # restores hard L1 after the existing stability checks pass.
            l0_prohibited=(
                l0_entry_prohibited_active
                and not self._l0_prohibited_soft_recovery_active
            ),
            full_width_recovery=full_width_recovery_requested,
            transition_active=transition_requested,
        )
        in_transition = self._applied_corridor_mode == "lane_transition"
        self._reference_path.target_lane_idx = applied_lane_idx
        self._reference_pathN.target_lane_idx = applied_lane_idx
        applied_is_constrained = applied_lane_idx is not None
        self._reference_path.is_overtaking = applied_is_constrained
        self._reference_pathN.is_overtaking = applied_is_constrained
        if self._l1_probe_active and applied_lane_idx == 1:
        ...
        soft_rejoin_enabled = (
            ...
            and not recovery_active
            and (
                not self._prepass_fallback_recovery_active
                or l0_entry_prohibited_active
            )
            and (
                not self._prepass_fallback_commit_pending
                or l0_entry_prohibited_active
            )
            and not self._prepass_fallback_follow_active
            and not self._follow_escape_active
        ...
        if not l0_entry_prohibited_active:
            self._update_prepass_soft_reference(
                enabled=prepass_soft_guidance_enabled,
                lane_idx=self._prepass_soft_candidate_lane_idx,
                now_sec=current_time_sec,
            )
        outer_candidate_guidance_enabled = bool(
            self._outer_candidate_guidance is not None
            and self._outer_candidate_guidance["source"] in ("normal", "preempt")
            and self._reference_path is self._reference_pathN_center
            and applied_lane_idx is None
            and not self._mpc_safety_recovery_active
            and not self._post_reverse_full_width_recovery_active
        ...
        outer_candidate_guidance_applied = (
            self._update_outer_candidate_guidance_reference(
                enabled=outer_candidate_guidance_enabled,
                now_sec=current_time_sec,
            )
        )
        if l0_entry_prohibited_active:
            self.get_logger().info(
                "[L0ProhibitedSoftRecovery] "
                f"center_wp={center_wp_temp}, "
                f"physical_ego_lane={physical_ego_lane_idx}, "
                f"lateral_e_y={self._carN_center.spatial_state.e_y:.2f}m, "
                f"l1_lateral_error={l1_lateral_error:.2f}m, "
                f"old_soft_candidate="
                f"{'L' + str(l0_prohibited_old_soft_candidate) if l0_prohibited_old_soft_candidate in (0, 2) else 'none'}, "
                "recovery_target=L1, "
                f"corridor={'full-width' if applied_lane_idx is None else 'L' + str(applied_lane_idx) + '-hard'}, "
                f"heading_error={math.degrees(center_heading_error):.1f}deg, "
                f"lateral_speed={center_lateral_speed:.2f}m/s, "
                f"yaw_rate={center_yaw_rate:.2f}rad/s, "
                f"prediction_fit={l1_prediction_fit_ratio:.2f}/"
        ...
        self._update_overtake_transition_soft_reference(
            ...
            lane_idx=self._target_lane_idx,
            now_sec=current_time_sec,
            transition_end_sec=self._constraint_transition_until,
            transition_duration_sec=OUTER_CONSTRAINT_TRANSITION_SEC,
        )
        ...
        candidate_guidance_applied_for_solve = bool(
            outer_candidate_guidance_applied
            or (
                prepass_soft_guidance_enabled
                and self._outer_candidate_guidance is not None
                and self._outer_candidate_guidance["source"] == "fallback"
                and self._outer_candidate_guidance["lane_idx"]
                    == self._prepass_soft_candidate_lane_idx
            )
        )

        v2x_transaction_start = time.perf_counter()
        # The first committed solve owns precedence.  Otherwise a
        # candidate-guided full-width solve may consume only the immediately
        # preceding fresh full-width grid.  With no valid prior grid the solve
        # deliberately remains the conservative flattened warm-up and cannot
        # authorize a commit.
        commit_time_aligned_transaction = (
            self._activate_outer_commit_time_aligned_v2x())
        candidate_time_aligned_transaction = None
        if not commit_time_aligned_transaction:
            candidate_time_aligned_transaction = (
                self._activate_outer_candidate_time_aligned_v2x(
                    guidance_applied=candidate_guidance_applied_for_solve,
                    applied_lane_idx=applied_lane_idx,
                )
            )
        
        # MPCの実行
        if candidate_guidance_applied_for_solve:
            mpc_solve_source = "candidate"
        elif (
            recovery_active
            or self._mpc_safety_recovery_active
            or self._post_reverse_full_width_recovery_active
        ):
            mpc_solve_source = "fallback"
        else:
            mpc_solve_source = "normal"
        mpc_control_start = time.perf_counter()
        try:
            with self._stats.time_block("control"):
                u, max_delta = self._mpc.get_control()
        finally:
            mpc_control_end = time.perf_counter()
            self._record_mpc_solve_timing(
                self._mpc,
                mpc_solve_source,
                (mpc_control_end - mpc_control_start) * 1000.0,
            )
            # One solve only.  Subsequent outer-lane and ordinary MPC cycles
            # deliberately return to the existing conservative flatten mode.
            self._mpc.dynamic_v2x_by_step = None
            self._mpc.static_occupancy_data = None
            self._outer_commit_time_aligned_v2x = None
        self._metrics_observe_mpc_result()
        self._capture_outer_commit_actual_prediction_diagnostic(
            applied_lane_idx=applied_lane_idx,
            transition_duration_sec=OUTER_CONSTRAINT_TRANSITION_SEC,
            now_sec=current_time_sec,
        )
        self._capture_outer_candidate_guided_prediction(
            guidance_applied=candidate_guidance_applied_for_solve,
            applied_lane_idx=applied_lane_idx,
            time_aligned_transaction=candidate_time_aligned_transaction,
            candidate_state_time_sec=mpc_state_time_sec,
        )
        self._capture_latest_full_width_time_grid(
            full_width_ownership=bool(
                applied_lane_idx is None
                and self._mpc is self._mpcN_center
                and self._reference_path is self._reference_pathN_center
                and not commit_time_aligned_transaction
                and not self._mpc_safety_recovery_active
                and not self._post_reverse_full_width_recovery_active
                and not recovery_active
                and not self._parallel_abort_active
        ...
        motionhold_full_width_solve_valid = bool(
            ordinary_horizon_motion_hold
            and motionhold_candidate_vehicle_id is not None
            and motionhold_candidate_lane_idx in (0, 2)
            and self._reference_path is self._reference_pathN_center
            and applied_lane_idx is None
            and self._mpc.infeasibility_counter == 0
            and self._mpc.current_prediction is not None
            ...
            and not initial_start_boost_active
            and not initial_start_lateral_hold_active
            and not soft_rejoin_enabled
            and not race_handoff_soft_enabled
            and not initial_start_soft_l0_enabled
            and not prepass_soft_guidance_enabled
            and not outer_candidate_guidance_applied
            and not overtake_soft_transition_enabled
        )
        if motionhold_full_width_solve_valid:
            self._motionhold_validated_full_width = {
                "vehicle_id": motionhold_candidate_vehicle_id,
                "lane_idx": motionhold_candidate_lane_idx,
                "control_loop": self._loop,
                "mpc_id": id(self._mpc),
                "prediction_id": id(self._mpc.current_prediction),
            }
            self.get_logger().info(
                "[MotionHoldFullWidthValidated] "
                f"vehicle_id={motionhold_candidate_vehicle_id}, "
                f"lane=L{motionhold_candidate_lane_idx}, "
                f"loop={self._loop}, "
                f"prediction_id={id(self._mpc.current_prediction)}",
                throttle_duration_sec=0.5,
            )
        elif ordinary_horizon_motion_hold:
            self._discard_motionhold_validated_full_width(
                "motionhold_full_width_solve_not_valid")

        pure_pursuit_wall_safe_this_cycle = False
        pure_pursuit_v2x_safe_this_cycle = False
        pure_pursuit_safe_this_cycle = False
        pure_pursuit_v2x_blocker = None
        pure_pursuit_v2x_stop_requested = False
        wall_v2x_stop_requested = False
        legacy_wall_fallback_speed_limit = None
        if self._steering_fallback_enabled:
            ...
            if self._steering_fallback_armed:
                ...
                # Do not release fallback ownership merely because MPC solved
                # accurately for enough cycles. When PP still reports that the
                # current pose is outside the wall and a V2X vehicle occupies
                # the parallel envelope, the controlled-pass proof below must
                # continue to run every cycle.
                wall_v2x_reauthentication_required = False
                if self._steering_fallback_success_cycles >= (
                    self._steering_fallback_success_required
                ):
                    release_delta, _, _, release_reason = (
                        self._active_path_pure_pursuit_feedback(
                            predicted_pose, v))
                    if release_delta is not None:
                        release_safe, release_validation_reason, _ = (
                            self._pure_pursuit_feedback_is_safe(
                                predicted_pose, v, release_delta))
                        if not release_safe:
                            release_reason = release_validation_reason
                    if release_reason.startswith("wall_at_step=0"):
                        wall_v2x_reauthentication_required = bool(
                            self._wall_v2x_nearby_vehicle(predicted_pose)
                            is not None
                        )

                if (
                    self._steering_fallback_success_cycles >=
                        self._steering_fallback_success_required
                    and not wall_v2x_reauthentication_required
                ):
                    self._steering_fallback_armed = False
                    self._steering_fallback_success_cycles = 0
                    self.get_logger().info(
                    ...
                    if fallback_delta is not None:
                        pp_wall_safe, validation_reason, pp_rollout = (
                            self._pure_pursuit_feedback_is_safe(
                                predicted_pose, v, fallback_delta))
                        pure_pursuit_wall_safe_this_cycle = pp_wall_safe
                        if not pp_wall_safe:
                            fallback_delta = None
                            reason = validation_reason
                        else:
                            (
                                pure_pursuit_v2x_safe_this_cycle,
                                pure_pursuit_v2x_blocker,
                            ) = self._pure_pursuit_rollout_is_v2x_safe(
                                predicted_pose, pp_rollout)
                            pure_pursuit_safe_this_cycle = bool(
                                pure_pursuit_wall_safe_this_cycle
                                and pure_pursuit_v2x_safe_this_cycle)
                    if fallback_delta is None:
                        nearby_vehicle = (
                            self._wall_v2x_nearby_vehicle(predicted_pose)
                            if reason.startswith("wall_at_step=0")
                            else None
                        )
                        controlled_pass = False
                        pass_result = None
                        if nearby_vehicle is not None:
                            controlled_pass, pass_result = (
                                self._wall_v2x_controlled_pass_is_safe(
                                    predicted_pose,
                                    nearby_vehicle["vehicle_id"],
                                    mpc_fresh_accurate=solution_accurate,
                                )
                            )
                        if controlled_pass:
                            # Preserve this cycle's already calculated MPC
                            # speed and steering. Later longitudinal safety
                            # limiters remain authoritative.
                            fallback_delta = float(u[1])
                            controller = "mpc_controlled_pass_through"
                            self.get_logger().warn(
                                "[WallV2XPassThrough] fresh MPC proves a "
                                "wall-improving, vehicle-clear completed pass: "
                                f"wp={self._car.wp_id}, "
                                f"vehicle_id={nearby_vehicle['vehicle_id']}, "
                                "current_relative_longitudinal="
                                f"{pass_result['current_longitudinal']:.2f}m, "
                                "predicted_terminal_relative_longitudinal="
                                f"{pass_result['terminal_longitudinal']:.2f}m, "
                                "predicted_longitudinal_clearance="
                                f"{pass_result['terminal_longitudinal_clearance']:.2f}m, "
                                "wall_violation_current/end="
                                f"{pass_result['current_wall_violation']:.3f}/"
                                f"{pass_result['terminal_wall_violation']:.3f}m, "
                                "action=controlled_pass_through",
                                throttle_duration_sec=0.5,
                            )
                        else:
                            ...
                            if reason.startswith("wall_at_step="):
                                legacy_wall_fallback_speed_limit = max(
                                    float(getattr(
                                        self._cfg.mpc,
                                        "safety_recovery_speed",
                                        1.0,
                                    )),
                                    0.0,
                                )
                                u[0] = min(
                                    float(u[0]),
                                    legacy_wall_fallback_speed_limit,
                                )
                            if nearby_vehicle is not None:
                                wall_v2x_stop_requested = True
                                details = pass_result or {}
                                self.get_logger().warn(
                                    "[WallV2XStop] wall-unsafe Pure Pursuit "
                                    "with a nearby V2X vehicle has no proven "
                                    "safe pass: "
                                    f"wp={self._car.wp_id}, wall_reason={reason}, "
                                    f"vehicle_id={nearby_vehicle['vehicle_id']}, "
                                    "relative_longitudinal="
                                    f"{nearby_vehicle['longitudinal']:.2f}m, "
                                    "lateral_clearance="
                                    f"{nearby_vehicle['lateral_clearance']:+.2f}m, "
                                    f"mpc_fresh_accurate={solution_accurate}, "
                                    "wall_improving="
                                    f"{details.get('wall_improving', False)}, "
                                    f"v2x_safe={details.get('v2x_safe', False)}, "
                                    "predicted_pass_complete="
                                    f"{details.get('pass_complete', False)}, "
                                    "action=stop",
                                    throttle_duration_sec=0.5,
                                )
                    else:
                        if pure_pursuit_safe_this_cycle:
                            # Safety and emergency-brake processing later in
                            # this cycle may still reduce or stop this request.
                            u[0] = self._steering_fallback_speed
                        else:
                            # Keep PP steering ownership, but never replace an
                            # unsafe rollout with non-zero legacy feedback.
                            pure_pursuit_v2x_stop_requested = True
                            controller = "pure_pursuit_v2x_stop"
                            u[0] = 0.0
                            blocker = pure_pursuit_v2x_blocker or {}
                            self.get_logger().warn(
                                "[PurePursuitV2XUnsafe] wall-safe Pure "
                                "Pursuit rollout conflicts with V2X: "
                                f"wp={self._car.wp_id}, "
                                f"vehicle_id={blocker.get('vehicle_id')}, "
                                f"controller={controller}, wall_safe=True, "
                                "v2x_safe=False, current_distance="
                                f"{blocker.get('distance')}, "
                                "current_longitudinal_clearance="
                                f"{blocker.get('longitudinal_clearance')}, "
                                "current_lateral_clearance="
                                f"{blocker.get('lateral_clearance')}, "
                                "action=stop",
                                throttle_duration_sec=0.5,
                            )
                    u[1] = fallback_delta
        ...
        race_probe_wall_start = time.perf_counter()
        race_probe_get_control_before_ms = sum(
            record["get_control_wall_ms"]
            for record in self._control_mpc_timing_records
        )
        ...
        race_probe_wall_end = time.perf_counter()
        race_probe_get_control_after_ms = sum(
            record["get_control_wall_ms"]
            for record in self._control_mpc_timing_records
        )
        race_probe_get_control_wall_ms = max(
            race_probe_get_control_after_ms
            - race_probe_get_control_before_ms,
            0.0,
        )
        ...
        recovery_request_confirmed = bool(
            self._mpc.recovery_requested
            and self._mpc_recovery_request_cycles >= recovery_confirm_cycles
        )
        outer_lane_recovery_confirmed = bool(
            recovery_request_confirmed
            and not race_to_center_failure_grace_active
        )
        if self._prepass_fallback_commit_pending:
            commit_lane_applied = (
        ...
        if (
            ...
            self._prepass_last_confirmed_commit = (
                int(committed_lane_idx), float(current_time_sec))
            self._prepass_fallback_commit_pending = False
            self._prepass_fallback_commit_lane_idx = None
            self._prepass_fallback_commit_success_since = None
            self._prepass_attempted_outer_lanes.clear()
            self._clear_prepass_timeout_progress()
            self.get_logger().info(
                "[PrepassFallbackCommit] provisional commit confirmed: selected "
                "fallback lane was applied and solved successfully for the "
                "required duration; releasing exclusive ownership while keeping "
                f"attempt-level short-failure history: lane=L{committed_lane_idx}, "
                f"new_commit_lane=L{committed_lane_idx}, "
        ...
        if (
            ...
            repeated_short_commit_failure = (
                self._record_prepass_short_commit_failure(
                    applied_lane_idx,
                    current_time_sec,
                    "PrepassLaneFallbackSafetyTrigger: "
                    f"{self._mpc.failure_reason}",
                )
            )
            if repeated_short_commit_failure:
                self._switch_prepass_to_follow(
                ...
                # This solve failed and ownership is moving to full-width
                # recovery. Preserve the failed side separately, but do not
                # recreate an active outer-lane commit before a formal retry.
                self._overtake_lane_idx = None
                self._prepass_fallback_lane_idx = None
                self._prepass_fallback_blocked = False
                self._prepass_fallback_recovery_active = True
        ...
        if recovery_request_confirmed:
            ...
            if not self._mpc_safety_recovery_active:
                ...
                self._metrics_set_safety_recovery(True, current_time_sec)
            self._mpc_safety_recovery_active = True
            self._mpc_safety_recovery_success_cycles = 0
            u[0] = 0.0
        elif self._mpc_safety_recovery_active:
            ...
            if should_count_mpc_recovery_success(
                ...
                if (
                    ...
                    self._mpc_safety_recovery_active = False
                    self._metrics_set_safety_recovery(
                        False, current_time_sec)
                    self._mpc_safety_recovery_success_cycles = 0
        ...
        if precommit_safe_outer_corridor:
            ...
            safe_lane_idx = 2 if left_is_free else 0
            self.get_logger().info(
                "[SlowLeadPrecommitSpeedRelease] safe outer corridor is "
                "already verified; keeping normal approach speed before "
        ...
        if self.USE_OBSTACLE_AVOIDANCE:
            ...
            if hasattr(self, '_v2x_tracker'):
                ...
                # Pre-confirm moving traffic before it enters the 6 m
                # emergency zone.  The same strict early-clearance and fresh
                # MPC checks are used, so this removes the one-cycle brake at
                # zone entry without relaxing the collision criteria.
                for vehicle_id in active_vehicle_ids:
                    ...
                    preview_is_relevant = bool(
                        ...
                        <= self._moving_vehicle_brake_bypass_preview_distance
                    ...
                    if buf:
                        ...
                        if dist < EMERGENCY_BRAKE_DIST and dist > 0.01:
                            ...
                            if (
                                ...
                                if not lateral_envelopes_conflict:
                                    self.get_logger().info(
                                        "[EmergencyBrakeSkip] "
            ...
            self._metrics_set_emergency_brake(emergency_brake_active)
            ...
            slow_lead_active_speed_margin = (
                ...
                and self._reference_path.is_overtaking
                else self._slow_lead_overtake_speed_margin
            )
            if (
                slow_lead_speed_control_active
                and slow_lead_speed_match_required
            ):
                # Start preserving the longitudinal gap at the 25 m prepare
                # gate, before the 18 m outer-lane commit gate is crossed.
                ref_vel_kmph = min(
                    ref_vel_kmph,
        ...
        if self._mpc_safety_recovery_active:
            ...
            if (
                ...
                pure_pursuit_wall_safe_this_cycle
                and not pure_pursuit_v2x_safe_this_cycle
            ):
                u[0] = 0.0
                blocker = pure_pursuit_v2x_blocker or {}
                self.get_logger().warn(
                    "[MPCSafetyRecoveryForwardCreepBlockedV2X] "
                    f"wp={self._car.wp_id}, "
                    f"blocking_vehicle_id={blocker.get('vehicle_id')}, "
                    "action=hold_stop",
                    throttle_duration_sec=1.0,
                )
        if self._follow_escape_active:
        ...
        # Apply after every ordinary longitudinal override, including start
        # boost, follow restart, Pure Pursuit recovery creep, and disabled-
        # control deceleration. Existing stuck recovery may still take
        # ownership below under its own safety gates.
        if legacy_wall_fallback_speed_limit is not None:
            u[0] = min(float(u[0]), legacy_wall_fallback_speed_limit)

        # Stop owners remain stricter than the wall-unsafe legacy speed cap.
        if wall_v2x_stop_requested or pure_pursuit_v2x_stop_requested:
            u[0] = 0.0
        ...
        stuck_and_command_prep_start = time.perf_counter()
        ...
        if recovering_from_stuck:
            bug_acc_enabled = False
            acc = self._stuck_recovery_acceleration(float(u[0]), float(v))
            self.get_logger().info(
            ...
            or wall_v2x_stop_requested
            or pure_pursuit_v2x_stop_requested
        ):
            bug_acc_enabled = False
            acc = np.clip(
                self.KP * (u[0] - abs(v)),
        ...
        # Tracking diagnostic only.  This must not change control behavior.
        # Compare the current vehicle state, latest MPC prediction, and the
        # command which is about to be sent to AWSIM.
        if ENABLE_RUNTIME_DIAGNOSTICS and self._loop % 10 == 0:
            actual_frenet = self._center_frenet(
                float(pose.x), float(pose.y))
            actual_e_y = (
                None if actual_frenet is None
                else float(actual_frenet[1])
            )
            ...
            if self._mpc.current_prediction is not None:
                current_pred_x, current_pred_y = (
                    self._mpc.current_prediction
                )
                if len(current_pred_x) > 0 and len(current_pred_y) > 0:
                    pred_x = float(current_pred_x[0])
                    pred_y = float(current_pred_y[0])
                    pred_frenet = self._center_frenet(
                        pred_x, pred_y)
                    pred_e_y = (
                        None if pred_frenet is None
                        else float(pred_frenet[1])
                    )

                    prediction_times = getattr(
                        self._mpc,
                        "current_prediction_times",
                        None,
                    )
                    if (
                        prediction_times is not None
                        and len(prediction_times) > 0
                    ):
                        pred_t = float(prediction_times[0])
            ...
            self.get_logger().info(
                "[MPCTrackingDiagnostic] "
                f"loop={self._loop}, "
                f"dt={dt:.4f}s, "
                f"source={steering_source}, "
                f"actual_x={float(pose.x):.3f}, "
                f"actual_y={float(pose.y):.3f}, "
                f"actual_e_y={actual_e_y}, "
                f"speed={float(v):.3f}m/s, "
                f"pred_x={pred_x}, "
                f"pred_y={pred_y}, "
                f"pred_e_y={pred_e_y}, "
                f"pred_t={pred_t}, "
                f"cmd_speed={float(u[0]):.3f}m/s, "
                f"raw_steer={raw_steer:+.4f}rad, "
                f"published_steer={published_steer:+.4f}rad, "
                f"fallback_armed={self._steering_fallback_armed}"
            )
        ...
        publish_control_start = time.perf_counter()
        self._publish_control_command(now, u, acc, bug_acc_enabled)
        publish_control_end = time.perf_counter()

        if ENABLE_RUNTIME_DIAGNOSTICS:
            # State history and plots are visualization-only.  Final lap
            # summaries use the controller's lap counters, not these buffers.
            self._sim_logger.log(self._car, u, t)
            self._sim_logger.plot_animation(
                t, self._loop, self._current_laps, self._lap_times,
                is_colliding, u, self._mpc, self._car,
            )
        ...
        # Timing diagnostic only. Does not alter controller behavior.
        wall_cycle_end = time.perf_counter()
        sleep_wall_sec = wall_after_sleep - wall_sleep_start
        body_wall_sec = wall_cycle_end - wall_after_sleep
        total_wall_sec = wall_cycle_end - wall_cycle_start

        solve_call_count = sum(
            record["solve_call_count"]
            for record in self._control_mpc_timing_records
        )
        solve_wall_total_ms = sum(
            record["solve_wall_total_ms"]
            for record in self._control_mpc_timing_records
        )
        solve_wall_max_ms = max(
            (
                record["solve_wall_max_ms"]
                for record in self._control_mpc_timing_records
            ),
            default=0.0,
        )
        mpc_get_control_wall_ms = sum(
            record["get_control_wall_ms"]
            for record in self._control_mpc_timing_records
        )
        mpc_get_control_by_source_ms = {}
        for record in self._control_mpc_timing_records:
            source = str(record["source"])
            mpc_get_control_by_source_ms[source] = (
                mpc_get_control_by_source_ms.get(source, 0.0)
                + float(record["get_control_wall_ms"])
            )
        measured_phase_wall_ms = sum(control_phase_wall_ms.values())
        # get_control includes the optimizer solves and constraint generation.
        # Subtract it once; optimizer solve time is reported separately and is
        # intentionally not added again to the known-phase total.
        known_phase_total_ms = (
            measured_phase_wall_ms + mpc_get_control_wall_ms)
        unaccounted_ms = max(
            body_wall_sec * 1000.0 - known_phase_total_ms,
            0.0,
        )
        pre_mpc_ms = max(
            (mpc_control_start - wall_after_sleep) * 1000.0
            - measured_phase_wall_ms,
            0.0,
        )
        overtake_selection_existing_ms = sum(
            control_phase_wall_ms.get(name, 0.0)
            for name in (
                "vehicle_passage",
                "horizon_conflicts",
                "lane_quality",
        ...
        candidate_commit_existing_ms = sum(
            control_phase_wall_ms.get(name, 0.0)
            for name in (
                "commit_corridor",
                "guided_candidate_checks",
            )
        )
        state_and_target_prep_ms = max(
            (overtake_selection_start - wall_after_sleep) * 1000.0,
            0.0,
        )
        overtake_selection_ms = max(
            (preempt_evaluation_start - overtake_selection_start) * 1000.0
            - overtake_selection_existing_ms,
            0.0,
        )
        preempt_evaluation_ms = max(
            (candidate_commit_selection_start - preempt_evaluation_start)
            * 1000.0,
            0.0,
        )
        preempt_detail_total_ms = sum((
            preempt_target_prep_ms,
            preempt_passage_evaluation_ms,
            preempt_lane_candidate_ms,
            preempt_prospective_ms,
            preempt_prediction_envelope_ms,
            preempt_stability_ms,
            preempt_commit_decision_ms,
        ))
        other_preempt_ms = max(
            preempt_evaluation_ms - preempt_detail_total_ms,
            0.0,
        )
        preempt_prediction_envelope_detail_total_ms = sum(
            value for name, value
            in preempt_prediction_envelope_detail.items()
            if name.endswith("_ms")
        )
        preempt_prediction_envelope_other_ms = max(
            preempt_prediction_envelope_ms
            - preempt_prediction_envelope_detail_total_ms,
            0.0,
        )
        preempt_envelope_collision_detail_total_ms = sum((
            preempt_envelope_collision_detail.get(
                "frenet_projection_ms", 0.0),
            preempt_envelope_collision_detail.get(
                "clearance_evaluation_ms", 0.0),
        ))
        preempt_envelope_collision_other_ms = max(
            preempt_envelope_collision_detail.get("total_ms", 0.0)
            - preempt_envelope_collision_detail_total_ms,
            0.0,
        )
        candidate_commit_selection_ms = max(
            (recovery_lane_safety_start - candidate_commit_selection_start)
            * 1000.0
            - candidate_commit_existing_ms,
            0.0,
        )
        recovery_lane_safety_ms = max(
            (corridor_ownership_start - recovery_lane_safety_start) * 1000.0,
            0.0,
        )
        corridor_ownership_ms = max(
            (reference_handoff_start - corridor_ownership_start) * 1000.0,
            0.0,
        )
        reference_handoff_ms = max(
            (v2x_transaction_start - reference_handoff_start) * 1000.0,
            0.0,
        )
        v2x_transaction_ms = max(
            (mpc_control_start - v2x_transaction_start) * 1000.0,
            0.0,
        )
        pre_mpc_detail_total_ms = sum((
            state_and_target_prep_ms,
            overtake_selection_ms,
            preempt_evaluation_ms,
            candidate_commit_selection_ms,
            recovery_lane_safety_ms,
            corridor_ownership_ms,
            reference_handoff_ms,
            v2x_transaction_ms,
        ))
        other_pre_mpc_ms = max(
            pre_mpc_ms - pre_mpc_detail_total_ms,
            0.0,
        )
        post_mpc_fallback_ms = max(
            (race_probe_wall_start - mpc_control_end) * 1000.0,
            0.0,
        )
        race_probe_surrounding_ms = max(
            (race_probe_wall_end - race_probe_wall_start) * 1000.0
            - race_probe_get_control_wall_ms,
            0.0,
        )
        recovery_and_safety_ms = max(
            (stuck_and_command_prep_start - race_probe_wall_end) * 1000.0,
            0.0,
        )
        stuck_and_command_prep_ms = max(
            (publish_control_start - stuck_and_command_prep_start) * 1000.0,
            0.0,
        )
        publish_control_ms = max(
            (publish_control_end - publish_control_start) * 1000.0,
            0.0,
        )
        post_publish_tail_ms = max(
            (wall_cycle_end - publish_control_end) * 1000.0,
            0.0,
        )
        new_unaccounted_phase_total_ms = sum((
            pre_mpc_ms,
            post_mpc_fallback_ms,
            race_probe_surrounding_ms,
            recovery_and_safety_ms,
            stuck_and_command_prep_ms,
            publish_control_ms,
            post_publish_tail_ms,
        ))
        remaining_unaccounted_ms = max(
            body_wall_sec * 1000.0
            - known_phase_total_ms
            - new_unaccounted_phase_total_ms,
            0.0,
        )

        if self._loop % 10 == 0 or total_wall_sec > 0.050:
            self.get_logger().info(
                "[ControlTimingDiagnostic] "
                f"loop={self._loop}, "
                f"ros_dt={dt:.4f}s, "
                f"sleep_wall={sleep_wall_sec * 1000.0:.1f}ms, "
                f"body_wall={body_wall_sec * 1000.0:.1f}ms, "
                f"total_wall={total_wall_sec * 1000.0:.1f}ms"
            )
            self.get_logger().info(
                "[ControlTimingBreakdown] "
                f"loop={self._loop}, "
                f"body_wall_ms={body_wall_sec * 1000.0:.3f}, "
                f"solve_call_count={solve_call_count}, "
                f"solve_wall_total_ms={solve_wall_total_ms:.3f}, "
                f"solve_wall_max_ms={solve_wall_max_ms:.3f}, "
                "mpc_get_control_wall_ms="
                f"{mpc_get_control_wall_ms:.3f}, "
                "commit_corridor_ms="
                f"{control_phase_wall_ms.get('commit_corridor', 0.0):.3f}, "
                "guided_candidate_checks_ms="
                f"{control_phase_wall_ms.get('guided_candidate_checks', 0.0):.3f}, "
                "measured_non_mpc_phase_total_ms="
                f"{measured_phase_wall_ms:.3f}, "
                f"known_phase_total_ms={known_phase_total_ms:.3f}, "
                f"unaccounted_ms={unaccounted_ms:.3f}"
            )

        if (
            body_wall_sec * 1000.0 > 25.0
            or self._loop % 40 == 0
            or remaining_unaccounted_ms > 20.0
        ):
            self.get_logger().info(
                "[ControlUnaccountedTiming] "
                f"loop={self._loop}, "
                f"body_wall_ms={body_wall_sec * 1000.0:.3f}, "
                "existing_phase_total_ms="
                f"{measured_phase_wall_ms:.3f}, "
                "vehicle_passage_ms="
                f"{control_phase_wall_ms.get('vehicle_passage', 0.0):.3f}, "
                "horizon_conflicts_ms="
                f"{control_phase_wall_ms.get('horizon_conflicts', 0.0):.3f}, "
                "lane_quality_ms="
                f"{control_phase_wall_ms.get('lane_quality', 0.0):.3f}, "
                "commit_corridor_ms="
                f"{control_phase_wall_ms.get('commit_corridor', 0.0):.3f}, "
                "guided_candidate_checks_ms="
                f"{control_phase_wall_ms.get('guided_candidate_checks', 0.0):.3f}, "
                ...
                f"{mpc_get_control_wall_ms:.3f}, "
                "mpc_normal_ms="
                f"{mpc_get_control_by_source_ms.get('normal', 0.0):.3f}, "
                "mpc_candidate_ms="
                f"{mpc_get_control_by_source_ms.get('candidate', 0.0):.3f}, "
                "mpc_fallback_ms="
                f"{mpc_get_control_by_source_ms.get('fallback', 0.0):.3f}, "
                "mpc_probe_ms="
                f"{mpc_get_control_by_source_ms.get('probe', 0.0):.3f}, "
                f"pre_mpc_ms={pre_mpc_ms:.3f}, "
                "post_mpc_fallback_ms="
                f"{post_mpc_fallback_ms:.3f}, "
                "race_probe_surrounding_ms="
                f"{race_probe_surrounding_ms:.3f}, "
                "recovery_and_safety_ms="
                f"{recovery_and_safety_ms:.3f}, "
                "stuck_and_command_prep_ms="
                f"{stuck_and_command_prep_ms:.3f}, "
                f"publish_control_ms={publish_control_ms:.3f}, "
                f"post_publish_tail_ms={post_publish_tail_ms:.3f}, "
                "new_unaccounted_phase_total_ms="
                f"{new_unaccounted_phase_total_ms:.3f}, "
                "remaining_unaccounted_ms="
                f"{remaining_unaccounted_ms:.3f}"
            )

        if pre_mpc_ms > 20.0 or body_wall_sec * 1000.0 > 100.0:
            self.get_logger().info(
                "[PreMPCTiming] "
                f"loop={self._loop}, "
                "state_and_target_prep_ms="
                f"{state_and_target_prep_ms:.3f}, "
                f"overtake_selection_ms={overtake_selection_ms:.3f}, "
                f"preempt_evaluation_ms={preempt_evaluation_ms:.3f}, "
                "candidate_commit_selection_ms="
                f"{candidate_commit_selection_ms:.3f}, "
                "recovery_lane_safety_ms="
                f"{recovery_lane_safety_ms:.3f}, "
                f"corridor_ownership_ms={corridor_ownership_ms:.3f}, "
                f"reference_handoff_ms={reference_handoff_ms:.3f}, "
                f"v2x_transaction_ms={v2x_transaction_ms:.3f}, "
                f"other_pre_mpc_ms={other_pre_mpc_ms:.3f}, "
                "existing_pre_mpc_phase_total_ms="
                f"{overtake_selection_existing_ms + candidate_commit_existing_ms:.3f}, "
                f"pre_mpc_detail_total_ms={pre_mpc_detail_total_ms:.3f}, "
                f"total_pre_mpc_ms={pre_mpc_ms:.3f}"
            )

        if preempt_evaluation_ms > 20.0:
            self.get_logger().info(
                "[PreemptTiming] "
                f"loop={self._loop}, "
                f"eligible={preempt_eligible}, "
                f"target_prep_ms={preempt_target_prep_ms:.3f}, "
                "passage_evaluation_ms="
                f"{preempt_passage_evaluation_ms:.3f}, "
                f"lane_candidate_ms={preempt_lane_candidate_ms:.3f}, "
                f"prospective_ms={preempt_prospective_ms:.3f}, "
                f"prospective_l0_ms={preempt_prospective_l0_ms:.3f}, "
                f"prospective_l2_ms={preempt_prospective_l2_ms:.3f}, "
                "prediction_envelope_ms="
                f"{preempt_prediction_envelope_ms:.3f}, "
                f"stability_ms={preempt_stability_ms:.3f}, "
                f"commit_decision_ms={preempt_commit_decision_ms:.3f}, "
                f"other_preempt_ms={other_preempt_ms:.3f}, "
                f"detail_total_ms={preempt_detail_total_ms:.3f}, "
                f"total_preempt_ms={preempt_evaluation_ms:.3f}"
            )

        if preempt_prediction_envelope_ms > 20.0:
            self.get_logger().info(
                "[PreemptPredictionEnvelopeTiming] "
                f"loop={self._loop}, "
                "guidance_check_ms="
                f"{preempt_prediction_envelope_detail['guidance_check_ms']:.3f}, "
                "prediction_prepare_ms="
                f"{preempt_prediction_envelope_detail['prediction_prepare_ms']:.3f}, "
                "geometry_prepare_ms="
                f"{preempt_prediction_envelope_detail['geometry_prepare_ms']:.3f}, "
                "dynamic_v2x_prepare_ms="
                f"{preempt_prediction_envelope_detail['dynamic_v2x_prepare_ms']:.3f}, "
                "envelope_collision_ms="
                f"{preempt_prediction_envelope_detail['envelope_collision_ms']:.3f}, "
                "terminal_fit_ms="
                f"{preempt_prediction_envelope_detail['terminal_fit_ms']:.3f}, "
                "geometry_frenet_cache_hits="
                f"{preempt_prediction_envelope_detail['geometry_frenet_cache_hits']}, "
                "geometry_frenet_cache_misses="
                f"{preempt_prediction_envelope_detail['geometry_frenet_cache_misses']}, "
                "geometry_center_frenet_call_count="
                f"{preempt_prediction_envelope_detail['geometry_center_frenet_call_count']}, "
                f"other_ms={preempt_prediction_envelope_other_ms:.3f}, "
                "detail_total_ms="
                f"{preempt_prediction_envelope_detail_total_ms:.3f}, "
                "total_prediction_envelope_ms="
                f"{preempt_prediction_envelope_ms:.3f}"
            )
            if preempt_envelope_collision_detail:
                self.get_logger().info(
                    "[EnvelopeCollisionTiming] "
                    f"loop={self._loop}, "
                    "candidate_sample_count="
                    f"{preempt_envelope_collision_detail.get('candidate_sample_count', 0)}, "
                    "moving_vehicle_count="
                    f"{preempt_envelope_collision_detail.get('moving_vehicle_count', 0)}, "
                    "pair_check_count="
                    f"{preempt_envelope_collision_detail.get('pair_check_count', 0)}, "
                    "center_frenet_call_count="
                    f"{preempt_envelope_collision_detail.get('center_frenet_call_count', 0)}, "
                    "candidate_frenet_cache_hits="
                    f"{preempt_envelope_collision_detail.get('candidate_frenet_cache_hits', 0)}, "
                    "candidate_frenet_cache_misses="
                    f"{preempt_envelope_collision_detail.get('candidate_frenet_cache_misses', 0)}, "
                    "envelope_external_candidate_frenet_cache_hits="
                    f"{preempt_envelope_collision_detail.get('external_candidate_frenet_cache_hits', 0)}, "
                    "envelope_external_candidate_frenet_cache_misses="
                    f"{preempt_envelope_collision_detail.get('external_candidate_frenet_cache_misses', 0)}, "
                    "frenet_projection_ms="
                    f"{preempt_envelope_collision_detail.get('frenet_projection_ms', 0.0):.3f}, "
                    "clearance_evaluation_ms="
                    f"{preempt_envelope_collision_detail.get('clearance_evaluation_ms', 0.0):.3f}, "
                    f"other_ms={preempt_envelope_collision_other_ms:.3f}, "
                    "detail_total_ms="
                    f"{preempt_envelope_collision_detail_total_ms:.3f}, "
                    "total_ms="
                    f"{preempt_envelope_collision_detail.get('total_ms', 0.0):.3f}, "
                    "conflict_found="
                    f"{preempt_envelope_collision_detail.get('conflict_found', False)}, "
                    "early_returned="
                    f"{preempt_envelope_collision_detail.get('early_returned', False)}"
                )
    ...
    def run(self) -> None:
        ...
        self._sim_logger = SimulationLogger(
            self.get_logger(),
            self._car.temporal_state.x,
            self._car.temporal_state.y,
            bool(
                ENABLE_RUNTIME_DIAGNOSTICS
                and self._cfg.sim_logger.animation_enabled
            ),
            bool(ENABLE_RUNTIME_DIAGNOSTICS and self.SHOW_PLOT_ANIMATION),
            bool(ENABLE_RUNTIME_DIAGNOSTICS and self.PLOT_RESULTS),
    ...
    def stop(self):
        ...
        metric_now = self._metrics_now_sec()
        self._metrics_end_overtake(
            "aborted", metric_now, "controller_stop")
        self._metrics_set_safety_recovery(False, metric_now)
        if not self._metrics_summary_written:
            self._metrics_write_summary(
                "run", "", metric_now - self._metrics_run_started_sec,
                self._metrics_total)
            self._metrics_summary_written = True
