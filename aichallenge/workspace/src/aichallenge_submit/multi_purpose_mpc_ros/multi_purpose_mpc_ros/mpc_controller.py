...
class MPCController(Node):
    ...
    def _initialize(self) -> None:
        ...
        self._outer_shadow_probe_interval_cycles = max(int(getattr(
            switch_cfg, "outer_shadow_probe_interval_cycles", 2)), 1)
    ...
    def _run_outer_lane_shadow_probe(
        ...
        if success:
            ...
            self._outer_shadow_retry_countdown = (
                self._outer_shadow_probe_interval_cycles - 1)
        else:
            self._outer_shadow_success_cycles = 0
            self._outer_shadow_retry_countdown = (
                max(
                    self._outer_shadow_retry_interval_cycles,
                    self._outer_shadow_probe_interval_cycles,
                ) - 1)
            self.get_logger().info(
