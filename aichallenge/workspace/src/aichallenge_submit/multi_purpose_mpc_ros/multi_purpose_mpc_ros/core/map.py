...
def remove_small_holes(ar, area_threshold=5, connectivity=8):
    mask = ar.astype(bool)
    ...
    small_holes = np.where(sizes < area_threshold)[0]
    small_holes = small_holes[small_holes > 0]
    if len(small_holes) == 0:
        return mask
    holes_mask = np.isin(labeled, small_holes)
    return mask | holes_mask
...
def line_aa(x0, y0, x1, y1):
    ...
    while True:
        ...
        if e2 < dx:
            err += dx
            y += sy
    return np.array(x_list, dtype=np.int32), np.array(y_list, dtype=np.int32), None
