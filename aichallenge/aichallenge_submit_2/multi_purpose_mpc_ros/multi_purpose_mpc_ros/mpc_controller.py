...
class MPCController(Node):
    ...
    def _relative_lane_vehicle_samples(self, pose, ego_speed: float):
        ...
        for vehicle_id in self._v2x_tracker.active_vehicle_ids():
            ...
            longitudinal = self._center_relative_longitudinal(
                pose.x, pose.y, vehicle_x, vehicle_y, pose.theta)
            ...
            future_longitudinal = self._center_relative_longitudinal(
                ego_future_x, ego_future_y,
                future_x, future_y, pose.theta)
    ...
    def _latched_follow_target_state(self, pose, now_sec: float):
        ...
        return {
            ...
            "longitudinal": self._center_relative_longitudinal(
                pose.x, pose.y, target_x, target_y, pose.theta),
    ...
    def _center_relative_longitudinal(
        self, ego_x: float, ego_y: float,
        other_x: float, other_y: float,
        fallback_heading: float,
    ) -> float:
        """Return signed other-minus-ego distance along the Center path."""
        ego_frenet = self._center_frenet(ego_x, ego_y)
        other_frenet = self._center_frenet(other_x, other_y)
        if ego_frenet is not None and other_frenet is not None:
            arc_delta = signed_closed_path_arc_distance(
                ego_frenet[0], other_frenet[0],
                self._center_arc_total_length)
            if arc_delta is not None:
                return float(arc_delta)
        return relative_longitudinal_distance(
            other_x - ego_x, other_y - ego_y, fallback_heading)

    def _emergency_envelope_risk(
        self, pose: Pose2D, ego_speed: float, vehicle_id,
        vehicle_x: float, vehicle_y: float,
        forward_distance: float = 6.0,
        prediction_sec: float = 1.0,
    ):
        """Evaluate current/future body overlap in the Center Frenet frame.

        Lane IDs are intentionally not used as a veto.  The ego vehicle uses
        the live MPC prediction when available, while the other vehicle uses
        the V2X constant-velocity estimate.
        """
        ...
        lateral_limit = self._parallel_critical_clearance
        longitudinal_limit = self._parallel_safety_longitudinal_clearance
        horizon = max(float(prediction_sec), 0.0)

        ego_samples = [(0.0, float(pose.x), float(pose.y))]
        prediction = getattr(self._mpc, "current_prediction", None)
        if prediction is not None:
            pred_x, pred_y = prediction
            if pred_x and len(pred_x) == len(pred_y):
                for index, (ego_x, ego_y) in enumerate(zip(pred_x, pred_y)):
                    time_index = min(
                        index + 2, len(self._v2x_t_samples) - 1)
                    sample_time = float(self._v2x_t_samples[time_index])
                    if 0.0 < sample_time <= horizon:
                        ego_samples.append((
                            sample_time, float(ego_x), float(ego_y)))
        if len(ego_samples) == 1 and horizon > 0.0:
            for sample_time in np.linspace(0.2, horizon, 5):
                ego_samples.append((
                    float(sample_time),
                    pose.x + ego_speed * math.cos(pose.theta) * sample_time,
                    pose.y + ego_speed * math.sin(pose.theta) * sample_time,
                ))

        vehicle_vx, vehicle_vy = self._v2x_tracker.velocity(vehicle_id)
        best = None
        for sample_time, ego_x, ego_y in ego_samples:
            other_x = vehicle_x + vehicle_vx * sample_time
            other_y = vehicle_y + vehicle_vy * sample_time
            ego_frenet = self._center_frenet(ego_x, ego_y)
            other_frenet = self._center_frenet(other_x, other_y)
            ...
            if arc_delta is None:
                continue
            lateral_distance = abs(other_frenet[1] - ego_frenet[1])
            lateral_clearance = lateral_vehicle_clearance(
                lateral_distance, ego_width, other_half_width)
            longitudinal_clearance = longitudinal_vehicle_clearance(
                arc_delta, ego_half_length, other_half_length)
            risk = bool(
                -0.5 <= float(arc_delta) <= max(float(forward_distance), 0.0)
                and lateral_clearance <= lateral_limit
                and longitudinal_clearance <= longitudinal_limit
            )
            candidate = {
                "risk": risk,
                ...
                "longitudinal_clearance": float(longitudinal_clearance),
            }
            if risk:
                return candidate
            if best is None or (
                lateral_clearance + longitudinal_clearance
                < best["lateral_clearance"]
                    + best["longitudinal_clearance"]
            ):
                best = candidate
        return best or {
            "risk": False,
            "time": 0.0,
            "arc": math.inf,
            "lateral_distance": math.inf,
            "lateral_clearance": math.inf,
            "longitudinal_clearance": math.inf,
        }
    ...
    def _control(self):
        ...
        if self.USE_OBSTACLE_AVOIDANCE and hasattr(self, '_v2x_tracker'):
            for vid in self._v2x_tracker.active_vehicle_ids():
                ...
                if buf:
                    ...
                    center_longitudinal = self._center_relative_longitudinal(
                        pose.x, pose.y, opp_x, opp_y, pose.theta)

                    # Center arc owns front/rear classification.  Keep the
                    # waypoint magnitude only because switching thresholds
                    # are configured in waypoint units.
                    wp_separation = abs(wp_diff)
                    if center_longitudinal >= 0.0:
                        if wp_separation < closest_opp_ahead:
                            closest_opp_ahead = wp_separation
                            closest_opp_ahead_id = vid
                    else:
                        if wp_separation < closest_opp_behind:
                            closest_opp_behind = wp_separation
        ...
        min_wp_diff = 99999
        min_center_longitudinal = math.inf
        ...
        if self.USE_OBSTACLE_AVOIDANCE and hasattr(self, '_v2x_tracker'):
            ...
            for vid in self._v2x_tracker.active_vehicle_ids():
                ...
                if buf:
                    ...
                    rel_forward = self._center_relative_longitudinal(
                        pose.x, pose.y, opp_x, opp_y, pose.theta)
                    ...
                    if rel_forward > 0.0 and wp_diff < 40:
                        if rel_forward < min_center_longitudinal:
                            min_center_longitudinal = rel_forward
                            min_wp_diff = wp_diff
        ...
        if self._parallel_abort_active and not recovery_active:
            ...
            if abort_buf:
                ...
                abort_longitudinal = self._center_relative_longitudinal(
                    pose.x, pose.y, abort_x, abort_y, pose.theta)
        ...
        if latched_target_id is not None and hasattr(self, '_v2x_tracker'):
            ...
            if target_buf:
                ...
                latched_target_longitudinal = (
                    self._center_relative_longitudinal(
                        pose.x, pose.y, target_x, target_y, pose.theta)
                )
        ...
        if self.USE_OBSTACLE_AVOIDANCE:
            ...
            # Lane IDs are diagnostic only. Center arc owns front/rear
            # classification, and body-envelope overlap owns collision risk.
                ...
                for vid in self._v2x_tracker.active_vehicle_ids():
                    ...
                    if buf:
                        ...
                        if (
                            ...
                            continue
                        arc_delta = signed_closed_path_arc_distance(
                            ego_center_frenet[0], opp_center_frenet[0],
                            self._center_arc_total_length)
                        if arc_delta is None:
                            continue
                        envelope = self._emergency_envelope_risk(
                            pose, v, vid, opp_x, opp_y,
                            forward_distance=EMERGENCY_BRAKE_DIST,
                            prediction_sec=1.0,
                        )
                        lateral_dist = abs(
                            opp_center_frenet[1] - ego_center_frenet[1])
                        lateral_clearance = lateral_vehicle_clearance(
                            lateral_dist,
                        ...
                        current_risk = bool(
                            0.01 < arc_delta < EMERGENCY_BRAKE_DIST
                            and lateral_clearance
                                <= self._parallel_critical_clearance
                        )
                        if not (current_risk or envelope["risk"]):
                            continue
                        ...
                        opp_lane_idx = self._center_lane_index_for_offset(
                            opp_x, opp_y, opp_center_frenet[1])
                        same_lane = (
                        ...
                        if (
                            ...
                            and not envelope["risk"]
                        ):
                            continue

                        braking_arc = float(arc_delta)
                        if envelope["risk"]:
                            predicted_arc = max(0.01, float(envelope["arc"]))
                            if not current_risk:
                                braking_arc = predicted_arc
                            else:
                                braking_arc = min(braking_arc, predicted_arc)
                        braking_arc = max(braking_arc, 0.01)
                        ...
                        v_ref_emg = (
                            opp_spd
                            + K_p_emg * (braking_arc - d_target_emg)
                        )
                        ...
                        if (
                            ...
                            and braking_arc < emergency_stopped_blocker_dist
                        ):
                            emergency_stopped_blocker_id = vid
                            emergency_stopped_blocker_dist = braking_arc
                        min_emergency_speed = (
                        ...
                        if braking_arc < emergency_brake_vehicle_distance:
                            emergency_brake_vehicle_distance = braking_arc
                            emergency_brake_vehicle_id = vid
                        self.get_logger().warn(
                            f"[EmergencyBrake] vehicle_id={vid} "
                            f"arc={braking_arc:.2f}m, "
                            f"lateral={lateral_dist:.2f}m, "
                            f"lateral_clearance={lateral_clearance:.2f}m, "
                            f"ego_lane={ego_lane_idx}, "
                            f"opp_lane={opp_lane_idx}, "
                            f"predicted_envelope={envelope['risk']}, "
                            f"envelope_t={envelope['time']:.2f}s, "
                            f"pred_arc={envelope['arc']:+.2f}m, "
                            f"pred_lat_clear="
                            f"{envelope['lateral_clearance']:+.2f}m, "
                            f"pred_lon_clear="
                            f"{envelope['longitudinal_clearance']:+.2f}m. "
            ...
            if (
                ...
                if buf:
                    ...
                    longitudinal = self._center_relative_longitudinal(
                        pose.x, pose.y, opp_x, opp_y, pose.theta)
                    # Negative Center arc means the latched vehicle is behind.
                    if longitudinal < 0.0:
            ...
            if (
                ...
                for vid in self._v2x_tracker.active_vehicle_ids():
                    ...
                    if buf:
                        ...
                        dx = opp_x - pose.x
                        dy = opp_y - pose.y
                        longitudinal_d = self._center_relative_longitudinal(
                            pose.x, pose.y, opp_x, opp_y, pose.theta)
