...
class MPC:
    def __init__(self, model, N, Q, R, QN, StateConstraints, InputConstraints,
        ...
        self.QN = QN #終端コスト
        self.wp_id_offset = wp_id_offset #lookahead的なやつ
        self.use_obstacle_avoidance = use_obstacle_avoidance #障害物回避使うか
        self.use_path_constraints_topic = use_path_constraints_topic #経路制約をtopicで受け取るか。Falseなら内部計算、TrueならROSTopicで受信
        self.model = model #車両モデル
        self.nx = self.model.n_states#状態数(速度、横位置、ヨー、ヨーレート)
        self.nu = 2 #入力の次元数？[速度Vと曲率κ]
        self.state_constraints = StateConstraints #状態制約(横ずれや姿勢誤差)
        self.input_constraints = InputConstraints #入力制約[速度と曲率]
        self.ay_max = ay_max #横化速度制限　大きければ速く曲がる

        # 追加: ステアリングレート制限関連のパラメータ
        self.max_steering_rate = max_steering_rate #ステアリング角速度制限。1秒で何radまで変えられるか
        self.previous_steering = 0.0  # 前回のステア角

        # 追加: ay_maxによる速度制限の方式切り替え
        #未来全体で最も急なカーブをみるかどうか。Trueなら減速が早くなる
        self.use_max_kappa_pred = use_max_kappa_pred
        # 既存の初期化
        self.current_prediction = None
        #最適化問題を溶けきれなかったとき(解なし)の回数
        self.infeasibility_counter = 0
        #最後に解けた地点のWaypoint
        self.last_solved_wp_id = 0
        #現在の状態、次回のMPCでKappa_Predを計算するときの初期予測
        self.current_control = np.zeros((self.nu*self.N))
        #OSQPという計算機(ソルバ)を用意した。
        self.optimizer = osqp.OSQP()
        #ログの出力用カウンター
        self.debug_counter = 0

        #障害物回避をしない場合に実行する。
        #左右の境界を計算して後で使うlb,ubを作る準備
    ...
    def _init_problem(self, N, safety_margin):
        ...
        # Reference vector　参照軌道用ベクトル
        ur = np.zeros(nu_N) #入力目標
        xr = np.zeros(nx_N) #状態目標
        ...
        # 曲率予測を取得。前回MPCが予測した未来軌道から取得している
        ...
        # 全区間においてモデル、制約、目標を作るホライゾンループ
        for n in range(N):
            # Get waypoint information
            #最近傍Waypointと次のWaypointの取得
        ...
        if self.use_obstacle_avoidance and not self.use_path_constraints_topic:
            ub, lb, _ = self.model.reference_path.update_path_constraints(
                ...
                N, self.model.length, self.model.width, safety_margin) #安全マージンにより、コース幅よりも狭い範囲で設定をする
        else:
        ...
        # Get equality matrix
        # x_(k+1) - A_k * x_k - B_k * u_k = 0 になるようにする。
        ...
        # ステアリングレート制約の境界(上限と下限)
        # rate=0.5, Ts=0.1　なら　上限と下限は±0.05rad
